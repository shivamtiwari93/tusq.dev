...
> **M45 Charter Sketch Reservation — 2026-04-27, run_79db9c1f34791188, turn_99cd4b15e01e46ad, PM attempt 1, HEAD `9187fa7`.** Reserves the `### M45: Static Capability Output Schema Items Type Index` detail block for the dev materialization turn. The block will codify: purpose statement; command shape `tusq items index [--items-type <value>] [--manifest <path>] [--out <path>] [--json]`; closed nine-value `output_schema_items_type` bucket-key enum (`object | array | string | number | integer | boolean | null | not_applicable | unknown`); closed three-value `aggregation_key` enum (`items_type | not_applicable | unknown`); frozen tier function (output_schema null/non-object → unknown; output_schema.type non-string → unknown; output_schema.type !== 'array' → not_applicable; output_schema.type === 'array' AND items malformed/items.type non-string/items.type outside JSON-Schema primitive set → unknown; else → literal items.type value); the integer-NOT-collapsed-to-number rule (distinct from M42); frozen 8-field per-bucket entry shape; top-level `warnings[]` rule with five frozen reason codes (`output_schema_field_missing`, `output_schema_field_not_object`, `output_schema_items_field_missing_when_type_is_array`, `output_schema_items_field_not_object_when_type_is_array`, `output_schema_items_type_field_missing_or_invalid_when_type_is_array`); the `not_applicable`-bucket-emits-NO-warning rule; closed-enum bucket iteration order (`object → array → string → number → integer → boolean → null → not_applicable → unknown` — deterministic stable-output convention only, NOT UI-rendering-precedence-ranked); within-bucket manifest-declared-order rule; case-sensitive lowercase-only `--items-type` filter rule; empty-capabilities and stdout-discipline rules; non-persistence-of-`output_schema_items_type` rule; no-`oneOf`/`anyOf`/`allOf`-walking rule; no-tuple-style-walking rule; no-multi-type-walking rule; no-nested-array-walking rule (`items.items.type` NOT examined); read-only invariants extending M30–M44's byte-identity guard list to include `tusq description index`. Constraint 38 will be appended to the Constraints section (M45 is a planning aid; NOT a runtime response validator / frontend component generator / rendering completeness certifier). VISION sources cited (acceptance contract item 2): `.planning/VISION.md` lines 66–72 (`### Frontend, Design System, And Product UX` under § Input Sources — primary aggregation source, has NOT been the primary aggregation source for any shipped milestone) and `.planning/VISION.md` lines 19–32 (`### The Promise` — re-cited for the frontend-rendering-paraphrase framing only).
