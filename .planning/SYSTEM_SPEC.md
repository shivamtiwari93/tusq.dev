...
> **M67 Charter Sketch Reservation — 2026-04-28, run_a32ec6b13eb1a938, turn_87000dff8c907d84, PM attempt 1.** Reserves the M67 boundary for the dev turn to materialize. **M67 axis**: `input_schema.properties[firstKey].exclusiveMinimum` (JSON-Schema Draft 6+ finite-number numeric-EXCLUSIVE-lower-bound validation keyword — `value > exclusiveMinimum`). **Bucket-key enum (closed four-value)**: `lower_exclusive_bounded | lower_exclusive_unbounded | not_applicable | unknown`. **Aggregation_key enum (closed three-value)**: `numeric_lower_exclusive_bound_constraint | not_applicable | unknown`. **Six frozen warning reason codes** (5th: `input_schema_properties_first_property_descriptor_invalid`; 6th: `input_schema_properties_first_property_exclusive_minimum_invalid_when_present`). **Bucket iteration order**: `lower_exclusive_bounded → lower_exclusive_unbounded → not_applicable → unknown` (deterministic stable-output only — explicitly NOT an ecosystem-integration-compiler-criticality / Slack-slash-command-priority / Teams-action-input-strictness / Zendesk-Intercom-Salesforce-HubSpot-Gainsight-form-validator-priority / OpenAI-Claude-MCP-tool-call-wrapper-pre-validation-strictness / marketplace-package-strictness-tier / ecosystem-adapter-deprecation-priority / packaged-tool-fallback-guidance-strength ranking). **NULL-AS-ABSENT** (mirrors M55–M66): `exclusiveMinimum: null` → `lower_exclusive_unbounded` (no warning). **ZERO-IS-VALID-EXCLUSIVE-LOWER-BOUND (M67-SPECIFIC, mirrors M65)**: `exclusiveMinimum: 0` → `lower_exclusive_bounded` (no warning) — JSON-Schema Draft 6+ permits zero as a valid finite-number strict-lower bound. **NEGATIVE-IS-VALID-EXCLUSIVE-LOWER-BOUND (M67-SPECIFIC, mirrors M65)**: `exclusiveMinimum: -273.15` → `lower_exclusive_bounded` (no warning) — JSON-Schema permits arbitrary negative finite-number strict-lower bounds. **FRACTIONAL-IS-VALID-EXCLUSIVE-LOWER-BOUND (M67-SPECIFIC)**: `exclusiveMinimum: 0.5` / `exclusiveMinimum: -0.001` → `lower_exclusive_bounded` (no warning). **DRAFT-4-BOOLEAN-IS-INVALID (M67-SPECIFIC)**: JSON-Schema Draft 4 used `exclusiveMinimum: true` (BOOLEAN) paired with `minimum: <num>`; M67 reads ONLY the Draft 6+ FINITE-NUMBER form, so `exclusiveMinimum: true` / `exclusiveMinimum: false` → `unknown` WITH 6th code (cross-Draft polyfill deferred to `M-Above-Draft4-Crossref-1`). **STRICT predicate**: `typeof v === 'number' && Number.isFinite(v)` — NO `Number()`/`parseFloat()`/truthy coercion (NOT `>= 0`, NOT `> 0`, NOT `Number.isInteger`). NaN/Infinity/-Infinity/non-numeric/Draft-4-boolean → `unknown` WITH 6th code. **CLI surface**: 50 → 51. **New noun**: `above` inserted as the FIRST entry in the post-`docs` alphabetical block, immediately before `approve` (`above` (a=97, b=98) < `approve` (a=97, p=112) at position 1 (`b` (98) < `p` (112))). **Subcommand**: `index`. **Flags**: `--manifest`, `--json`, `--out` (rejects `.tusq/`), `--above` (case-sensitive lowercase only). **Result-array field**: `first_property_exclusive_minimum_states[]`. **Per-bucket field**: `input_schema_first_property_exclusive_minimum`. **Read-only invariant**: 37 prior peer index commands byte-identical pre/post. **Non-persistence**: `input_schema_first_property_exclusive_minimum` MUST NOT be written into `tusq.manifest.json`. **VISION primary**: `.planning/VISION.md` lines 535–537 (`### Ecosystem Integration Compiler`) — first milestone to use this section as primary aggregation source. **VISION re-cited**: lines 164–173 (`### Tools`) as structural anchor only. **Distinct from**: M44, M48, M49–M61, M62 (`.minLength`), M63 (`.maxLength`), M64 (`.multipleOf`), M65 (`.minimum` — INCLUSIVE-lower-bound `value >= min`, opposite equality semantics), M66 (`.maximum` — INCLUSIVE-upper-bound, opposite direction). **Deferred 11 successor milestones**: `M-Above-All-Properties-ExclusiveMinimum-Index-1`, `M-Above-Nested-Property-ExclusiveMinimum-Index-1`, `M-Above-Output-First-Property-ExclusiveMinimum-Index-1`, `M-Above-Minimum-Crossref-1`, `M-Above-Maximum-Crossref-1`, `M-Above-Below-Crossref-1` (exclusiveMinimum + exclusiveMaximum joint distribution / empty-feasible-set), `M-Above-Joint-Validity-Crossref-1`, `M-Above-Type-Applicability-Crossref-1`, `M-Above-Draft4-Crossref-1`, `M-Above-Persistence-1`, `M-Above-Doc-Contradiction-1`, `M-Above-LLM-ExclusiveMinimum-Inferrer-1`. Dev turn replaces this reservation with a full M67 § "Input Schema First Property ExclusiveMinimum Numeric-Exclusive-Lower-Bound Annotation Presence Index" section + Constraint 60 ("non-runtime-validator / non-doc-contradiction-detector / non-minimum-aggregator / non-exclusive-maximum-aggregator / non-Draft-4-polyfill / non-joint-validity-validator / non-type-applicability-validator / non-LLM-exclusive-minimum-inferrer / non-ecosystem-integration-compiler-criticality-tier-emitter / non-statistical-aggregator").
