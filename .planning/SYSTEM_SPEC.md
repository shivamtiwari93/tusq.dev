...
> **M63 Charter Sketch Reservation — 2026-04-28, run_e29754fde9f7e4f7, turn_9b08017116bfbfdc, PM attempt 1.** Reserves the `### M63: Static Capability Input Schema First Property MaxLength String-Length-Ceiling Annotation Presence Index` detail block for the dev materialization turn. PM-frozen scope (dev MUST carry forward verbatim): closed four-value `input_schema_first_property_max_length` bucket-key enum `length_capped | length_uncapped | not_applicable | unknown`; closed three-value aggregation_key enum `string_length_ceiling_constraint | not_applicable | unknown`; six frozen warning reason codes (FIFTH `input_schema_properties_first_property_descriptor_invalid` carried forward from M55–M62; SIXTH `input_schema_properties_first_property_max_length_invalid_when_present` axis-specific consolidating ALL non-non-negative-integer malformations — non-numeric values such as strings/booleans/arrays/objects, negative integers, fractional numbers like `1.5`, NaN, Infinity, -Infinity — under a single code); strict-numeric classifier with `Number.isInteger(firstVal.maxLength) && firstVal.maxLength >= 0` for `length_capped` (NO `Number()` coercion of strings, NO `parseInt()` coercion); NULL-AS-ABSENT rule (maxLength:null → length_uncapped, no warning — mirrors M55–M62 null-as-absent precedent); EXPLICIT-ZERO-IS-CAPPED rule (maxLength:0 → length_capped, no warning — mirrors M58/M60/M61/M62 explicit-default-value precedent; `0` is a valid non-negative integer per JSON-Schema and represents an operator-declared empty-string-only ceiling that is semantically distinct from absent); NO-TYPE-COERCION (maxLength:'1'/'0'/true/false/[1]/{} → unknown WITH 6th code; the `Number.isInteger` predicate evaluates to `false` on all non-number primitives and on NaN/Infinity); only `unknown` bucket emits warnings; bucket iteration order `length_capped → length_uncapped → not_applicable → unknown` (deterministic stable-output convention only — explicitly NOT an evals-and-regression-coverage / schema-drift-severity / permission-regression-priority / marketplace-package-validation-strictness-tier / UI-surface-smoke-test-criticality / workflow-test-priority / injection-payload-rejection-priority / tool-input-truncation-priority ranking); empty buckets MUST NOT appear; case-sensitive lowercase-only `--ceiling` filter; non-persistence rule (`input_schema_first_property_max_length` MUST NOT be written into `tusq.manifest.json`); CLI surface growth 46 → 47 with new top-level noun `ceiling` and single subcommand `index`, inserted alphabetically between `caption` and `choice` in the post-`docs` block; result-array field name `first_property_max_length_states`; per-bucket field name `input_schema_first_property_max_length`; 8-field per-bucket entry shape (`input_schema_first_property_max_length`, `aggregation_key`, `capability_count`, `capabilities[]`, `approved_count`, `gated_count`, `has_destructive_side_effect`, `has_restricted_or_confidential_sensitivity`); within-bucket order = manifest declared order; 33-peer-index byte-identity invariant; classifier MUST NOT cross-reference `minLength` (M62 axis — joint distribution deferred to `M-Ceiling-MinLength-Crossref-1`), `pattern` (M59 axis), or `format` (M53 axis); classifier MUST NOT validate that the property's `type === 'string'` (deferred to `M-Ceiling-Type-Applicability-Crossref-1`); classifier MUST NOT validate that `maxLength >= minLength` when both are present (pair-validity deferred to `M-Ceiling-MinLength-Crossref-1`); 10 deferred successor milestones (`M-Ceiling-All-Properties-MaxLength-Index-1`, `M-Ceiling-Nested-Property-MaxLength-Index-1`, `M-Ceiling-Output-First-Property-MaxLength-Index-1`, `M-Ceiling-MinLength-Crossref-1`, `M-Ceiling-Pattern-Crossref-1`, `M-Ceiling-Format-Crossref-1`, `M-Ceiling-Type-Applicability-Crossref-1`, `M-Ceiling-Persistence-1`, `M-Ceiling-Doc-Contradiction-1`, `M-Ceiling-LLM-MaxLength-Inferrer-1`). Constraint 56 RESERVED for the dev materialization turn: "non-runtime-validator / non-doc-contradiction-detector / non-minLength-aggregator / non-pattern-crossref / non-format-crossref / non-type-applicability-validator / non-LLM-maxLength-inferrer / non-evals-and-regression-coverage-tier-emitter / non-statistical-aggregator." M63-vs-M62 distinctness: `tusq ceiling index` aggregates per-property `input_schema.properties[firstKey].maxLength` (non-negative-integer string-length UPPER-bound — JSON-Schema Draft 4+ numeric validation keyword); `tusq floor index` aggregates per-property `input_schema.properties[firstKey].minLength` (non-negative-integer string-length LOWER-bound — JSON-Schema Draft 4+ numeric validation keyword) — orthogonal JSON-Schema string-length-bound keywords (upper-bound vs. lower-bound), read-only-invariant peers. The dev turn replaces this reservation block with the full § M63 section detailing the materialized constraint number and the binding to `src/cli.js` symbols.
