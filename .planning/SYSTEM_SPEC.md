...
### M44: Static Capability Description Word Count Tier Index — Charter Sketch Reservation

> **Charter Sketch Reservation — 2026-04-27, run_d309bfeea0f99431, turn_60fb9c5205835c3e, PM attempt 1, HEAD `ae45cfa`.** This block reserves a slot for the full M44 detail block (purpose statement, command shape, frozen four-value `description_word_count_tier` bucket-key enum, frozen two-value `aggregation_key` enum, frozen tier-function rules, frozen 8-field per-bucket entry shape, top-level `warnings[]` rule with three frozen reason codes, closed-enum bucket iteration order rule, within-bucket manifest-declared-order rule, case-sensitive `--tier` filter rule, empty-capabilities and stdout-discipline rules, read-only invariants including the explicit non-persistence-of-`description_word_count_tier` rule, the no-sub-schema-walking rule, the no-markdown-stripping rule, the no-per-language-handling rule, and the explicit non-runtime-doc-validator / non-doc-conformance-detector / non-description-generator / non-claim-completeness-certifier boundary statement, plus Constraint 37). The dev materialization turn will replace this Reservation with the full § M44 detail block under HEAD `ae45cfa`+. Frozen scope (PM-bound, dev MUST carry forward verbatim): `bucket-key enum = low | medium | high | unknown`; `aggregation_key enum = tier | unknown`; `tier function = description.trim().split(/\s+/u).length with low ≤7 / medium 8-14 / high ≥15 / unknown for missing/null/non-string/empty-after-trim`; `per-bucket entry = 8 fields (description_word_count_tier, aggregation_key, capability_count, capabilities[], approved_count, gated_count, has_destructive_side_effect, has_restricted_or_confidential_sensitivity)`; `warnings[] = always-present (--json), three frozen reasons (description_field_missing, description_field_not_string, description_field_empty_after_trim)`; `bucket order = low → medium → high → unknown (deterministic stable-output convention only — explicitly NOT doc-quality-ranked, NOT doc-completeness-ranked, NOT doc-richness-ranked, NOT doc-staleness-blast-radius-ranked, NOT customer-facing-truth-surface-ranked)`; `--tier filter = case-sensitive lowercase-only`; `non-persistence rule = description_word_count_tier MUST NOT be written into tusq.manifest.json`; `no-markdown-stripping rule = markdown / HTML tags / punctuation / numbers NOT stripped (whitespace-token splitting is naïve and Unicode-aware via the u flag)`; `no-sub-schema-walking rule = input_schema.description / output_schema.description / examples[].description NOT consulted`; `no-per-language-handling rule = CJK / RTL / no-whitespace scripts NOT grapheme-clustered (naïve whitespace-only tokenization)`; `threshold-pair immutability rule = the 7 / 14 thresholds are inline integer literals, post-ship-frozen`; `result-array field name = tiers (plural numeric-band-tier convention, consistent with M37/M38/M39/M40/M41)`; `CLI surface 27 → 28 with new noun description inserted alphabetically between confidence and diff`; `eval count 34 → ≥35`. Deferred successor milestones: M-Description-Char-Length-Tier-Index-1, M-Description-Sentence-Count-Tier-Index-1, M-Description-Sub-Schema-Word-Count-Tier-Index-1, M-Description-Per-Language-Word-Count-Tier-Index-1, M-Description-Cross-Axis-Claim-Richness-Index-1, M-Description-Per-Cause-Sub-Bucket-Index-1, M-Description-Word-Count-Tier-Threshold-Adjustment-1, M-Description-Persistence-1, M-Description-Doc-Contradiction-1, M-Description-Markdown-Strip-1, M-Description-Runtime-Conformance-1.
