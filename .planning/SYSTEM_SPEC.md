...
> PM has bound **M43: Static Capability Input Schema Primary Parameter Source Index Export from Manifest Evidence (~0.5 day) — V1.24 (PROPOSED)** as the next bounded roadmap increment, derived from intake charter `intent_1777305750366_23b6` (vision_scan, category `roadmap_exhausted_vision_open`) at HEAD `03440e6`. The dev materialization turn MUST add a fully-detailed `### M43` block to this file containing: purpose statement; command shape `tusq request index [--source <value>] [--manifest <path>] [--out <path>] [--json]`; the closed seven-value `input_schema_primary_parameter_source` bucket-key enum (`path | request_body | query | header | mixed | none | unknown`); the closed two-value `aggregation_key` enum (`source | unknown`); the frozen tier function (closed four-value property-`source` set match `{path, request_body, query, header}` → primary locus / `mixed` / `none` / `unknown` catchall as defined in ROADMAP § M43 Charter Bound); the frozen 8-field per-bucket entry shape (`input_schema_primary_parameter_source`, `aggregation_key`, `capability_count`, `capabilities[]`, `approved_count`, `gated_count`, `has_destructive_side_effect`, `has_restricted_or_confidential_sensitivity`); the top-level `warnings[]` rule (always present in `--json` mode for shape stability) with five frozen reason codes (`input_schema_field_missing`, `input_schema_field_not_object`, `input_schema_properties_field_missing`, `input_schema_properties_field_not_object`, `input_schema_property_source_field_missing_or_invalid`); the closed-enum bucket iteration order rule (`path → request_body → query → header → mixed → none → unknown`); the within-bucket manifest-declared-order rule; the case-sensitive lowercase-only `--source` filter rule; the empty-capabilities and stdout-discipline rules; the read-only invariants (manifest mtime + SHA-256 + per-capability `capability_digest` byte-identical pre/post; `tusq compile` byte-identical; `tusq surface plan`, `tusq domain index`, `tusq effect index`, `tusq sensitivity index`, `tusq method index`, `tusq auth index`, `tusq confidence index`, `tusq pii index`, `tusq examples index`, `tusq input index`, `tusq output index`, `tusq path index`, `tusq response index` byte-identical pre/post); the explicit non-persistence-of-`input_schema_primary_parameter_source` rule; the `cookie`/`file`/`multipart`/`form-data` → `unknown` rule; the array-of-sources → `unknown` rule; the `mixed` bucket no-pair-enumeration rule; the optional-vs-required collapse rule; and the explicit non-runtime-request-executor / non-request-payload-validator / non-input-contract-conformance-detector / non-request-generator / non-input-contract-certifier boundary statement. Constraint 36 ("`tusq request index` is a planning aid that surfaces what HTTP-request input parameter sources the manifest's capabilities declare across `input_schema.properties[*].source` and the cross-axis side-effect/sensitivity exposure of each source bucket. ...") MUST be added to the Constraints section in the same dev turn. The frozen seven-value `input_schema_primary_parameter_source` bucket-key enum, the closed two-value `aggregation_key` enum (`source | unknown`), the closed four-value property-`source` value set (`path`, `request_body`, `query`, `header`), and the five-value warning reason-code enum are immutable post-ship; any addition is a material governance event requiring a fresh ROADMAP entry.
