...
PM bound **M42: Static Capability Output Schema Top-Level Type Index Export from Manifest Evidence (~0.5 day) — V1.23 (PROPOSED)** in this turn under intake charter `intent_1777302673460_98ce` (vision_scan, category `roadmap_exhausted_vision_open`). VISION sources cited: `.planning/VISION.md` lines 48–54 (`### Database, Warehouse, And Data Model` — primary aggregation source, fresh; "DTOs, serializers, validation layers" + "data lineage between operational tables, APIs, analytics events, and customer-facing surfaces") and `.planning/VISION.md` lines 40–46 (`### Code And API Surface` — re-cited for output-schema-top-level-type-derivation framing). Charter Bound (PM-frozen): closed seven-value `output_schema_top_level_type` bucket-key enum (`object | array | string | number | boolean | null | unknown`); closed two-value `aggregation_key` enum (`type | unknown`); tier function as literal exact-string match against the closed six-value JSON Schema 2020-12 spec primitive set (`object`, `array`, `string`, `number`, `boolean`, `null`); `'integer'` and any other non-spec-primitive string value bucketed as `unknown` (integer-subset distinction reserved for `M-Output-Type-Integer-Subset-Index-1`); compositional schemas (`oneOf`/`anyOf`/`allOf`) without top-level `type` bucketed as `unknown` (no walking); array-of-types (`type: ['object', 'null']`) bucketed as `unknown` (no array unwrapping); frozen 8-field per-bucket entry shape (`output_schema_top_level_type`, `aggregation_key`, `capability_count`, `capabilities[]`, `approved_count`, `gated_count`, `has_destructive_side_effect`, `has_restricted_or_confidential_sensitivity`); top-level `warnings[]` only in `--json` mode (always present, even when empty `[]`) with five frozen reason codes (`output_schema_field_missing`, `output_schema_field_not_object`, `output_schema_type_field_missing`, `output_schema_type_field_not_string`, `output_schema_type_field_value_not_in_json_schema_primitive_set`); closed-enum bucket iteration order `object → array → string → number → boolean → null → unknown` (deterministic stable-output convention only — explicitly NOT data-contract-completeness-ranked, NOT shape-complexity-ranked, NOT data-lineage-blast-radius-ranked, NOT DTO-richness-ranked, NOT JSON-Schema-spec-precedence-ranked); within-bucket manifest declared order; case-sensitive lowercase-only `--type` filter; non-persistence rule (`output_schema_top_level_type` MUST NOT be written into `tusq.manifest.json`); read-only invariants (manifest mtime + SHA-256 + every capability_digest byte-identical pre/post; `tusq compile`, `tusq surface plan`, `tusq domain index`, `tusq effect index`, `tusq sensitivity index`, `tusq method index`, `tusq auth index`, `tusq confidence index`, `tusq pii index`, `tusq examples index`, `tusq input index`, `tusq output index`, `tusq path index` all byte-identical pre/post); CLI surface 25 → 26 with new noun `response` and single subcommand `index` inserted alphabetically between `redaction` and `sensitivity` (rationale: result field name `types` is plural and categorical, so a fresh noun preserves the M31–M41 single-subcommand-per-noun precedent); orthogonal-to-M40 framing (M40 = property-count axis on output_schema.properties; M42 = top-level-type axis on output_schema.type — same field, different axis); eval scenario `output-schema-top-level-type-index-determinism` (eval count 32 → ≥33). Constraint 35 (proposed; M42 is a planning aid, NOT a runtime response executor / response-payload validator / data-contract conformance detector / response generator / data-contract certifier) reserved for SYSTEM_SPEC § Constraints in the dev materialization turn. Deferred successor milestones: `M-Output-Type-Integer-Subset-Index-1`, `M-Output-Type-Multi-Type-Index-1`, `M-Output-Type-OneOf-AnyOf-AllOf-Index-1`, `M-Output-Type-Persistence-1`, `M-Output-Type-Doc-Contradiction-1`, `M-Output-Type-Runtime-Conformance-1`. The full SYSTEM_SPEC § M42 detail block and Constraint 35 will be materialized in the dev implementation turn before any source code lands; this Reservation block names the charter and freezes the scope decisions for dev to carry forward verbatim.
