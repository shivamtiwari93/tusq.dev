...
> **M70 Charter Sketch Reservation — 2026-04-28, run_7aa3510dbb544d16, turn_47f0529338159ca6, PM attempt 1.** Reserves the M70 boundary as **non-runtime-decoder-implementer / non-ingestion-pre-decoder / non-RFC-2045-validator / non-RFC-4648-validator / non-doc-contradiction-detector / non-canonical-vocabulary-validator / non-content-media-type-aggregator / non-format-aggregator / non-pattern-aggregator / non-joint-validity-validator / non-type-applicability-validator / non-LLM-content-encoding-inferrer / non-statistical-aggregator** per Constraint 63 (proposed). M70 axis: `input_schema.properties[firstKey].contentEncoding` (JSON-Schema Draft 7+ string-content-transfer-encoding annotation). Bucket-key enum (closed four-value): `encoded | unencoded | not_applicable | unknown`. Aggregation_key enum (closed three-value): `content_transfer_encoding_constraint | not_applicable | unknown`. Six frozen warning reason codes (5 baseline + axis-specific 6th `input_schema_properties_first_property_content_encoding_invalid_when_present`). Bucket iteration order: `encoded → unencoded → not_applicable → unknown` (deterministic stable-output only). M70-SPECIFIC rules: NULL-AS-ABSENT, EMPTY-STRING-AS-ABSENT (deliberate divergence from M69's FALSY-IS-VALID-CONST), TYPE-APPLICABILITY-STRING, DRAFT-7-STRING-IS-VALID-CONTENT-ENCODING, ANY-NON-EMPTY-STRING-IS-ENCODED (canonical RFC-2045/RFC-4648 seven-value validation deferred to `M-Wire-Canonical-Set-Validator-1`). CLI surface 53 → 54: new noun `wire` between `version` and `help`. Read-only invariants: 40 prior peer index commands byte-identical pre/post; non-persistence rule. The full M70 § "Input Schema First Property Content Encoding Annotation Presence Index" section will replace this reservation block at dev materialization. See ROADMAP.md § M70 for the full PM-frozen scope and 18 unchecked items.
