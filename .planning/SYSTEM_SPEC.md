...
> **M88 Materialized — 2026-04-29, run_73ffb608f7c8a510, turn_f2827707dfc5e04a, dev attempt 1.** `tusq witness index` implemented. CLI surface 71→72. `witness` after `wire` (last noun in dispatch; wire(w=119,i=105,r=114,e=101) < witness(w=119,i=105,t=116,n=110,e=101,s=115,s=115) at pos 2 (r(114)<t(116))). Flags: `--manifest`, `--json`, `--out`, `--witness` (case-sensitive, closed four-value enum `typed|untyped|not_applicable|unknown`). Output: `first_property_contains_states[]` with 8-field per-bucket shape. Six frozen warning reason codes: 5 baseline + axis-specific 6th `input_schema_properties_first_property_contains_invalid_when_present`. TYPE-APPLICABILITY-ARRAY-RESTRICTION (M88-INHERITED-FROM-M87: firstVal.type !== 'array' → not_applicable); NO-TUPLE-ITEMS-PREREQUISITE (M88-DISTINCT-FROM-M87: classifier MUST NOT gate not_applicable on Array.isArray(firstVal.items) — Draft-7 §6.4.6 has NO interaction with items keyword; contains classifies regardless of items shape); ABSENT-AS-UNTYPED (hasOwnProperty ownership detection); UNDEFINED-EXPLICIT-AS-UNKNOWN WITH 6th code; NULL-AS-UNKNOWN (M88-INHERITED-FROM-M87: contains:null → unknown WITH 6th code; Draft-7 schemas are boolean-or-object, null is neither; distinct from M86 NULL-AS-TYPED for default); BOOLEAN-SCHEMA-AS-TYPED (true and false both typed — Draft-7 §4.4 boolean schemas; mirrors M77 BOOLEAN-AS-TYPED for additionalProperties and M87 BOOLEAN-SCHEMA-AS-TYPED for additionalItems); EMPTY-OBJECT-SCHEMA-AS-UNTYPED (M88-INHERITED-FROM-M81/M87: {} → untyped; distinct from M84/M86 EMPTY-PLAIN-OBJECT-AS-TYPED-INCLUDING-EMPTY); NON-EMPTY-OBJECT-SUBSCHEMA-AS-TYPED (subschema-shape NOT recursively walked); ARRAY-AS-UNKNOWN (M88-INHERITED-FROM-M87: Array.isArray(contains) → unknown WITH 6th code; Draft-7 §6.4.6 MUST be boolean OR single object subschema, NOT array); NO-COERCION. Aggregation_key: `existential_element_match_subschema|not_applicable|unknown`. Bucket order: typed→untyped→not_applicable→unknown. npm test exits 0 (79 scenarios). See IMPLEMENTATION_NOTES.md § M88 for full detail.

<!-- M88 Charter Sketch Reservation: 2026-04-29, run_73ffb608f7c8a510, turn_6da4977ce67c78e1, PM attempt 1, HEAD prior to PM edits at `0df6321`. Reserves a future § "Input Schema First Property Contains Annotation Presence Index" section for the planning-phase M88 charter. Axis: `input_schema.properties[firstKey].contains` (JSON-Schema Draft 7 §6.4.6 EXISTENTIAL-ELEMENT-MATCH-SUBSCHEMA annotation governing the at-least-one-element-must-match-the-subschema declaration on the first input parameter; TWELFTH first-property axis annotation in the M77+ first-property cluster after M77 `additionalProperties`, M78 `minProperties`, M79 `maxProperties`, M80 `patternProperties`, M81 `propertyNames`, M82 `required`, M83 `dependencies`, M84 `const`, M85 `enum`, M86 `default`, M87 `additionalItems`; the SECOND in the cluster whose Draft-7 keyword is restricted to `firstVal.type === 'array'` schemas — Draft-7 §6.4.6 defines `contains` as meaningful ONLY on array-typed schemas; INHERITS the **TYPE-APPLICABILITY-ARRAY-RESTRICTION** invariant from M87; the FIRST in the array-restricted sub-cluster to introduce the **NO-TUPLE-ITEMS-PREREQUISITE** invariant — Draft-7 §6.4.6 has NO interaction with the `items` keyword, so the classifier MUST NOT gate `not_applicable` on `Array.isArray(firstVal.items)`; the AXIS-DISTINCT sibling of M87 `additionalItems` already shipped as `extra index` — both axes operate on `firstVal.type === 'array'` but M87 reads the ∀-tail-validation schema (conditional on tuple-items) whereas M88 reads the ∃-existential-element-match schema (regardless of items shape); also the AXIS-DISTINCT sibling of M71 `items` — M88 captures the existential ∃ counterpart of M71's universal ∀). Bucket-key enum (closed four-value): `typed | untyped | not_applicable | unknown`. Aggregation_key enum (closed three-value): `existential_element_match_subschema | not_applicable | unknown`. Six frozen warning reason codes (5 baseline + axis-specific 6th `input_schema_properties_first_property_contains_invalid_when_present`). Bucket iteration order: `typed → untyped → not_applicable → unknown` (deterministic stable-output only — NOT a Support-Copilot-priority / Sales-Copilot-priority / CSM-Copilot-priority / Implementation-Copilot-priority / Admin-Copilot-priority / Security-Review-Copilot-priority / copilot-role-dispatch-rank / copilot-input-validation-rank / copilot-governance-evidence-rank / SDK-existential-precondition-emit-priority ranking). Rules: TYPE-APPLICABILITY-ARRAY-RESTRICTION (M88-INHERITED-FROM-M87, distinct from M77–M83 TYPE-APPLICABILITY-OBJECT and from M84/M85/M86 NO-TYPE-APPLICABILITY-OBJECT-RESTRICTION: `firstVal.type !== 'array'` → `not_applicable`); NO-TUPLE-ITEMS-PREREQUISITE (M88-DISTINCT-FROM-M87: classifier MUST NOT gate `not_applicable` on `firstVal.items` shape — Draft-7 §6.4.6 has NO interaction with the `items` keyword); ABSENT-AS-UNTYPED (own `'contains'` not present → `untyped`); UNDEFINED-EXPLICIT-AS-UNKNOWN (own-property present but value is JS `undefined` → unknown WITH 6th code); NULL-AS-UNKNOWN (M88-INHERITED-FROM-M87 NULL-AS-UNKNOWN, distinct from M86 NULL-AS-TYPED for `default`: `contains === null` → unknown WITH 6th code — Draft-7 schemas are boolean OR object, null is neither); BOOLEAN-SCHEMA-AS-TYPED (`typeof === 'boolean'` → typed for both `true` and `false` — Draft-7 §4.4 boolean schemas: `true` is satisfied by any non-empty array, `false` is the vacuous-existential; mirrors M77 BOOLEAN-AS-TYPED for `additionalProperties` and M87 BOOLEAN-SCHEMA-AS-TYPED for `additionalItems`); EMPTY-OBJECT-SCHEMA-AS-UNTYPED (M88-INHERITED-FROM-M81/M87: empty `{}` schema → `untyped` — `contains: {}` is satisfied by any non-empty array; distinct from M84/M86 EMPTY-PLAIN-OBJECT-AS-TYPED-INCLUDING-EMPTY for value-axes); NON-EMPTY-OBJECT-SUBSCHEMA-AS-TYPED (`Object.prototype.toString.call(v) === '[object Object]' && Object.keys(v).length >= 1` → typed; subschema-shape NOT recursively walked); ARRAY-AS-UNKNOWN (M88-INHERITED-FROM-M87: `Array.isArray(contains)` → unknown WITH 6th code — Draft-7 §6.4.6 MUST be boolean OR single object subschema, NOT array; distinct from `items` which CAN be a tuple-array); STRING/NUMBER/FUNCTION/SYMBOL/BIGINT/DATE/REGEXP etc → unknown WITH 6th code; NO-COERCION via `Array.from()`/`Object()`/`JSON.parse(JSON.stringify())`/`String(v)`/`Number(v)`/`Boolean(v)`/`!!v`/`+v` — strict per-type checks AND `Object.prototype.hasOwnProperty.call(firstVal, 'contains')` ownership detection. CLI surface 71 → 72 with new noun `witness` after `wire` (last in dispatch; wire(w=119,i=105,r=114,e=101) < witness(w=119,i=105,t=116,n=110,e=101,s=115,s=115) at pos 2 (r(114)<t(116))). Result-array field name: `first_property_contains_states[]`. Per-bucket field name: `input_schema_first_property_contains`. Constraint 81 (proposed) reserves M88 as "non-runtime-existential-validator / non-runtime-contains-emitter / non-SDK-existential-precondition-emitter / non-TypeScript-existential-quantifier-renderer / non-API-tool-schema-emitter / non-copilot-role-dispatcher / non-copilot-input-validator / non-copilot-governance-evidence-emitter / non-Support-Copilot-emitter / non-Sales-Copilot-emitter / non-CSM-Copilot-emitter / non-Implementation-Copilot-emitter / non-Admin-Copilot-emitter / non-Security-Review-Copilot-emitter / non-strict-tool-emitter-existential-emitter / non-MCP-server-existential-validator / non-marketplace-package-existential-validator / non-validation-layer-existential-validator / non-items-crossref / non-additional-items-crossref / non-min-items-crossref / non-max-items-crossref / non-unique-items-crossref / non-doc-contradiction-detector / non-LLM-existential-inferrer / non-statistical-aggregator / non-existential-coherence-validator / non-runtime-existential-mutator / non-format-crossref / non-pattern-string-crossref / non-numeric-bound-crossref / non-composition-axis-crossref / non-conditional-axis-crossref". Distinct from M87 (`additionalItems` — sibling array-restricted axis: M87 reads ∀-tail-validation schema, M88 reads ∃-existential schema; orthogonal in that a capability may declare BOTH simultaneously), M86 (`default` — UNCONDITIONAL sample-instance-value POINT, unrestricted by firstVal.type), M85 (`enum` — UNCONDITIONAL bounded value-SET), M84 (`const` — UNCONDITIONAL single-value PIN), M83 (`dependencies` — CONDITIONAL trigger-keyed MAP), M82 (`required` — key-presence list), M81 (`propertyNames` — single SCHEMA on EVERY name), M80 (`patternProperties` — regex-keyed-MAP), M77 (`additionalProperties` — object-extension-control, orthogonal OBJECT vs M88's ARRAY), M71 (`items` — per-element ∀ subschema for ALL positions, the universal-quantifier counterpart of M88's existential-quantifier — orthogonal ∀ vs ∃), M50 (manifest-firstKey required — different slot). VISION primary citation: lines 285–296 (`### Employee Copilots`) — first milestone to use this section as primary aggregation source; the section enumerates the employee-facing copilots ("Support Copilot", "Sales Engineer Copilot", "CSM / Account Copilot", "Implementation / Onboarding Copilot", "Admin / Operations Copilot", "Security Review Copilot") that combine tools, skills, workflows, knowledge artifacts, IAM context, customer/account context, and governance policies — directly mappable to copilot-role dispatch routing, copilot input-validation pre-warning, and copilot governance evidence collection. The full M88 § will be materialized by the dev role's implementation phase per the M27–M87 PM-vs-dev ownership split. M88 is implementation-ready planned work, not shipped. -->

<!-- M87 Charter Sketch Reservation: 2026-04-29, run_1073dd9a1aacf17f, turn_995950460bb780f8, PM attempt 1, HEAD prior to PM edits at `352d64e`. Reserves a future § "Input Schema First Property AdditionalItems Annotation Presence Index" section for the planning-phase M87 charter. Axis: `input_schema.properties[firstKey].additionalItems` (JSON-Schema Draft 7 §6.4.2 ARRAY-TAIL-ITEMS-SCHEMA annotation governing the tail-validation-schema declaration for items beyond the positional tuple; ELEVENTH first-property axis annotation in the M77+ first-property cluster after M77 `additionalProperties`, M78 `minProperties`, M79 `maxProperties`, M80 `patternProperties`, M81 `propertyNames`, M82 `required`, M83 `dependencies`, M84 `const`, M85 `enum`, M86 `default`; the FIRST in the cluster whose Draft-7 keyword is restricted to `firstVal.type === 'array'` schemas — Draft-7 §6.4.2 defines `additionalItems` as meaningful ONLY on array-typed schemas; the FIRST to introduce the **TYPE-APPLICABILITY-ARRAY-RESTRICTION** invariant (distinct from M77–M83 TYPE-APPLICABILITY-OBJECT and from M84/M85/M86 NO-TYPE-APPLICABILITY-OBJECT-RESTRICTION); the FIRST to introduce the **TUPLE-ITEMS-PREREQUISITE** invariant — Draft-7 §6.4.2 explicitly states `additionalItems` is ignored when `items` is a single schema or absent, so the classifier MUST gate `not_applicable` on `Array.isArray(firstVal.items)`; the AXIS-DISTINCT sibling of M71 `items` already shipped as `element index` — both axes operate on `firstVal.type === 'array'` but M71 reads the per-element subschema whereas M87 reads the tail-validation schema for items beyond the tuple positions). Bucket-key enum (closed four-value): `typed | untyped | not_applicable | unknown`. Aggregation_key enum (closed three-value): `array_tail_items_schema | not_applicable | unknown`. Six frozen warning reason codes (5 baseline + axis-specific 6th `input_schema_properties_first_property_additional_items_invalid_when_present`). Bucket iteration order: `typed → untyped → not_applicable → unknown` (deterministic stable-output only — NOT a developer-artifacts-priority / SDK-type-definition-emission-rank / API-tool-schema-readiness-rank / generated-integration-test-priority / eval-fixture-coverage-priority / drift-report-tail-typing-rank / migration-plan-priority / changelog-tail-typing-rank / marketplace-validation-report-priority / local-development-fixture-priority / strict-tool-emitter-tail-rank / variadic-tuple-emit-priority ranking). Rules: TYPE-APPLICABILITY-ARRAY-RESTRICTION (M87-SPECIFIC, distinct from M77–M83 TYPE-APPLICABILITY-OBJECT and from M84/M85/M86 NO-TYPE-APPLICABILITY-OBJECT-RESTRICTION: `firstVal.type !== 'array'` → `not_applicable`); TUPLE-ITEMS-PREREQUISITE (M87-SPECIFIC: `firstVal.items` absent OR `!Array.isArray(firstVal.items)` → `not_applicable` — Draft-7 §6.4.2 ignores `additionalItems` when `items` is a single schema); ABSENT-AS-UNTYPED (own `'additionalItems'` property not present → `untyped`); UNDEFINED-EXPLICIT-AS-UNKNOWN (own-property present but value is JS `undefined` → unknown WITH 6th code); NULL-AS-UNKNOWN (M87-SPECIFIC distinct from M86 NULL-AS-TYPED for `default`: `additionalItems === null` → unknown WITH 6th code — Draft-7 schemas are boolean OR object, null is neither); BOOLEAN-SCHEMA-AS-TYPED (`typeof === 'boolean'` → typed for both `true` and `false` — Draft-7 §4.4 boolean schemas; mirrors M77 BOOLEAN-AS-TYPED for `additionalProperties`); EMPTY-OBJECT-SCHEMA-AS-UNTYPED (M87-INHERITED-FROM-M81: empty `{}` schema → `untyped` — declares no constraint, equivalent to absent default; distinct from M84/M86 EMPTY-PLAIN-OBJECT-AS-TYPED-INCLUDING-EMPTY which apply to value-axes); NON-EMPTY-OBJECT-SUBSCHEMA-AS-TYPED (`Object.prototype.toString.call(v) === '[object Object]' && Object.keys(v).length >= 1` → typed; subschema-shape NOT recursively walked); ARRAY-AS-UNKNOWN (M87-SPECIFIC: `Array.isArray(additionalItems)` → unknown WITH 6th code — Draft-7 §6.4.2 MUST be boolean OR single object subschema, NOT array; distinct from `items` which CAN be a tuple-array); STRING/NUMBER/FUNCTION/SYMBOL/BIGINT/DATE/REGEXP etc → unknown WITH 6th code; NO-COERCION via `Array.from()`/`Object()`/`JSON.parse(JSON.stringify())`/`String(v)`/`Number(v)`/`Boolean(v)`/`!!v`/`+v` — strict per-type checks AND `Object.prototype.hasOwnProperty.call(firstVal, 'additionalItems')` ownership detection. CLI surface 70 → 71 with new noun `extra` between `examples` and `fixed` (examples(e=101,x=120,a=97) < extra(e=101,x=120,t=116) at pos 2 (a(97)<t(116)); extra(e=101) < fixed(f=102) at pos 0 (e(101)<f(102))). Result-array field name: `first_property_additional_items_states[]`. Per-bucket field name: `input_schema_first_property_additional_items`. Constraint 80 (proposed) reserves M87 as "non-runtime-array-tail-validator / non-runtime-tuple-tail-emitter / non-SDK-variadic-tuple-emitter / non-TypeScript-variadic-tuple-renderer / non-API-tool-schema-emitter / non-integration-test-fixture-emitter / non-eval-fixture-tail-position-emitter / non-drift-report-tail-typing-diff-emitter / non-marketplace-validation-report-emitter / non-changelog-tail-typing-emitter / non-migration-plan-tail-typing-emitter / non-local-development-fixture-emitter / non-strict-tool-emitter-tail-emitter / non-MCP-server-tail-validator / non-marketplace-package-tail-validator / non-validation-layer-tail-validator / non-items-crossref / non-min-items-crossref / non-max-items-crossref / non-unique-items-crossref / non-contains-crossref / non-doc-contradiction-detector / non-LLM-tail-inferrer / non-statistical-aggregator / non-tuple-coherence-validator / non-runtime-tail-mutator / non-format-crossref / non-pattern-string-crossref / non-numeric-bound-crossref / non-composition-axis-crossref / non-conditional-axis-crossref". Distinct from M86 (`default` — UNCONDITIONAL sample-instance-value POINT, distinct semantic and unrestricted by firstVal.type), M85 (`enum` — UNCONDITIONAL bounded value-SET), M84 (`const` — UNCONDITIONAL single-value PIN), M83 (`dependencies` — CONDITIONAL trigger-keyed MAP), M82 (`required` — key-presence list), M81 (`propertyNames` — single SCHEMA on EVERY name), M80 (`patternProperties` — regex-keyed-MAP), M77 (`additionalProperties` — object-extension-control, orthogonal OBJECT-EXTENSION vs M87's ARRAY-TAIL), M71 (`items` — per-element subschema for leading positions, orthogonal LEADING-POSITIONS vs M87's TAIL), M50 (manifest-firstKey required — different slot). VISION primary citation: lines 342–356 (`### Developer Artifacts`) — first milestone to use this section as primary aggregation source; the section enumerates the developer-facing artifacts ("SDK type definitions", "API/tool schemas", "generated integration tests", "eval fixtures", "drift reports", "migration plans", "changelog entries", "marketplace validation reports", "local development fixtures") that depend on tail-typing evidence for variadic-tuple SDK type emission, integration-test boundary-fixture coverage, and drift-report tail-typing diffs. The full M87 § will be materialized by the dev role's implementation phase per the M27–M86 PM-vs-dev ownership split. M87 is implementation-ready planned work, not shipped. -->

> **M87 Materialized — 2026-04-29, run_1073dd9a1aacf17f, turn_8491a1464c5bda92, dev attempt 2.** `tusq extra index` implemented. CLI surface 70→71. `extra` between `examples` and `fixed` (examples(e=101,x=120,a=97) < extra(e=101,x=120,t=116) at pos 2 (a(97)<t(116)); extra(e=101) < fixed(f=102) at pos 0 (e(101)<f(102))). Flags: `--manifest`, `--json`, `--out`, `--extra` (case-sensitive, closed four-value enum `typed|untyped|not_applicable|unknown`). Output: `first_property_additional_items_states[]` with 8-field per-bucket shape. Six frozen warning reason codes: 5 baseline + axis-specific 6th `input_schema_properties_first_property_additional_items_invalid_when_present`. TYPE-APPLICABILITY-ARRAY-RESTRICTION (M87-SPECIFIC: firstVal.type !== 'array' → not_applicable; distinct from M77–M83 TYPE-APPLICABILITY-OBJECT and M84/M85/M86 NO-TYPE-APPLICABILITY-OBJECT-RESTRICTION); TUPLE-ITEMS-PREREQUISITE (M87-SPECIFIC: firstVal.items absent OR !Array.isArray(firstVal.items) → not_applicable; Draft-7 §6.4.2 ignores additionalItems when items is a single schema); ABSENT-AS-UNTYPED (hasOwnProperty ownership detection); UNDEFINED-EXPLICIT-AS-UNKNOWN WITH 6th code; NULL-AS-UNKNOWN (M87-SPECIFIC distinct from M86 NULL-AS-TYPED: additionalItems:null → unknown WITH 6th code; Draft-7 schemas are boolean-or-object, null is neither); BOOLEAN-SCHEMA-AS-TYPED (true and false both typed — Draft-7 §4.4 boolean schemas; mirrors M77 BOOLEAN-AS-TYPED for additionalProperties); EMPTY-OBJECT-SCHEMA-AS-UNTYPED (M87-INHERITED-FROM-M81: {} → untyped; distinct from M84/M86 EMPTY-PLAIN-OBJECT-AS-TYPED-INCLUDING-EMPTY); NON-EMPTY-OBJECT-SUBSCHEMA-AS-TYPED (subschema-shape NOT recursively walked); ARRAY-AS-UNKNOWN (M87-SPECIFIC: Array.isArray(additionalItems) → unknown WITH 6th code; Draft-7 §6.4.2 MUST be boolean OR single object subschema, NOT array); NO-COERCION. Aggregation_key: `array_tail_items_schema|not_applicable|unknown`. Bucket order: typed→untyped→not_applicable→unknown. npm test exits 0 (78 scenarios). See IMPLEMENTATION_NOTES.md § M87 for full detail.

<!-- M86 Charter Sketch Reservation: 2026-04-29, run_083e290f5ee318f4, turn_2e58a486f97550ec, PM attempt 1, HEAD prior to PM edits at `37824cc`. Reserves a future § "Input Schema First Property Default Annotation Presence Index" section for the planning-phase M86 charter. Axis: `input_schema.properties[firstKey].default` (JSON-Schema Draft 7 §10.2 SAMPLE-INSTANCE-VALUE annotation governing the operator-declared pre-fill value on the first input parameter; TENTH first-property axis annotation in the M77+ first-property cluster after M77 `additionalProperties`, M78 `minProperties`, M79 `maxProperties`, M80 `patternProperties`, M81 `propertyNames`, M82 `required`, M83 `dependencies`, M84 `const`, M85 `enum`; the THIRD in the cluster whose Draft-7 keyword is NOT restricted to `type === 'object'` schemas — Draft-7 §10.2 defines `default` as applicable to ANY type; the SECOND in the post-M84 sub-cluster of NULL-AS-TYPED inheritance — `firstVal.default === null` → `typed`, mirroring M84 NULL-AS-TYPED for `const` and DISTINCT from M85 NULL-AS-ABSENT for `enum`; the SECOND in the post-M84 sub-cluster of JSON-ARRAY-AS-TYPED-INCLUDING-EMPTY and JSON-PLAIN-OBJECT-AS-TYPED-INCLUDING-EMPTY inheritance — `default: []` → typed and `default: {}` → typed, distinct from M82 EMPTY-ARRAY-AS-UNTYPED for `required`, M85 EMPTY-ARRAY-AS-UNKNOWN for `enum`, and M81 EMPTY-OBJECT-SCHEMA-AS-UNTYPED for `propertyNames`; the AXIS-DISTINCT sibling of M84 `const` — BOTH axes accept ANY JSON value with identical structural classification rules, BUT the SEMANTIC purpose differs orthogonally: `const` is a CONSTRAINT narrowing accepted runtime values to exactly one, whereas `default` is a SAMPLE suggesting a pre-fill without constraining). Bucket-key enum (closed four-value): `typed | untyped | not_applicable | unknown`. Aggregation_key enum (closed three-value): `sample_instance_value | not_applicable | unknown`. Six frozen warning reason codes (5 baseline + axis-specific 6th `input_schema_properties_first_property_default_invalid_when_present`). Bucket iteration order: `typed → untyped → not_applicable → unknown` (deterministic stable-output only — NOT a tool-dry-run-priority / tool-emission-readiness-rank / MCP-tool-listing-priority / brand-matched-chat-prefill-rank / brand-matched-voice-prefill-rank / embeddable-widget-initial-state-priority / action-widget-default-emit-rank / marketplace-example-prompts-priority / strict-tool-emitter-default-emit-rank / dry-run-confirmation-path-priority / IAM-context-prefill-rank / audit-metadata-emit-priority ranking). Rules: NO-TYPE-APPLICABILITY-OBJECT-RESTRICTION (M86-inherited from M84/M85: classifier MUST NOT inspect `firstVal.type` to gate `not_applicable` — Draft-7 §10.2 defines `default` as applicable to ANY type; outer-schema `type === 'object'` prerequisite RETAINED because without an object outer there is no `firstVal`); ABSENT-AS-UNTYPED (own `'default'` property not present → `untyped`, no warning); NULL-AS-TYPED (M86-inherited from M84, distinct from M85 NULL-AS-ABSENT and from M55–M83 NULL-AS-ABSENT: `firstVal.default === null` → `typed` — Draft-7 §10.2 explicitly permits `default: null` as a meaningful sample-instance-value); UNDEFINED-EXPLICIT-AS-UNKNOWN (own-property present but value is JS `undefined` → unknown WITH 6th code; JavaScript `undefined` is NOT a JSON value); JSON-STRING-AS-TYPED; JSON-FINITE-NUMBER-AS-TYPED (`typeof === 'number' && Number.isFinite` → typed; non-finite NaN/Infinity/-Infinity → unknown WITH 6th code); JSON-BOOLEAN-AS-TYPED (typed for both `true` and `false`); JSON-ARRAY-AS-TYPED (`Array.isArray` → typed for empty `[]` and non-empty arrays — element-shape NOT recursively walked; M86-inherited from M84, distinct from M82 EMPTY-ARRAY-AS-UNTYPED and M85 EMPTY-ARRAY-AS-UNKNOWN); JSON-PLAIN-OBJECT-AS-TYPED (`Object.prototype.toString.call(v) === '[object Object]'` → typed for empty `{}` and non-empty plain-object — key/value-shape NOT recursively walked; M86-inherited from M84, distinct from M81 EMPTY-OBJECT-SCHEMA-AS-UNTYPED); DRAFT-7-ANY-JSON-VALUE-IS-VALID-DEFAULT (any other value → unknown WITH 6th code); NO-COERCION via `Array.from()`/`Object()`/`JSON.parse(JSON.stringify())`/`String(v)`/`Number(v)`/`Boolean(v)`/`!!v`/`+v` — strict per-type checks AND treat literal JS `null` as `typed` via explicit `firstVal.default === null` reference equality; the classifier MUST NOT use `firstVal.default ? ... : ...` truthy/falsy short-circuit (which would mis-classify `0`, `''`, `false`, `null`, `[]`, `{}` as falsy-or-truthy in JS-but-meaningful-as-JSON-defaults). CLI surface 69 → 70 with new noun `prefill` between `policy` and `preset` (policy(p=112,o=111) < prefill(p=112,r=114) at pos 1 (o(111)<r(114)); prefill(p=112,r=114,e=101,f=102) < preset(p=112,r=114,e=101,s=115) at pos 3 (f(102)<s(115))). Result-array field name: `first_property_default_states[]`. Per-bucket field name: `input_schema_first_property_default`. Constraint 79 (proposed) reserves M86 as "non-runtime-default-injector / non-runtime-pre-fill-emitter / non-runtime-form-prefiller / non-dry-run-input-synthesizer / non-tool-dry-run-emitter / non-MCP-tool-listing-default-emitter / non-marketplace-example-prompts-emitter / non-brand-matched-chat-prefill-emitter / non-brand-matched-voice-prefill-emitter / non-embeddable-widget-initial-state-emitter / non-action-widget-initial-state-emitter / non-strict-tool-emitter-default-emitter / non-MCP-server-default-validator / non-marketplace-package-default-validator / non-validation-layer-default-validator / non-const-crossref / non-enum-crossref / non-required-crossref / non-property-names-crossref / non-pattern-properties-crossref / non-additional-properties-crossref / non-min-properties-crossref / non-max-properties-crossref / non-dependencies-crossref / non-doc-contradiction-detector / non-LLM-default-inferrer / non-statistical-aggregator / non-default-vs-const-coherence-validator / non-default-runtime-mutator / non-format-crossref / non-pattern-string-crossref / non-numeric-bound-crossref". Distinct from M85 (`enum` — sibling first-property axis on the same firstVal but different shape: M85 is an UNCONDITIONAL bounded value-SET — orthogonal SET vs M86's POINT; M85 EMPTY-ARRAY-AS-UNKNOWN vs M86 JSON-ARRAY-AS-TYPED-INCLUDING-EMPTY; M85 NULL-AS-ABSENT vs M86 NULL-AS-TYPED), M84 (`const` — sibling first-property axis on the same firstVal: BOTH accept ANY JSON value with identical structural rules but DIFFERENT semantic purpose — `const` is CONSTRAINT, `default` is SAMPLE; orthogonal in that a capability may declare BOTH `const: 'create'` AND `default: 'create'` simultaneously), M83 (`dependencies` — CONDITIONAL trigger-keyed MAP), M82 (`required` — key-presence list), M81 (`propertyNames` — single SCHEMA on EVERY name), M80 (`patternProperties` — regex-keyed-MAP), M50 (manifest-firstKey required — different slot). VISION primary citation: lines 164–174 (`### Tools`) — first milestone to use this section as primary aggregation source; the section enumerates the lowest-level emitted artifact (the Tool — a single AI-callable verb derived from an approved capability), explicitly names "strict schemas" and "dry-run and confirmation support" as Tool attributes; a capability whose first input property declares `default: <value>` is the strongest pre-runtime signal that the capability has operator-declared dry-run-input synthesis evidence — directly mappable to Tool dry-run-payload synthesis, brand-matched-chat/voice form-prefilling, embeddable-widget initial-state rendering, MCP-server tool-listing metadata, marketplace example-prompts emission, and V1 Tool-emission prioritization. The full M86 § will be materialized by the dev role's implementation phase per the M27–M85 PM-vs-dev ownership split. M86 is implementation-ready planned work, not shipped. -->

> **M86 Materialized — 2026-04-29, run_083e290f5ee318f4, turn_fc4027d5c8789062, dev attempt 1.** `tusq prefill index` implemented. CLI surface 69→70. `prefill` between `policy` and `preset` (policy(p=112,o=111)<prefill(p=112,r=114) at pos 1 (o(111)<r(114)); prefill(p=112,r=114,e=101,f=102)<preset(p=112,r=114,e=101,s=115) at pos 3 (f(102)<s(115))). Flags: `--manifest`, `--json`, `--out`, `--prefill` (case-sensitive, closed four-value enum `typed|untyped|not_applicable|unknown`). Output: `first_property_default_states[]` with 8-field per-bucket shape. Six frozen warning reason codes: 5 baseline + axis-specific 6th `input_schema_properties_first_property_default_invalid_when_present`. NO-TYPE-APPLICABILITY-OBJECT-RESTRICTION (M86-inherited from M84/M85: default applies to ANY firstVal type per Draft-7 §10.2); NULL-AS-TYPED (M86-INHERITED-FROM-M84: default:null → typed — Draft-7 §10.2 permits null as valid sample-instance-value; distinct from M85 NULL-AS-ABSENT for enum and from M55–M83 NULL-AS-ABSENT); ABSENT-AS-UNTYPED (hasOwnProperty check protects 0/false/null/''/[]/{} from falsy mis-classification); UNDEFINED-EXPLICIT-AS-UNKNOWN WITH 6th code; JSON-STRING-AS-TYPED; JSON-FINITE-NUMBER-AS-TYPED (NaN/Infinity/-Infinity → unknown WITH 6th code); JSON-BOOLEAN-AS-TYPED (both true and false — NO falsy-coercion); JSON-ARRAY-AS-TYPED-INCLUDING-EMPTY (default:[] → typed — M86-DISTINCT-FROM-M82 EMPTY-ARRAY-AS-UNTYPED and M85 EMPTY-ARRAY-AS-UNKNOWN); JSON-PLAIN-OBJECT-AS-TYPED-INCLUDING-EMPTY (default:{} → typed — M86-DISTINCT-FROM-M81 EMPTY-OBJECT-SCHEMA-AS-UNTYPED); DRAFT-7-ANY-JSON-VALUE-IS-VALID-DEFAULT+NO-COERCION. Aggregation_key: `sample_instance_value|not_applicable|unknown`. Bucket order: typed→untyped→not_applicable→unknown. npm test exits 0 (77 scenarios). See IMPLEMENTATION_NOTES.md § M86 for full detail.

> **M85 Materialized — 2026-04-29, run_c78dec96e45c4d0c, turn_7d666108b5f79c9b, dev attempt 2.** `tusq allowed index` implemented. CLI surface 68→69. `allowed` between `above` and `below` (above(a=97,b=98)<allowed(a=97,l=108) at pos 1 (b(98)<l(108)); allowed(a=97)<below(b=98) at pos 0 (a(97)<b(98))). Flags: `--manifest`, `--json`, `--out`, `--allowed` (case-sensitive, closed four-value enum `typed|untyped|not_applicable|unknown`). Output: `first_property_enum_states[]` with 8-field per-bucket shape. Six frozen warning reason codes: 5 baseline + axis-specific 6th `input_schema_properties_first_property_enum_invalid_when_present`. NO-TYPE-APPLICABILITY-OBJECT-RESTRICTION (M85-inherited from M84: enum applies to ANY firstVal type per Draft-7 §6.1.2, distinct from M77–M83 TYPE-APPLICABILITY-OBJECT); NULL-AS-ABSENT (M85-SPECIFIC distinct from M84 NULL-AS-TYPED: firstVal.enum===null → untyped — null is a single value not a set; cannot be a meaningful enum declaration); ABSENT-AS-UNTYPED; UNDEFINED-EXPLICIT-AS-UNKNOWN (WITH 6th code); NON-ARRAY-AS-UNKNOWN (Draft-7 §6.1.2 MUST be array, WITH 6th code); EMPTY-ARRAY-AS-UNKNOWN (M85-SPECIFIC distinct from M82 EMPTY-ARRAY-AS-UNTYPED for required and from M84 JSON-ARRAY-AS-TYPED for const: Draft-7 §6.1.2 SHOULD have at least one element — empty enum is pathological, flagged WITH 6th code); NON-EMPTY-ARRAY-WITH-ALL-VALID-JSON-ELEMENTS-AS-TYPED (string|finite-number|boolean|null|Array.isArray|[object Object] elements; nested element shape NOT recursively walked); ARRAY-WITH-INVALID-ELEMENT-AS-UNKNOWN (undefined/NaN/Infinity/function/Symbol/BigInt/Date/RegExp → unknown WITH 6th code); DRAFT-7-NON-EMPTY-ARRAY-OF-VALID-JSON-VALUES-IS-VALID-ENUM (NO duplicate-element detection — Draft-7 SHOULD be unique is loose); NO-COERCION via Array.from/Object/JSON-roundtrip/String/Number/Boolean/!!. Bucket order: typed→untyped→not_applicable→unknown. npm test exits 0 (76 scenarios). See IMPLEMENTATION_NOTES.md § M85 for full detail.

<!-- M85 Charter Sketch Reservation: 2026-04-29, run_c78dec96e45c4d0c, turn_fa7c922678a9a76c, PM attempt 1, HEAD prior to PM edits at `e5f5452`. Reserves a future § "Input Schema First Property Enum Annotation Presence Index" section for the planning-phase M85 charter. Axis: `input_schema.properties[firstKey].enum` (JSON-Schema Draft 7 §6.1.2 SET-OF-ALLOWED-VALUES annotation governing the discrete bounded value-set declaration on the first input parameter; NINTH first-property axis annotation in the M77+ first-property cluster after M77 `additionalProperties`, M78 `minProperties`, M79 `maxProperties`, M80 `patternProperties`, M81 `propertyNames`, M82 `required`, M83 `dependencies`, M84 `const`; the SECOND in the cluster whose Draft-7 keyword is NOT restricted to `type === 'object'` schemas — Draft-7 §6.1.2 defines `enum` as applicable to ANY type; the FIRST in the post-M84 sub-cluster of NULL-AS-ABSENT inheritance — `firstVal.enum === null` → `untyped`, distinct from M84's NULL-AS-TYPED for `const`; the FIRST in the cluster of EMPTY-ARRAY-AS-UNKNOWN — Draft-7 §6.1.2 SHOULD have at least one element, an empty enum is a SHOULD-violation flagged with the axis-specific 6th frozen reason code, distinct from M82's EMPTY-ARRAY-AS-UNTYPED for `required` and from M84's JSON-ARRAY-AS-TYPED for `const`). Bucket-key enum (closed four-value): `typed | untyped | not_applicable | unknown`. Aggregation_key enum (closed three-value): `value_set_constraint | not_applicable | unknown`. Six frozen warning reason codes (5 baseline + axis-specific 6th `input_schema_properties_first_property_enum_invalid_when_present`). Bucket iteration order: `typed → untyped → not_applicable → unknown` (deterministic stable-output only — NOT an embeddable-widget-priority / action-widget-emit-priority / command-palette-emit-priority / select-list-render-priority / V1-framework-agnostic-embed-priority / Ctrl+K-surface-emit-priority / script-tag-embed-emit-priority / web-component-embed-emit-priority / React-package-emit-priority / Vue-package-emit-priority / Svelte-package-emit-priority / insight-widget-emit-priority ranking). Rules: NO-TYPE-APPLICABILITY-OBJECT-RESTRICTION (M85-inherited from M84: classifier MUST NOT inspect `firstVal.type` to gate `not_applicable` — Draft-7 §6.1.2 defines `enum` as applicable to ANY type; outer-schema `type === 'object'` prerequisite RETAINED because without an object outer there is no `firstVal`); ABSENT-AS-UNTYPED (own `'enum'` property not present → `untyped`, no warning); NULL-AS-ABSENT (M85-SPECIFIC distinct from M84 NULL-AS-TYPED: `firstVal.enum === null` → `untyped`); UNDEFINED-EXPLICIT-AS-UNKNOWN (own-property present but value is JS `undefined` → `unknown` WITH 6th code); NON-ARRAY-AS-UNKNOWN (any non-array value → `unknown` WITH 6th code; Draft-7 §6.1.2 MUST be array); EMPTY-ARRAY-AS-UNKNOWN (M85-SPECIFIC: `Array.isArray && length === 0` → `unknown` WITH 6th code; Draft-7 §6.1.2 SHOULD have at least one element); NON-EMPTY-ARRAY-WITH-ALL-VALID-JSON-ELEMENTS-AS-TYPED (every element is string OR finite number OR boolean OR `=== null` OR Array.isArray OR `[object Object]`; nested element shape NOT recursively walked); ARRAY-WITH-INVALID-ELEMENT-AS-UNKNOWN (any element is `undefined`/`NaN`/`Infinity`/`-Infinity`/function/Symbol/BigInt/Date/RegExp/etc → `unknown` WITH 6th code); DRAFT-7-NON-EMPTY-ARRAY-OF-VALID-JSON-VALUES-IS-VALID-ENUM (NO duplicate-element detection — Draft-7 SHOULD be unique is loose; uniqueness reserved for a future M-Allowed-Uniqueness-Checker successor); NO-COERCION via `Array.from()`/`Object()`/`JSON.parse(JSON.stringify())`/`String(v)`/`Number(v)`/`Boolean(v)`/`!!v`/`+v` — strict per-type checks AND treat literal JS `null` as a valid array-element via explicit `=== null`. CLI surface 68 → 69 with new noun `allowed` between `above` and `below` (above(a=97,b=98) < allowed(a=97,l=108) at pos 1 (b(98)<l(108)); allowed(a=97) < below(b=98) at pos 0 (a(97)<b(98))). Result-array field name: `first_property_enum_states[]`. Per-bucket field name: `input_schema_first_property_enum`. Constraint 78 (proposed) reserves M85 as "non-runtime-value-set-validator / non-runtime-value-membership-rejector / non-action-widget-emitter / non-command-palette-emitter / non-script-tag-embed-emitter / non-web-component-embed-emitter / non-React-package-emitter / non-Vue-package-emitter / non-Svelte-package-emitter / non-action-widget-palette-emitter / non-select-list-renderer / non-insight-widget-emitter / non-Ctrl+K-surface-emitter / non-framework-agnostic-embed-emitter / non-status-field-extractor / non-discrete-state-lexicon-emitter / non-action-verb-extractor / non-enum-crossref-with-status-fields-extractor / non-enum-crossref-with-state-machine-emitter / non-const-crossref / non-default-crossref / non-required-crossref / non-property-names-crossref / non-pattern-properties-crossref / non-additional-properties-crossref / non-min-properties-crossref / non-max-properties-crossref / non-dependencies-crossref / non-doc-contradiction-detector / non-LLM-enum-inferrer / non-statistical-aggregator / non-uniqueness-checker / non-cardinality-bound-checker". Distinct from M84 (`const` — sibling first-property axis on the same firstVal but different shape: M84 is an UNCONDITIONAL single-value pin; M85 is an UNCONDITIONAL bounded value-SET), M83 (`dependencies` — CONDITIONAL trigger-keyed MAP), M82 (`required` — key-presence list), M81 (`propertyNames` — single SCHEMA on EVERY name), M80 (`patternProperties` — regex-keyed-MAP), M50 (manifest-firstKey required — different slot). VISION primary citation: lines 218–227 (`### Embeddable Widgets And Command Surfaces`) — first milestone to use this section as primary aggregation source; the section enumerates a discrete value-set of action verbs (refund/assign/create/update/upgrade/suspend/invite) and command-surface modalities exactly mappable to Draft-7 `enum` annotations. The full M85 § will be materialized by the dev role's implementation phase per the M27–M84 PM-vs-dev ownership split. M85 is implementation-ready planned work, not shipped. -->

> **M84 Materialized — 2026-04-29, run_91fd37340cef71d1, turn_cc991c7e1fc3d487, dev attempt 1.** All PM-frozen scope carried forward verbatim. Implementation: `classifyInputSchemaFirstPropertyConstant` (pure function; NO-TYPE-APPLICABILITY-OBJECT-RESTRICTION: M84-SPECIFIC — MUST NOT inspect firstVal.type to gate not_applicable (Draft-7 §6.1.3 defines const as applicable to ANY type); ABSENT-AS-UNTYPED: own-property 'const' not present → untyped, no warning — Draft-7 default no value pin; UNDEFINED-EXPLICIT-AS-UNKNOWN: own-property present but value is JS undefined → unknown WITH 6th code; NULL-AS-TYPED: const:null → typed, no warning — M84-SPECIFIC, distinct from M55–M83 NULL-AS-ABSENT (Draft-7 §6.1.3 permits const:null as valid pin to literal JSON null); JSON-STRING-AS-TYPED: typeof 'string' → typed; JSON-FINITE-NUMBER-AS-TYPED: typeof 'number' && Number.isFinite → typed; NaN/Infinity/-Infinity → unknown WITH 6th code; JSON-BOOLEAN-AS-TYPED: typeof 'boolean' → typed; JSON-ARRAY-AS-TYPED: Array.isArray → typed including empty []; JSON-PLAIN-OBJECT-AS-TYPED: Object.prototype.toString.call(v)==='[object Object]' → typed including empty {}; DRAFT-7-ANY-JSON-VALUE-IS-VALID-CONST: any other → unknown WITH 6th code; NO-COERCION via Array.from/Object/JSON-roundtrip/String/Number/Boolean/!!), `buildInputSchemaFirstPropertyConstantIndex`, `formatInputSchemaFirstPropertyConstantIndex`, `cmdConstant`/`cmdConstantIndex`/`parseConstantIndexArgs` in `src/cli.js`; CLI surface 67→68 (`constant` between `confidence` and `crowded`; confidence(con,f=102)<constant(con,s=115) at pos 3; constant(c,o)<crowded(c,r) at pos 1); 18-case M84 smoke matrix; eval scenario `input-schema-first-property-const-index-determinism` (74→75 scenarios). npm test exits 0 with 75 scenarios. See IMPLEMENTATION_NOTES.md § M84 for full detail.

<!-- M84 Charter Sketch Reservation: 2026-04-29, run_91fd37340cef71d1, turn_d96a96af76c22441, PM attempt 1, HEAD prior to PM edits at `ff1e109`. Reserves a future § "Input Schema First Property Const Annotation Presence Index" section for the planning-phase M84 charter. Axis: `input_schema.properties[firstKey].const` (JSON-Schema Draft 7 §6.1.3 SINGLE-VALUE-PIN annotation governing the locked-to-exactly-one-value declaration on the first input parameter; EIGHTH first-property axis annotation in the M77+ first-property cluster after M77 `additionalProperties`, M78 `minProperties`, M79 `maxProperties`, M80 `patternProperties`, M81 `propertyNames`, M82 `required`, M83 `dependencies`; the FIRST in the cluster whose Draft-7 keyword is NOT restricted to `type === 'object'` schemas — Draft-7 §6.1.3 defines `const` as applicable to ANY type; the FIRST in the cluster whose ABSENT-vs-PRESENT distinction includes the NULL-AS-TYPED rule, distinct from M55–M83's NULL-AS-ABSENT pattern). Bucket-key enum (closed four-value): `typed | untyped | not_applicable | unknown`. Aggregation_key enum (closed three-value): `value_pinning_constraint | not_applicable | unknown`. Six frozen warning reason codes (5 baseline + axis-specific 6th `input_schema_properties_first_property_const_invalid_when_present`). Bucket iteration order: `typed → untyped → not_applicable → unknown` (deterministic stable-output only — NOT a workflow-priority / state-machine-readiness-rank / state-transition-graph-emit-priority / status-field-extractor-rank / enum-extractor-priority / workflow-checkpoint-rank / discrete-state-lexicon-rank / .tusq/state-machines.json-emit-priority / .tusq/workflows.json-emit-priority / inferred-workflow-editability-rank ranking). Rules: NO-TYPE-APPLICABILITY-OBJECT-RESTRICTION (M84-SPECIFIC: classifier MUST NOT inspect `firstVal.type` to gate `not_applicable` — Draft-7 §6.1.3 defines `const` as applicable to ANY type; outer-schema `type === 'object'` prerequisite RETAINED because without an object outer there is no `firstVal`); ABSENT-AS-UNTYPED (own `'const'` property not present → `untyped`, no warning); NULL-AS-TYPED (M84-SPECIFIC, distinct from M55–M83 NULL-AS-ABSENT: `firstVal.const === null` → `typed` — Draft-7 §6.1.3 explicitly permits `const: null` as a valid pin to the literal JSON `null`); JSON-STRING-AS-TYPED (`typeof === 'string'` → typed); JSON-FINITE-NUMBER-AS-TYPED (`typeof === 'number' && Number.isFinite` → typed; non-finite NaN/Infinity/-Infinity → unknown WITH 6th code); JSON-BOOLEAN-AS-TYPED (`typeof === 'boolean'` → typed); JSON-ARRAY-AS-TYPED (`Array.isArray` → typed for empty `[]` and non-empty arrays — element-shape NOT recursively validated; the empty-array LITERAL pins to the empty-array VALUE, distinct from M82's EMPTY-ARRAY-AS-UNTYPED); JSON-PLAIN-OBJECT-AS-TYPED (`Object.prototype.toString.call(v) === '[object Object]'` → typed for empty `{}` and non-empty plain-object — the empty-object LITERAL pins to the empty-object VALUE, distinct from M81's EMPTY-OBJECT-SCHEMA-AS-UNTYPED); UNDEFINED-EXPLICIT-AS-UNKNOWN (own-property `'const'` present but value is JavaScript `undefined` → unknown WITH 6th code; JavaScript `undefined` is NOT a JSON value); DRAFT-7-ANY-JSON-VALUE-IS-VALID-CONST (any other value: function / Symbol / BigInt / Date / RegExp / Set / Map / WeakMap / WeakSet / Promise / ArrayBuffer / DataView / typed-array / Error / generator / iterator / custom class instance → unknown WITH 6th code; NO-COERCION via `Array.from()` / `Object()` / `JSON.parse(JSON.stringify())` / `String(v)` / `Number(v)` / `Boolean(v)` / `!!v` / `+v` — strict per-type `typeof` / `Number.isFinite` / `Array.isArray` / `Object.prototype.toString.call(v) === '[object Object]'` checks AND treat literal JavaScript `null` as `typed` via explicit `firstVal.const === null` reference equality, NOT via `==` or any nullish-coalesce trick). CLI surface 67 → 68 with new noun `constant` between `confidence` and `crowded` (confidence(c=99,o=111,n=110,f=102) < constant(c=99,o=111,n=110,s=115) at pos 3 (f(102)<s(115)); constant(c=99,o=111,n=110,s=115) < crowded(c=99,r=114) at pos 1 (o(111)<r(114))). Result-array field name: `first_property_const_states[]`. Per-bucket field name: `input_schema_first_property_const`. Constraint 77 (proposed) reserves M84 as "non-runtime-value-pin-validator / non-runtime-value-equality-rejector / non-runtime-state-checker / non-DTO-const-validator / non-developer-artifacts-emitter / non-strict-tool-emitter-const-validator / non-MCP-server-const-validator / non-marketplace-package-const-validator / non-validation-layer-const-validator / non-enum-crossref / non-default-crossref / non-required-crossref / non-property-names-crossref / non-pattern-properties-crossref / non-additional-properties-crossref / non-min-properties-crossref / non-max-properties-crossref / non-dependencies-crossref / non-doc-contradiction-detector / non-LLM-const-inferrer / non-statistical-aggregator / non-state-machine-emitter / non-state-transition-graph-emitter / non-status-field-extractor / non-enum-extractor / non-workflow-checkpoint-emitter / non-discrete-state-lexicon-emitter / non-stuck-state-detector". Distinct from M83 (`dependencies` — sibling first-property axis on the same firstVal but different shape: M83 is a CONDITIONAL trigger-keyed MAP of LIST-or-SCHEMA values; M84 is an UNCONDITIONAL single-value pin), M82 (`required` — sibling: UNCONDITIONAL flat array of NAME strings), M81 (`propertyNames` — sibling: single SCHEMA on EVERY name), M80 (`patternProperties` — sibling: regex-keyed MAP of SCHEMA values), M50 (`firstKey required` membership — manifest-firstKey-level, different field/slot entirely). VISION primary citation: lines 424–449 (`## Workflow And State Understanding`) — first milestone to use this section as primary aggregation source; the section enumerates inputs to workflow inference (routes, services, jobs, validators, enums, database models, status fields, state transition functions, integration handlers, UI flow hints) and prescribes explicit emitted artifacts (`.tusq/workflows.json`, `.tusq/state-machines.json`, workflow references in `tusq.manifest.json`); a capability whose first input property declares `const: <state-value>` is the strongest pre-runtime signal that the capability is locked to a specific node in an inferred state machine. The full M84 § will be materialized by the dev role's implementation phase per the M27–M83 PM-vs-dev ownership split. M84 is implementation-ready planned work, not shipped. -->

<!-- M83 Charter Sketch Reservation SUPERSEDED by M83 Materialized below. Original text: 2026-04-29, run_4be2c82d93272ed2, turn_2cf67f4599fb20f8, PM attempt 1, HEAD prior to PM edits at `6b72847`. Reserves a future § "Input Schema First Property Dependencies Annotation Presence Index" section for the planning-phase M83 charter. Axis: `input_schema.properties[firstKey].dependencies` (JSON-Schema Draft 7 §6.5.7 OBJECT-PROPERTY-DEPENDENCIES-HETEROGENEOUS-MAP annotation governing the conditional-dependency map from property-name → either array-of-property-names (LIST/dependentRequired form) OR plain-object Draft-7 sub-schema (SCHEMA/dependentSchemas form) on object-typed first input parameters; SEVENTH object-axis annotation in the M77+ object-axis cluster after M77 `additionalProperties`, M78 `minProperties`, M79 `maxProperties`, M80 `patternProperties`, M81 `propertyNames`, and M82 `required`; the HETEROGENEOUS-VALUE-MAP-typed sibling — orthogonal field semantics: M77 governs presence-of-extras, M78/M79 govern key-COUNT bounds, M80 governs regex-MAP-key-typing, M81 governs single-schema-on-EVERY-key-NAME, M82 governs WHICH SPECIFIC key NAMES MUST be present, M83 governs CONDITIONAL co-presence and co-shape requirements keyed by triggering property name; the only object-axis whose value map permits TWO fundamentally different value types per entry, the LIST/SCHEMA bifurcation that Draft-2019-09 split into the separate `dependentRequired` and `dependentSchemas` keywords). Bucket-key enum (closed four-value): `typed | untyped | not_applicable | unknown`. Aggregation_key enum (closed three-value): `object_property_dependencies_constraint | not_applicable | unknown`. Six frozen warning reason codes (5 baseline + axis-specific 6th `input_schema_properties_first_property_dependencies_invalid_when_present`). Bucket iteration order: `typed → untyped → not_applicable → unknown` (deterministic stable-output only — NOT an ecosystem-bots-priority / Slack-bot-emit-readiness-rank / Teams-bot-emit-priority / Zendesk-app-readiness-rank / Intercom-workflow-priority / Salesforce-copilot-rank / HubSpot-copilot-priority / Gainsight-CSM-assistant-rank / admin-console-embed-priority / integration-config-completeness-rank / conditional-required-validator-priority ranking). Rules: TYPE-APPLICABILITY-OBJECT (`firstVal.type` is a string but NOT `'object'` → `not_applicable`, parallel to M77/M78/M79/M80/M81/M82 — Draft-7 defines `dependencies` ONLY for `type === 'object'`); ABSENT-AS-UNTYPED (own property absent OR `dependencies: undefined` → `untyped` — Draft-7 default no conditional dependency); NULL-AS-ABSENT (`dependencies: null` → `untyped` — mirrors M55–M82); EMPTY-MAP-AS-UNTYPED (`dependencies: {}` plain-object with zero own keys → `untyped` — Draft-7 §6.5.7 empty map equivalent to absence); NON-EMPTY-PLAIN-OBJECT-WITH-VALID-HETEROGENEOUS-VALUES-AS-TYPED (M83-SPECIFIC: `Object.prototype.toString.call(v) === '[object Object]'` AND `Object.keys(v).length >= 1` AND every value is EITHER (a) `Array.isArray(value)` AND every element is `typeof === 'string'` AND every element string `length >= 1` (the LIST/dependentRequired form — empty arrays are PERMITTED as a LIST entry per Draft-7 §6.5.7 even though empty top-level `required: []` is EMPTY-ARRAY-AS-UNTYPED at M82; the asymmetry exists because a `dependencies` MAP entry's PRESENCE in the map is itself the constraint declaration, while top-level `required: []` declares no triggering names at all) OR (b) `Object.prototype.toString.call(value) === '[object Object]'` (the SCHEMA/dependentSchemas form — any plain-object Draft-7 sub-schema is permitted, including `{}` as the empty-schema; keyword coherence NOT validated by classifier — Draft-7 leaves keyword interpretation to the validator) → `typed`; uniqueness of LIST elements NOT validated, semantic coherence of SCHEMA forms NOT validated, key-name validity in the outer MAP keyspace NOT validated — Draft-7 leaves all three to the validator); DRAFT-7-MAP-WITH-VALID-HETEROGENEOUS-VALUES-IS-VALID-DEPENDENCIES (any other `firstVal.dependencies` value: string scalar / finite number / NaN / Infinity / -Infinity / boolean / array / non-plain-object reference type, OR plain-object MAP whose ANY value is NOT `Array.isArray` AND NOT `[object Object]`, OR plain-object MAP whose ANY array-typed value contains a non-string element / empty-string element → `unknown` WITH 6th code; NO-COERCION via `Array.from()` / `Object()` / `JSON.parse(JSON.stringify())` / `String(v).split(...)` / `Boolean(v)` / `!!v` — strict `Object.prototype.toString.call(v) === '[object Object]'` for the outer MAP case AND strict per-value classification (strict `Array.isArray(value)` for the LIST case with strict `typeof element === 'string' && element.length >= 1` for every element AND strict `Object.prototype.toString.call(value) === '[object Object]'` for the SCHEMA case)). CLI surface 66 → 67 with new noun `dependent` between `crowded` and `divisor` (crowded(c=99) < dependent(d=100) at pos 0 (c(99)<d(100)); dependent(d=100, e=101) < divisor(d=100, i=105) at pos 1 (e(101)<i(105))). Result-array field name: `first_property_dependencies_states[]`. Per-bucket field name: `input_schema_first_property_dependencies`. Constraint 76 (proposed) reserves M83 as "non-runtime-conditional-required-validator / non-runtime-dependent-schema-applier / non-runtime-conditional-rejector / non-DTO-dependent-required-validator / non-DTO-dependent-schemas-validator / non-developer-artifacts-emitter / non-strict-tool-emitter-dependencies-validator / non-MCP-server-dependencies-validator / non-marketplace-package-dependencies-validator / non-validation-layer-dependencies-validator / non-required-crossref / non-property-names-crossref / non-pattern-properties-crossref / non-additional-properties-crossref / non-min-properties-crossref / non-max-properties-crossref / non-properties-count-crossref / non-doc-contradiction-detector / non-LLM-dependencies-inferrer / non-statistical-aggregator / non-keyword-coherence-validator / non-dependent-required-vs-dependent-schemas-disambiguator / non-state-machine-emitter / non-eligibility-emitter / non-conditional-flow-emitter / non-integration-config-validator / non-Slack-bot-emitter / non-Teams-bot-emitter / non-Zendesk-app-emitter / non-Intercom-workflow-emitter / non-Salesforce-copilot-emitter / non-HubSpot-copilot-emitter / non-cycle-detector". Distinct from M82 (`required` — sibling object-axis on the same firstVal but different shape: M82 is an UNCONDITIONAL flat array of NAME strings; M83 is a CONDITIONAL trigger-keyed MAP of either ARRAY-of-name-strings (LIST) or SCHEMA values), M81 (`propertyNames` — sibling object-axis: M81 is a single SCHEMA applied to EVERY property name uniformly governing what names are PERMITTED; M83 governs CONDITIONAL co-presence on a per-trigger-name basis), M80 (`patternProperties` — sibling object-axis: M80 is a regex-keyed MAP applying SCHEMA constraints to keys whose names match a regex; M83 is a name-keyed MAP applying CONDITIONAL constraints triggered by the literal key name's presence), M50 (`firstKey required` membership — manifest-firstKey-level, different field/slot entirely). VISION primary citation: lines 313–325 (`### Ecosystem Bots And Integrations`, in § What We Output) — first milestone to use this section as primary aggregation source; the section enumerates Slack/Teams/Zendesk/Intercom/Salesforce/HubSpot/Gainsight integration packages whose configurations carry CONDITIONAL fields. The full M83 § will be materialized by the dev role's implementation phase per the M27–M82 PM-vs-dev ownership split. M83 is implementation-ready planned work, not shipped. -->

> **M83 Materialized — 2026-04-29, run_4be2c82d93272ed2, turn_57cac3046689ed6e, dev attempt 1.** All PM-frozen scope carried forward verbatim. Implementation: `classifyInputSchemaFirstPropertyDependencies` (pure function; TYPE-APPLICABILITY-OBJECT: firstVal.type string but NOT 'object' → not_applicable — Draft-7 defines dependencies ONLY for type==='object'; mirrors M77/M78/M79/M80/M81/M82; ABSENT-AS-UNTYPED: dependencies absent/undefined → untyped, no warning; NULL-AS-ABSENT: dependencies:null → untyped, no warning — mirrors M55–M82; EMPTY-MAP-AS-UNTYPED: dependencies:{} → untyped, no warning — Draft-7 §6.5.7 empty map equivalent to absence — M83-SPECIFIC; NON-EMPTY-PLAIN-OBJECT-WITH-VALID-HETEROGENEOUS-VALUES-AS-TYPED: outer plain-object MAP with >=1 own keys AND every value is EITHER Array.isArray+every-element-typeof==='string'+every-element.length>=1 (LIST form; empty array [] PERMITTED — M83-SPECIFIC asymmetry vs M82 EMPTY-ARRAY-AS-UNTYPED) OR Object.prototype.toString.call(v)==='[object Object]' (SCHEMA form; empty {} permitted) → typed, no warning; DRAFT-7-MAP-WITH-VALID-HETEROGENEOUS-VALUES-IS-VALID-DEPENDENCIES: any other → unknown WITH 6th code; NO-COERCION via Array.from/Object/JSON-roundtrip/String.split/Boolean/!!), `buildInputSchemaFirstPropertyDependenciesIndex`, `formatInputSchemaFirstPropertyDependenciesIndex`, `cmdDependent`/`cmdDependentIndex`/`parseDependentIndexArgs` in `src/cli.js`; CLI surface 66→67 (`dependent` between `crowded` and `divisor`; crowded(c=99)<dependent(d=100) at pos 0; dependent(d=100,e=101)<divisor(d=100,i=105) at pos 1); 18-case M83 smoke matrix; eval scenario `input-schema-first-property-dependencies-index-determinism` (73→74 scenarios). npm test exits 0 with 74 scenarios. See IMPLEMENTATION_NOTES.md § M83 for full detail.

> **M82 Materialized — 2026-04-29, run_14a2d7109f75b8b8, turn_985f297eece43905, dev attempt 1.** All PM-frozen scope carried forward verbatim. Implementation: `classifyInputSchemaFirstPropertyRequired` (pure function; TYPE-APPLICABILITY-OBJECT: firstVal.type string but NOT 'object' → not_applicable — Draft-7 defines required ONLY for type==='object'; mirrors M77/M78/M79/M80/M81; ABSENT-AS-UNTYPED: required absent/undefined → untyped, no warning — Draft-7 default no required-keys list; NULL-AS-ABSENT: required:null → untyped, no warning — mirrors M55–M81; EMPTY-ARRAY-AS-UNTYPED: required:[] → untyped, no warning — Draft-7 §5.21 empty array equivalent to absence — M82-SPECIFIC; NON-EMPTY-STRING-ARRAY-AS-TYPED: Array.isArray AND length>=1 AND every typeof==='string' AND every length>=1 → typed, no warning; DRAFT-7-ARRAY-OF-NON-EMPTY-STRING-IS-VALID-REQUIRED: any other value → unknown WITH 6th code; NO-COERCION via Array.from/Object/JSON-roundtrip/String.split/Boolean/!!), `buildInputSchemaFirstPropertyRequiredIndex`, `formatInputSchemaFirstPropertyRequiredIndex`, `cmdRequired`/`cmdRequiredIndex`/`parseRequiredIndexArgs` in `src/cli.js`; CLI surface 65→66 (`required` between `request` and `response`; request(r=114,e=101,q=113,u=117,e=101)<required(r=114,e=101,q=113,u=117,i=105) at pos 4 (e(101)<i(105)); required(r=114,e=101,q=113)<response(r=114,e=101,s=115) at pos 2 (q(113)<s(115))); 18-case M82 smoke matrix; eval scenario `input-schema-first-property-required-index-determinism` (72→73 scenarios). npm test exits 0 with 73 scenarios. See IMPLEMENTATION_NOTES.md § M82 for full detail.

<!-- M82 Charter Sketch Reservation — 2026-04-29, run_14a2d7109f75b8b8, turn_ecf66d223469cd45, PM attempt 1, HEAD prior to PM edits at `3fcda55`. Reserves a future § "Input Schema First Property Required Annotation Presence Index" section for the planning-phase M82 charter. Axis: `input_schema.properties[firstKey].required` (JSON-Schema Draft 7 §5.21 OBJECT-PROPERTY-REQUIRED-KEYS-LIST annotation governing the array of property NAME strings that MUST be present at runtime on object-typed first input parameters — non-empty array of unique non-empty strings; SIXTH object-axis annotation in the M77+ object-axis cluster after M77 `additionalProperties`, M78 `minProperties`, M79 `maxProperties`, M80 `patternProperties`, and M81 `propertyNames`; LIST-typed sibling of all five prior axes — orthogonal field semantics: M77 governs presence-of-extras, M78/M79 govern key-COUNT bounds, M80 governs regex-MAP-key-typing, M81 governs single-schema-on-EVERY-key-NAME, M82 governs WHICH SPECIFIC key NAMES MUST be present). Bucket-key enum (closed four-value): `typed | untyped | not_applicable | unknown`. Aggregation_key enum (closed three-value): `object_property_required_keys_constraint | not_applicable | unknown`. Six frozen warning reason codes (5 baseline + axis-specific 6th `input_schema_properties_first_property_required_invalid_when_present`). Bucket iteration order: `typed → untyped → not_applicable → unknown` (deterministic stable-output only — NOT a domain-workflow-priority / state-machine-readiness-rank / approval-path-emit-priority / eligibility-evaluator-rank / limits-enforcement-priority / DTO-validator-rank / runtime-completeness-checker-priority ranking). Rules: TYPE-APPLICABILITY-OBJECT (`firstVal.type` is a string but NOT `'object'` → `not_applicable`, parallel to M77/M78/M79/M80/M81 — Draft-7 defines `required` ONLY for `type === 'object'`); ABSENT-AS-UNTYPED (own property absent OR `required: undefined` → `untyped` — Draft-7 default no required-keys list); NULL-AS-ABSENT (`required: null` → `untyped` — mirrors M55–M81); EMPTY-ARRAY-AS-UNTYPED (`required: []` → `untyped` — Draft-7 §5.21 empty array equivalent to absence; an explicit "considered and set to none" marker that nonetheless declares no constraint); NON-EMPTY-STRING-ARRAY-AS-TYPED (`Array.isArray(v)` AND `v.length >= 1` AND every element is `typeof === 'string'` AND every element string `length >= 1` → `typed` — JSON-Schema Draft 7 §5.21 declares `required` MUST be a non-empty array of unique non-empty strings; uniqueness NOT validated by classifier — Draft-7 leaves uniqueness to validator; tusq treats a duplicate-element array as still-typed with the operator-declared key set; keyword coherence with `properties` NOT validated — Draft-7 leaves cross-field coherence to the validator); DRAFT-7-ARRAY-OF-NON-EMPTY-STRING-IS-VALID-REQUIRED (any other `firstVal.required` value: string scalar / finite number / NaN / Infinity / -Infinity / boolean / plain-object / non-plain-object reference type, OR Array containing any non-string element / any empty-string element → `unknown` WITH 6th code; NO-COERCION via `Array.from()` / `Object()` / `JSON.parse(JSON.stringify())` / `String(v).split(...)` / `Boolean(v)` / `!!v` — strict `Array.isArray(v)` for the array case AND strict `typeof element === 'string' && element.length >= 1` for every element). CLI surface 65 → 66 with new noun `required` between `request` and `response` (request(r=114, e=101, q=113, u=117, e=101) < required(r=114, e=101, q=113, u=117, i=105) at pos 4 (e(101)<i(105)); required(r=114, e=101, q=113) < response(r=114, e=101, s=115) at pos 2 (q(113)<s(115))). Result-array field name: `first_property_required_states[]`. Per-bucket field name: `input_schema_first_property_required`. Constraint 75 (proposed) reserves M82 as "non-runtime-required-keys-validator / non-runtime-missing-key-rejector / non-runtime-payload-completeness-checker / non-DTO-required-keys-validator / non-developer-artifacts-emitter / non-strict-tool-emitter-required-validator / non-MCP-server-required-keys-validator / non-marketplace-package-required-keys-validator / non-validation-layer-required-keys-validator / non-property-names-crossref / non-pattern-properties-crossref / non-additional-properties-crossref / non-min-properties-crossref / non-max-properties-crossref / non-properties-count-crossref / non-doc-contradiction-detector / non-LLM-required-inferrer / non-statistical-aggregator / non-uniqueness-validator / non-keyword-coherence-validator / non-state-machine-emitter / non-eligibility-emitter / non-approval-path-emitter / non-workflow-step-emitter". Distinct from M81 (`propertyNames` — sibling object-axis on the same firstVal but different field semantic and shape: M81 is a single SCHEMA applied to EVERY property name uniformly governing what names are PERMITTED; M82 is a LIST of specific keys that MUST be PRESENT — M81 governs name typing, M82 governs name presence), M50 (`firstKey required` membership at the manifest-firstKey level — different field/slot, manifest-shape question vs first-property-schema-shape question). VISION primary citation: lines 74–82 (`### Domain Model And Workflows`, in § Input Sources) — first milestone to use this section as primary aggregation source; the literal phrase "required fields" on line 78 is the VISION anchor. The full M82 § will be materialized by the dev role's implementation phase per the M27–M81 PM-vs-dev ownership split. M82 is implementation-ready planned work, not shipped. -->

> **M81 Materialized — 2026-04-29, run_367446ff4d285c65, turn_6ac22b755c1fd5d7, dev attempt 1.** All PM-frozen scope carried forward verbatim. Implementation: `classifyInputSchemaFirstPropertyPropertyNames` (pure function; TYPE-APPLICABILITY-OBJECT: firstVal.type string but NOT 'object' → not_applicable — Draft-7 defines propertyNames ONLY for type==='object'; mirrors M77/M78/M79/M80; ABSENT-AS-UNTYPED: propertyNames absent/undefined → untyped, no warning — Draft-7 default no name constraint; NULL-AS-ABSENT: propertyNames:null → untyped, no warning — mirrors M55–M80; BOOLEAN-TRUE-AS-UNTYPED: propertyNames===true → untyped, no warning — Draft-7 §4.4 boolean true equivalent to empty schema; BOOLEAN-FALSE-AS-TYPED: propertyNames===false → typed, no warning — Draft-7 §4.4 boolean false rejects all values; M81-SPECIFIC rule without analog in M77–M80; EMPTY-OBJECT-SCHEMA-AS-UNTYPED: propertyNames:{} plain-object with zero own keys → untyped, no warning — explicit empty schema is untyped equivalent of absent or true; PLAIN-OBJECT-WITH-OWN-KEYS-AS-TYPED: plain-object with >= 1 own enumerable keyword → typed, no warning — any plain-object schema with at least one keyword declares an explicit constraint; DRAFT-7-BOOLEAN-OR-OBJECT-IS-VALID-PROPERTY-NAMES: any other value → unknown WITH 6th code; NO-COERCION via strict Object.prototype.toString.call(v)==='[object Object]' for object case AND strict v===true||v===false for boolean case), `buildInputSchemaFirstPropertyPropertyNamesIndex`, `formatInputSchemaFirstPropertyPropertyNamesIndex`, `cmdNamed`/`cmdNamedIndex`/`parseNamedIndexArgs` in `src/cli.js`; CLI surface 64→65 (`named` between `most` and `nullable`; most(m=109)<named(n=110) at pos 0 (m(109)<n(110)); named(n=110,a=97)<nullable(n=110,u=117) at pos 1 (a(97)<u(117))); 18-case M81 smoke matrix; eval scenario `input-schema-first-property-property-names-index-determinism` (71→72 scenarios). npm test exits 0 with 72 scenarios. See IMPLEMENTATION_NOTES.md § M81 for full detail.

<!-- M81 Charter Sketch Reservation (superseded by M81 Materialized above) — 2026-04-29, run_367446ff4d285c65, turn_3b688ae4cc2b42e8, PM attempt 1, HEAD prior to PM edits at `3e2d570`. Reserves a future § "Input Schema First Property PropertyNames Annotation Presence Index" section for the planning-phase M81 charter. Axis: `input_schema.properties[firstKey].propertyNames` (JSON-Schema Draft 7 OBJECT-PROPERTY-NAME-SUBSCHEMA annotation governing the single Draft-7 sub-schema applied to every property NAME on object-typed first input parameters — boolean-or-plain-object Draft-7 schema; FIFTH object-axis annotation in the M77+ object-axis cluster after M77 `additionalProperties`, M78 `minProperties`, M79 `maxProperties`, and M80 `patternProperties`; SCHEMA-typed sibling of the MAP-typed M80 — `patternProperties` constrains keys whose names MATCH a regex to specific sub-schemas, `propertyNames` constrains the names themselves uniformly across ALL keys, the two are orthogonal and frequently combined at validation time but their static annotations are independent). Bucket-key enum (closed four-value): `typed | untyped | not_applicable | unknown`. Aggregation_key enum (closed three-value): `object_property_name_subschema_constraint | not_applicable | unknown`. Six frozen warning reason codes (5 baseline + axis-specific 6th `input_schema_properties_first_property_property_names_invalid_when_present`). Bucket iteration order: `typed → untyped → not_applicable → unknown` (deterministic stable-output only — NOT a code-and-API-surface-priority / OpenAPI-emit-readiness-rank / GraphQL-schema-precision-rank / SDK-codegen-priority / route-validator-rank / RPC-field-validator-priority / request-payload-name-rejection-priority ranking). Rules: TYPE-APPLICABILITY-OBJECT (`firstVal.type` is a string but NOT `'object'` → `not_applicable`, parallel to M77/M78/M79/M80 — Draft-7 defines `propertyNames` ONLY for `type === 'object'`); ABSENT-AS-UNTYPED (own property absent OR `propertyNames: undefined` → `untyped` — Draft-7 default no name constraint); NULL-AS-ABSENT (`propertyNames: null` → `untyped` — mirrors M55–M80); BOOLEAN-TRUE-AS-UNTYPED (`propertyNames: true` → `untyped` — Draft-7 §4.4 boolean-true equivalent to empty schema, accepts all names); BOOLEAN-FALSE-AS-TYPED (`propertyNames: false` → `typed` — Draft-7 §4.4 boolean-false rejects all values, applied to names this constrains permitted property names to the empty set; the strongest possible name constraint, NOT an absence marker); EMPTY-OBJECT-SCHEMA-AS-UNTYPED (`propertyNames: {}` plain-object with zero own enumerable keys → `untyped` — explicit empty schema declares no constraint, equivalent to absent or `true`); PLAIN-OBJECT-WITH-OWN-KEYS-AS-TYPED (`Object.prototype.toString.call(v) === '[object Object]'` AND `Object.keys(v).length >= 1` → `typed` — any plain-object schema with at least one own enumerable keyword declares an explicit constraint; keyword coherence NOT validated by the classifier — Draft-7 leaves keyword interpretation to the validator); DRAFT-7-BOOLEAN-OR-OBJECT-IS-VALID-PROPERTY-NAMES (any other `firstVal.propertyNames` value: string / finite number / NaN / Infinity / -Infinity / array / non-plain-object reference type → `unknown` WITH 6th code; NO-COERCION via `Object()` / `JSON.parse(JSON.stringify())` / `typeof v === 'object'` alone / `Boolean(v)` / `!!v` — strict `Object.prototype.toString.call(v) === '[object Object]'` for the object case AND strict `v === true || v === false` for the boolean case). CLI surface 64 → 65 with new noun `named` between `most` and `nullable` (most(m=109) < named(n=110) at pos 0; named(n=110, a=97) < nullable(n=110, u=117) at pos 1 (a(97)<u(117))). Result-array field name: `first_property_property_names_states[]`. Per-bucket field name: `input_schema_first_property_property_names`. Constraint 74 (proposed) reserves M81 as "non-runtime-property-name-validator / non-runtime-key-rejector / non-runtime-property-name-stripper / non-DTO-property-name-validator / non-developer-artifacts-emitter / non-strict-tool-emitter-property-names-validator / non-MCP-server-input-property-names-validator / non-marketplace-package-property-names-validator / non-validation-layer-property-names-validator / non-pattern-properties-crossref / non-additional-properties-crossref / non-min-properties-crossref / non-max-properties-crossref / non-required-crossref / non-properties-count-crossref / non-pattern-string-crossref / non-doc-contradiction-detector / non-LLM-property-names-inferrer / non-statistical-aggregator / non-regex-compiler / non-regex-validator / non-keyword-coherence-validator". Distinct from M80 (`patternProperties` — sibling object-axis on the same firstVal but different field semantic and shape: M80 is a MAP `{ <regex>: <subschema>, ... }` constraining keys whose names match specific regex patterns to specific sub-schemas; M81 is a single SCHEMA applied to EVERY property name uniformly), M59 (`pattern` — DIFFERENT FIELD SEMANTIC AND DIFFERENT TYPE-APPLICABILITY: `pattern` regex constrains string VALUES on `type: 'string'` firstVals; `propertyNames` schema constrains key NAMES on `type: 'object'` firstVals). VISION primary citation: lines 40–46 (`### Code And API Surface`, in § Input Sources) — first milestone to use this section as primary aggregation source. The full M81 § will be materialized by the dev role's implementation phase per the M27–M80 PM-vs-dev ownership split. M81 is implementation-ready planned work, not shipped. -->

> **M80 Materialized — 2026-04-29, run_9bd8558c73fb239e, turn_64938221e4a6227d, dev attempt 1.** All PM-frozen scope carried forward verbatim. Implementation: `classifyInputSchemaFirstPropertyPatternProperties` (pure function; TYPE-APPLICABILITY-OBJECT: firstVal.type string but NOT 'object' → not_applicable — Draft-7 defines patternProperties ONLY for type==='object'; ABSENT-AS-UNTYPED: patternProperties absent/undefined → untyped, no warning — Draft-7 default is no regex map; NULL-AS-ABSENT: patternProperties:null → untyped, no warning — mirrors M55–M79; EMPTY-MAP-AS-UNTYPED: patternProperties:{} plain-object with zero own keys → untyped, no warning — explicit empty map declares no regex-keyed sub-schemas; PLAIN-OBJECT-WITH-OWN-KEYS-AS-TYPED: plain-object with >= 1 own key AND every value is boolean OR plain-object subschema → typed, no warning; DRAFT-7-PLAIN-OBJECT-MAP-IS-VALID-PATTERN-PROPERTIES: any other value → unknown WITH 6th code; NO-COERCION via Object.prototype.toString.call() strict '[object Object]' checks for outer map AND each value), `buildInputSchemaFirstPropertyPatternPropertiesIndex`, `formatInputSchemaFirstPropertyPatternPropertiesIndex`, `cmdPartition`/`cmdPartitionIndex`/`parsePartitionIndexArgs` in `src/cli.js`; CLI surface 63→64 (`partition` between `parameter` and `path`; parameter(p=112,a=97,r=114,a=97)<partition(p=112,a=97,r=114,t=116) at pos 3 (a(97)<t(116)); partition(p=112,a=97,r=114,t=116)<path(p=112,a=97,t=116,h=104) at pos 2 (r(114)<t(116))); 18-case M80 smoke matrix; eval scenario `input-schema-first-property-pattern-properties-index-determinism` (70→71 scenarios); Constraint 73: non-runtime-pattern-property-validator / non-runtime-regex-key-matcher / non-runtime-property-stripper / non-DTO-pattern-property-deserializer / non-developer-artifacts-emitter / non-strict-tool-emitter-pattern-properties-validator / non-MCP-server-input-pattern-properties-validator / non-marketplace-package-pattern-properties-validator / non-validation-layer-pattern-properties-validator / non-additional-properties-crossref / non-min-properties-crossref / non-max-properties-crossref / non-property-names-crossref / non-required-crossref / non-properties-count-crossref / non-pattern-string-crossref / non-doc-contradiction-detector / non-LLM-pattern-properties-inferrer / non-statistical-aggregator / non-regex-compiler / non-regex-validator. Distinct from M79 (maxProperties — sibling object-axis, different field semantic: count-ceiling vs regex-key-pattern-map), M78 (minProperties — sibling object-axis, different field semantic: count-floor vs regex-key-pattern-map), M77 (additionalProperties — sibling object-axis, different field semantic: extension-control vs regex-key-pattern-map), M59 (pattern — DIFFERENT FIELD SEMANTIC AND DIFFERENT TYPE-APPLICABILITY: pattern regex constrains string VALUES on type:'string' firstVals; patternProperties regex map constrains object KEY-NAMES-AND-VALUE-SHAPES on type:'object' firstVals). npm test exits 0 with 71 scenarios. See IMPLEMENTATION_NOTES.md § M80 for full detail.

> **M80 Charter Sketch Reservation — 2026-04-29, run_9bd8558c73fb239e, turn_6cad760484a29149, PM attempt 1, HEAD prior to PM edits at `6a04b3f`.** [SUPERSEDED by M80 Materialized above] Reserves a future § "Input Schema First Property PatternProperties Annotation Presence Index" section for the planning-phase M80 charter. Axis: `input_schema.properties[firstKey].patternProperties` (JSON-Schema Draft 7 OBJECT-PROPERTY-NAME-PATTERN-MAP annotation governing the regex-keyed sub-schema map applied to object first input parameter's child entries — plain-object map of regex-string-key → boolean-or-plain-object-subschema per Draft-7; FOURTH object-axis annotation in the M77+ object-axis cluster after M77 `additionalProperties`, M78 `minProperties`, and M79 `maxProperties`; orthogonal field semantics from M77/M78/M79 — count-floor, count-ceiling, extension-control, and regex-key-pattern-map are the four primary Draft-7 object-keyword annotations on the first input property slot, completing the cluster). Bucket-key enum (closed four-value): `typed | untyped | not_applicable | unknown`. Aggregation_key enum (closed three-value): `object_property_pattern_map_constraint | not_applicable | unknown`. Six frozen warning reason codes (5 baseline + axis-specific 6th `input_schema_properties_first_property_pattern_properties_invalid_when_present`). Bucket iteration order: `typed → untyped → not_applicable → unknown` (deterministic stable-output only — NOT a developer-artifacts-priority / SDK-type-definition-readiness-rank / API-tool-schema-precision-rank / integration-test-fuzzing-priority / eval-fixture-coverage-rank / drift-report-strictness-tier / migration-plan-priority / changelog-entry-priority / marketplace-validation-strictness-rank / local-development-fixture-coverage-rank ranking). Rules: TYPE-APPLICABILITY-OBJECT (`firstVal.type` is a string but NOT `'object'` → `not_applicable`, parallel to M77/M78/M79 — Draft-7 defines `patternProperties` ONLY for `type === 'object'`); ABSENT-AS-UNTYPED (own property absent OR `patternProperties: undefined` → `untyped` — Draft-7 default no regex map); NULL-AS-ABSENT (`patternProperties: null` → `untyped` — mirrors M55–M79); EMPTY-MAP-AS-UNTYPED (`patternProperties: {}` plain-object map with zero own enumerable keys → `untyped` — explicit empty map declares no regex-keyed sub-schemas); PLAIN-OBJECT-WITH-OWN-KEYS-AS-TYPED (`Object.prototype.toString.call(v) === '[object Object]'` AND `Object.keys(v).length >= 1` AND every value is `typeof v === 'boolean'` OR `Object.prototype.toString.call(v) === '[object Object]'` → `typed` — Draft-7 `patternProperties` is a map of regex-string-key → boolean-or-plain-object-subschema; the regex strings themselves are NOT validated as compilable regexes by the classifier — Draft-7 leaves regex compilation to the validator); DRAFT-7-PLAIN-OBJECT-MAP-IS-VALID-PATTERN-PROPERTIES (any other `firstVal.patternProperties` value: string / finite number / NaN / Infinity / -Infinity / boolean / array / non-plain-object reference type, OR plain-object map with values that are not plain-object schemas and not booleans → `unknown` WITH 6th code; NO-COERCION via `Object()` / `JSON.parse(JSON.stringify())` / `typeof v === 'object'` alone — strict `Object.prototype.toString.call(v) === '[object Object]'` checks for the outer map AND for each value AND `(typeof value === 'boolean' || Object.prototype.toString.call(value) === '[object Object]')` for each value). CLI surface 63 → 64 with new noun `partition` between `parameter` and `path` (parameter(p=112,a=97,r=114,a=97) < partition(p=112,a=97,r=114,t=116) at pos 3 (a(97)<t(116)); partition(p=112,a=97,r=114,t=116) < path(p=112,a=97,t=116,h=104) at pos 2 (r(114)<t(116))). Result-array field name: `first_property_pattern_properties_states[]`. Per-bucket field name: `input_schema_first_property_pattern_properties`. Constraint 73 (proposed) reserves M80 as "non-runtime-pattern-property-validator / non-runtime-regex-key-matcher / non-runtime-property-stripper / non-DTO-pattern-property-deserializer / non-developer-artifacts-emitter / non-strict-tool-emitter-pattern-properties-validator / non-MCP-server-input-pattern-properties-validator / non-marketplace-package-pattern-properties-validator / non-validation-layer-pattern-properties-validator / non-additional-properties-crossref / non-min-properties-crossref / non-max-properties-crossref / non-property-names-crossref / non-required-crossref / non-properties-count-crossref / non-pattern-string-crossref / non-doc-contradiction-detector / non-LLM-pattern-properties-inferrer / non-statistical-aggregator / non-regex-compiler / non-regex-validator". Distinct from M79 (maxProperties — sibling object-axis on the same firstVal but different field semantic: count-ceiling vs regex-key-pattern-map), M78 (minProperties — sibling object-axis, different field semantic: count-floor vs regex-key-pattern-map), M77 (additionalProperties — sibling object-axis, different field semantic: extension-control vs regex-key-pattern-map; the two are layered together at validation time but the static annotation is orthogonal), M59 (`pattern` — DIFFERENT FIELD SEMANTIC AND DIFFERENT TYPE-APPLICABILITY: `pattern` regex constrains string VALUES on type:'string' firstVals whereas `patternProperties` regex map constrains object KEY-NAMES-AND-VALUE-SHAPES on type:'object' firstVals), M50 (firstKey required-status — different field semantic). VISION primary citation: lines 342–356 (`### Developer Artifacts`, in § What We Output) — first milestone to use this section as primary aggregation source. The full M80 § will be materialized by the dev role's implementation phase per the M27–M79 PM-vs-dev ownership split. M80 is implementation-ready planned work, not shipped.

> **M79 Materialized — 2026-04-29, run_73dd78f172735547, turn_f703b2d7af03a903, dev attempt 1.** All PM-frozen scope carried forward verbatim. Implementation: `classifyInputSchemaFirstPropertyMaxProperties` (pure function; TYPE-APPLICABILITY-OBJECT: firstVal.type string but NOT 'object' → not_applicable — Draft-7 defines maxProperties ONLY for type==='object'; ABSENT-AS-UNBOUNDED: maxProperties absent/undefined → unbounded, no warning — Draft-7 default is no ceiling; NULL-AS-ABSENT: maxProperties:null → unbounded, no warning — mirrors M55–M78; NON-NEGATIVE-INTEGER-IS-VALID-MAX-PROPERTIES: Number.isInteger(v) && v >= 0 → bounded, no warning; PRESENT-AS-PRESENT-ZERO: maxProperties:0 → bounded, no warning — explicit zero is a valid declared ceiling; mirrors M74 maxItems:0 and M78 minProperties:0; DRAFT-7-NON-NEGATIVE-INTEGER-IS-VALID-MAX-PROPERTIES: any other value → unknown WITH 6th code; NO-COERCION via Number()/parseInt()/parseFloat()/Boolean()/!!/v?true:false — strict Number.isInteger(v) && v >= 0 only), `buildInputSchemaFirstPropertyMaxPropertiesIndex`, `formatInputSchemaFirstPropertyMaxPropertiesIndex`, `cmdCrowded`/`cmdCrowdedIndex`/`parseCrowdedIndexArgs` in `src/cli.js`; CLI surface 62→63 (`crowded` between `confidence` and `description`; confidence(c=99,o=111)<crowded(c=99,r=114) at pos 1 (o(111)<r(114)); crowded(c=99)<description(d=100) at pos 0 (c(99)<d(100))); 18-case M79 smoke matrix; eval scenario `input-schema-first-property-max-properties-index-determinism` (69→70 scenarios); Constraint 72: non-runtime-object-property-count-ceiling-validator / non-runtime-object-key-count-enforcer / non-DTO-property-count-ceiling-validator / non-evals-and-regression-strictness-aggregator / non-min-properties-crossref / non-additional-properties-crossref / non-required-crossref / non-properties-count-crossref / non-pattern-properties-crossref / non-property-names-crossref / non-doc-contradiction-detector / non-LLM-max-properties-inferrer / non-statistical-aggregator. Distinct from M78 (minProperties — sibling object-axis, mirrored numeric direction: floor vs ceiling), M77 (additionalProperties — sibling object-axis, orthogonal field semantic: extension-control vs property-count-ceiling), M74 (maxItems — array-axis sibling-class, same mathematical ceiling shape different type domain). npm test exits 0 with 70 scenarios. See IMPLEMENTATION_NOTES.md § M79 for full detail.

> **M79 Charter Sketch Reservation — 2026-04-29, run_73dd78f172735547, turn_adf81f05e30487be, PM attempt 1, HEAD prior to PM edits at `0ef65da`.** [SUPERSEDED by M79 Materialized above] Axis: `input_schema.properties[firstKey].maxProperties` (JSON-Schema Draft 7 OBJECT-PROPERTY-COUNT-CEILING annotation governing the maximum number of keys an object first input parameter MAY contain — non-negative integer per Draft-7; THIRD object-axis annotation in the M77+ object-axis cluster after M77 `additionalProperties` and M78 `minProperties`; sibling-class to M74 `maxItems` (array-cardinality-ceiling) sharing the same non-negative-integer-ceiling mathematical shape but operating on type:'object' rather than type:'array'; direct numeric mirror of M78 `minProperties` floor on the same firstVal slot — floor vs ceiling). Bucket-key enum (closed four-value): `bounded | unbounded | not_applicable | unknown`. Aggregation_key enum (closed three-value): `object_property_count_ceiling_constraint | not_applicable | unknown`. Six frozen warning reason codes (5 baseline + axis-specific 6th `input_schema_properties_first_property_max_properties_invalid_when_present`). Bucket iteration order: `bounded → unbounded → not_applicable → unknown` (deterministic stable-output only — NOT an evals-and-regression-priority / schema-drift-detection-priority / golden-task-coverage-rank / marketplace-package-validation-priority / permission-regression-check-strictness-tier / UI-surface-smoke-test-priority / workflow-test-coverage-rank / regression-fixture-strictness-rank ranking). Rules: TYPE-APPLICABILITY-OBJECT (`firstVal.type` is a string but NOT `'object'` → `not_applicable`, parallel to M77/M78 — Draft-7 defines `maxProperties` ONLY for `type === 'object'`); ABSENT-AS-UNBOUNDED (own property absent OR `maxProperties: undefined` → `unbounded` — Draft-7 default no ceiling); NULL-AS-ABSENT (`maxProperties: null` → `unbounded` — mirrors M55–M78); NON-NEGATIVE-INTEGER-IS-VALID-MAX-PROPERTIES (`Number.isInteger(v) && v >= 0` → `bounded`; the integer `0` (explicit zero ceiling — empty-object-only-permitted-with-ceiling-considered) buckets as `bounded` — PRESENT-AS-PRESENT-ZERO mirroring M74 `maxItems: 0` and M78 `minProperties: 0` precedents); DRAFT-7-NON-NEGATIVE-INTEGER-IS-VALID-MAX-PROPERTIES (any other `firstVal.maxProperties` value: non-integer finite number / negative integer / NaN / Infinity / -Infinity / string / boolean / array / object / non-plain-object reference type → `unknown` WITH 6th code; NO-COERCION via `Number()` / `parseInt()` / `parseFloat()` / `Boolean()` / `!!` / `v ? true : false` — strict `Number.isInteger(v) && v >= 0` only). CLI surface 62 → 63 with new noun `crowded` between `confidence` and `description` (confidence(c=99,o=111) < crowded(c=99,r=114) at pos 1 (o(111)<r(114)); crowded(c=99) < description(d=100) at pos 0 (c(99)<d(100))). Result-array field name: `first_property_max_properties_states[]`. Per-bucket field name: `input_schema_first_property_max_properties`. Constraint 72 (proposed) reserves M79 as "non-runtime-object-property-count-ceiling-validator / non-runtime-object-key-count-rejector / non-runtime-property-stripper / non-DTO-property-count-ceiling-validator / non-evals-and-regression-strictness-aggregator / non-strict-tool-emitter-max-properties-validator / non-MCP-server-input-max-properties-validator / non-marketplace-package-max-properties-validator / non-validation-layer-max-properties-validator / non-min-properties-crossref / non-additional-properties-crossref / non-required-crossref / non-properties-count-crossref / non-pattern-properties-crossref / non-property-names-crossref / non-doc-contradiction-detector / non-LLM-max-properties-inferrer / non-statistical-aggregator". Distinct from M78 (minProperties — sibling object-axis on the same firstVal but mirrored numeric direction: floor vs ceiling), M77 (additionalProperties — sibling object-axis on the same firstVal but ORTHOGONAL field semantic: extension-control vs property-count-ceiling), M74 (maxItems — array-axis sibling-class, same mathematical ceiling different type domain: array element-count-ceiling vs object key-count-ceiling), M50 (firstKey required-status — different field semantic). VISION primary citation: lines 519–521 (`### Evals And Regression`, in § Product Modules) — first milestone to use this section as primary aggregation source. The full M79 § will be materialized by the dev role's implementation phase per the M27–M78 PM-vs-dev ownership split. M79 is implementation-ready planned work, not shipped.

> **M78 Materialized — 2026-04-29, run_b2c5d5143ed0e344, turn_5f19567fcdd8fb6c, dev attempt 1.** All PM-frozen scope carried forward verbatim. Implementation: `classifyInputSchemaFirstPropertyMinProperties` (pure function; TYPE-APPLICABILITY-OBJECT: firstVal.type string but NOT 'object' → not_applicable — Draft-7 defines minProperties ONLY for type==='object'; ABSENT-AS-UNBOUNDED: minProperties absent/undefined → unbounded, no warning — Draft-7 default is no floor; NULL-AS-ABSENT: minProperties:null → unbounded, no warning — mirrors M55–M77; NON-NEGATIVE-INTEGER-IS-VALID-MIN-PROPERTIES: Number.isInteger(v) && v >= 0 → bounded, no warning; PRESENT-AS-PRESENT-ZERO: minProperties:0 → bounded, no warning — explicit zero is a declared floor, NOT unbounded, mirrors M73 minItems:0; DRAFT-7-NON-NEGATIVE-INTEGER-IS-VALID-MIN-PROPERTIES: any other value → unknown WITH 6th code; NO-COERCION via Number()/parseInt()/parseFloat()/Boolean()/!!/v?true:false — strict Number.isInteger(v) && v >= 0 only), `buildInputSchemaFirstPropertyMinPropertiesIndex`, `formatInputSchemaFirstPropertyMinPropertiesIndex`, `cmdSparse`/`cmdSparseIndex`/`parseSparseIndexArgs` in `src/cli.js`; CLI surface 61→62 (`sparse` between `signature` and `strictness`; signature(s=115,i=105)<sparse(s=115,p=112) at pos 1 (i(105)<p(112)); sparse(s=115,p=112)<strictness(s=115,t=116) at pos 1 (p(112)<t(116))); 18-case M78 smoke matrix; eval scenario `input-schema-first-property-min-properties-index-determinism` (68→69 scenarios); Constraint 71: non-runtime-object-property-count-floor-validator / non-runtime-object-key-count-rejector / non-runtime-property-stripper / non-DTO-property-count-floor-validator / non-governance-observability-strictness-aggregator / non-strict-tool-emitter-min-properties-validator / non-MCP-server-input-min-properties-validator / non-marketplace-package-min-properties-validator / non-validation-layer-min-properties-validator / non-additional-properties-crossref / non-required-crossref / non-properties-count-crossref / non-max-properties-crossref / non-pattern-properties-crossref / non-property-names-crossref / non-doc-contradiction-detector / non-LLM-min-properties-inferrer / non-statistical-aggregator. Distinct from M77 (additionalProperties — sibling object-axis on the same firstVal but ORTHOGONAL field semantic: extension-control vs property-count-floor), M73 (minItems — array-axis sibling-class, same mathematical floor different type domain: array element-count-floor vs object key-count-floor), M50 (firstKey required-status — different field semantic). npm test exits 0 with 69 scenarios. See IMPLEMENTATION_NOTES.md § M78 for full detail.

> **M78 Charter Sketch Reservation — 2026-04-29, run_b2c5d5143ed0e344, turn_d8b72d32481cce2f, PM attempt 1, HEAD prior to PM edits at `dfcc6aa`.** [SUPERSEDED by M78 Materialized above] Reserves a future § "Input Schema First Property MinProperties Annotation Presence Index" section for the planning-phase M78 charter. Axis: `input_schema.properties[firstKey].minProperties` (JSON-Schema Draft 7 OBJECT-PROPERTY-COUNT-FLOOR annotation governing the minimum number of keys an object first input parameter MUST contain — non-negative integer per Draft-7; SECOND object-axis annotation in the M77+ object-axis cluster after M77 `additionalProperties`; sibling-class to M73 `minItems` (array-cardinality-floor) sharing the same non-negative-integer-floor mathematical shape but operating on type:'object' rather than type:'array'). Bucket-key enum (closed four-value): `bounded | unbounded | not_applicable | unknown`. Aggregation_key enum (closed three-value): `object_property_count_floor_constraint | not_applicable | unknown`. Six frozen warning reason codes (5 baseline + axis-specific 6th `input_schema_properties_first_property_min_properties_invalid_when_present`). Bucket iteration order: `bounded → unbounded → not_applicable → unknown` (deterministic stable-output only — NOT a governance-observability-priority / approval-gate-readiness-rank / audit-log-coverage-rank / drift-detection-priority / policy-file-strictness-tier / review-queue-priority / replay-dry-run-rejection-rank / evals-regression-strictness-rank ranking). Rules: TYPE-APPLICABILITY-OBJECT (`firstVal.type` is a string but NOT `'object'` → `not_applicable`, parallel to M77 — Draft-7 defines `minProperties` ONLY for `type === 'object'`); ABSENT-AS-UNBOUNDED (own property absent OR `minProperties: undefined` → `unbounded` — Draft-7 default no floor); NULL-AS-ABSENT (`minProperties: null` → `unbounded` — mirrors M55–M77); NON-NEGATIVE-INTEGER-IS-VALID-MIN-PROPERTIES (`Number.isInteger(v) && v >= 0` → `bounded`; the integer `0` (explicit zero floor — empty-object-permitted-with-floor-considered) buckets as `bounded` — PRESENT-AS-PRESENT-ZERO mirroring M73 `minItems: 0` precedent); DRAFT-7-NON-NEGATIVE-INTEGER-IS-VALID-MIN-PROPERTIES (any other `firstVal.minProperties` value: non-integer finite number / negative integer / NaN / Infinity / -Infinity / string / boolean / array / object / non-plain-object reference type → `unknown` WITH 6th code; NO-COERCION via `Number()` / `parseInt()` / `parseFloat()` / `Boolean()` / `!!` / `v ? true : false` — strict `Number.isInteger(v) && v >= 0` only). CLI surface 61 → 62 with new noun `sparse` between `signature` and `strictness` (signature(s=115,i=105) < sparse(s=115,p=112) at pos 1 (i(105)<p(112)); sparse(s=115,p=112) < strictness(s=115,t=116) at pos 1 (p(112)<t(116))). Result-array field name: `first_property_min_properties_states[]`. Per-bucket field name: `input_schema_first_property_min_properties`. Constraint 71 (proposed) reserves M78 as "non-runtime-object-property-count-floor-validator / non-runtime-object-key-count-rejector / non-runtime-property-stripper / non-DTO-property-count-floor-validator / non-governance-observability-strictness-aggregator / non-strict-tool-emitter-min-properties-validator / non-MCP-server-input-min-properties-validator / non-marketplace-package-min-properties-validator / non-validation-layer-min-properties-validator / non-additional-properties-crossref / non-required-crossref / non-properties-count-crossref / non-max-properties-crossref / non-pattern-properties-crossref / non-property-names-crossref / non-doc-contradiction-detector / non-LLM-min-properties-inferrer / non-statistical-aggregator". Distinct from M77 (additionalProperties — sibling object-axis on the same firstVal but ORTHOGONAL field semantic: extension-control vs property-count-floor), M73 (minItems — array-axis sibling-class, same mathematical floor different type domain: array element-count-floor vs object key-count-floor), M50 (firstKey required-status — different field semantic). VISION primary citation: lines 255–266 (`### Governance And Observability`) — first milestone to use this section as primary aggregation source. The full M78 § will be materialized by the dev role's implementation phase per the M27–M77 PM-vs-dev ownership split. M78 is implementation-ready planned work, not shipped.

> **M77 Materialized — 2026-04-29, run_23513ac80f87e34e, turn_57319dfd22963098, dev attempt 1.** All PM-frozen scope carried forward verbatim. Implementation: `classifyInputSchemaFirstPropertyAdditionalProperties` (pure function; TYPE-APPLICABILITY-OBJECT: firstVal.type string but NOT 'object' → not_applicable; ABSENT-AS-OPEN: additionalProperties absent/undefined → open, no warning — Draft-7 default is open; NULL-AS-ABSENT: additionalProperties:null → open, no warning — mirrors M55–M76; BOOLEAN-FALSE-AS-CLOSED: additionalProperties===false → closed, no warning; BOOLEAN-TRUE-AS-OPEN: additionalProperties===true → open, no warning; PLAIN-OBJECT-SCHEMA-AS-OPEN: Object.prototype.toString.call(additionalProperties)==='[object Object]' → open, no warning — Draft-7 schema-form: extra keys permitted, value shape restricted, still semantically OPEN; DRAFT-7-BOOLEAN-OR-OBJECT-IS-VALID-ADDITIONAL-PROPERTIES: any other value → unknown WITH 6th code; NO-COERCION via Boolean()/!!/v?true:false/Object() — strict ===true/===false and Object.prototype.toString checks only), `buildInputSchemaFirstPropertyAdditionalPropertiesIndex`, `formatInputSchemaFirstPropertyAdditionalPropertiesIndex`, `cmdOpen`/`cmdOpenIndex`/`parseOpenIndexArgs` in `src/cli.js`; CLI surface 60→61 (`open` between `obligation` and `output`; obligation(o=111,b=98)<open(o=111,p=112) at pos 1 (b(98)<p(112)); open(o=111,p=112)<output(o=111,u=117) at pos 1 (p(112)<u(117))); 18-case M77 smoke matrix; eval scenario `input-schema-first-property-additional-properties-index-determinism` (67→68 scenarios); Constraint 70: non-runtime-object-extension-validator / non-runtime-extra-property-rejector / non-runtime-property-stripper / non-DTO-extension-validator / non-governance-rollout-strictness-aggregator / non-strict-tool-emitter-additional-properties-validator / non-MCP-server-output-additional-properties-validator / non-marketplace-package-additional-properties-validator / non-validation-layer-additional-properties-validator / non-strictness-output-crossref / non-required-crossref / non-properties-count-crossref / non-min-properties-crossref / non-max-properties-crossref / non-pattern-properties-crossref / non-property-names-crossref / non-doc-contradiction-detector / non-LLM-additional-properties-inferrer / non-statistical-aggregator. Distinct from M40 (output schema top-level additionalProperties — SIBLING input-vs-output axis pair), M50 (firstKey required-status — different field semantic), M76 (firstKey items array-element-subschema — array-axis vs object-axis). npm test exits 0 with 68 scenarios. See IMPLEMENTATION_NOTES.md § M77 for full detail.

> **M77 Charter Sketch Reservation — 2026-04-28, run_23513ac80f87e34e, turn_75173aa324cb0486, PM attempt 1, HEAD prior to PM edits at `a937d8a`.** [SUPERSEDED by M77 Materialized above] Reserves a future § "Input Schema First Property AdditionalProperties Annotation Presence Index" section for the planning-phase M77 charter. Axis: `input_schema.properties[firstKey].additionalProperties` (JSON-Schema Draft 7 OBJECT-EXTENSION-CONTROL annotation governing closed-vs-open object semantics — `false` strictly closes the object to extra keys; `true` / absent / null permits extra keys (Draft-7 default OPEN); plain-object schema permits extra keys with per-key value-shape constraint (still semantically OPEN); first OBJECT-axis annotation in the M77+ object-axis cluster, parallel to the M73–M76 array-axis cluster which closed at M76). Bucket-key enum (closed four-value): `closed | open | not_applicable | unknown`. Aggregation_key enum (closed three-value): `object_property_extension_constraint | not_applicable | unknown`. Six frozen warning reason codes (5 baseline + axis-specific 6th `input_schema_properties_first_property_additional_properties_invalid_when_present`). Bucket iteration order: `closed → open → not_applicable → unknown` (deterministic stable-output only — NOT a governance-rollout-priority / strictness-readiness-rank / MCP-server-strictness-tier / marketplace-package-strictness-rank / DTO-extension-constraint-priority / runtime-payload-shape-tolerance / object-extra-key-shape-significance / validation-layer-strictness-tier ranking). Rules: TYPE-APPLICABILITY-OBJECT (`firstVal.type` is a string but NOT `'object'` → `not_applicable`, parallel to M73/M74/M75/M76 TYPE-APPLICABILITY-ARRAY); ABSENT-AS-OPEN (own property absent OR `additionalProperties: undefined` → `open` — Draft-7 default `additionalProperties: true`); NULL-AS-ABSENT (`additionalProperties: null` → `open`, mirrors M55–M76 null-as-absent + Draft-7 OPEN default); BOOLEAN-FALSE-AS-CLOSED (`additionalProperties === false` → `closed`); BOOLEAN-TRUE-AS-OPEN (`additionalProperties === true` → `open`); PLAIN-OBJECT-SCHEMA-AS-OPEN (`Object.prototype.toString.call(additionalProperties) === '[object Object]'` → `open` — Draft-7 schema-form: extra keys permitted with value-shape restriction, still semantically OPEN); DRAFT-7-BOOLEAN-OR-OBJECT-IS-VALID-ADDITIONAL-PROPERTIES (any other value: string / number / Array / NaN / Infinity / non-plain-object reference → `unknown` WITH 6th code; NO-COERCION via `Boolean()` / `!!` / `v ? true : false` / `Object()` — strict `=== true` / `=== false` and `Object.prototype.toString` checks only). CLI surface 60 → 61 with new noun `open` between `obligation` and `output` (obligation(o=111,b=98)<open(o=111,p=112) at pos 1 (b(98)<p(112)); open(o=111,p=112)<output(o=111,u=117) at pos 1 (p(112)<u(117))). Result-array field name: `first_property_additional_properties_states[]`. Per-bucket field name: `input_schema_first_property_additional_properties`. Constraint 70 (proposed) reserves M77 as "non-runtime-object-extension-validator / non-runtime-extra-property-rejector / non-runtime-property-stripper / non-DTO-extension-validator / non-governance-rollout-strictness-aggregator / non-strict-tool-emitter-additional-properties-validator / non-MCP-server-output-additional-properties-validator / non-marketplace-package-additional-properties-validator / non-validation-layer-additional-properties-validator / non-strictness-output-crossref / non-required-crossref / non-properties-count-crossref / non-min-properties-crossref / non-max-properties-crossref / non-pattern-properties-crossref / non-property-names-crossref / non-doc-contradiction-detector / non-LLM-additional-properties-inferrer / non-statistical-aggregator". VISION primary citation: line 523 (`### Governance And Rollout`) — first milestone to use this section as primary. The full M77 § will be materialized by the dev role's implementation phase per the M27–M76 PM-vs-dev ownership split. M77 is implementation-ready planned work, not shipped.

> **M76 Materialized — 2026-04-28, run_7614b689bb195a71, turn_81676e2d9987b70c, dev attempt 1.** All PM-frozen scope carried forward verbatim. Implementation: `classifyInputSchemaFirstPropertyItems` (pure function; TYPE-APPLICABILITY-ARRAY: firstVal.type string but NOT 'array' → not_applicable; ABSENT-AS-UNDECLARED: items absent/undefined → undeclared, no warning — Draft-7 default is no-element-shape-constraint; NULL-AS-ABSENT: items:null → undeclared, no warning — mirrors M55–M75; PLAIN-OBJECT-AS-DECLARED: Object.prototype.toString.call(items)==='[object Object]' → declared, no warning — Draft-7 primary form; ARRAY-OF-OBJECTS-AS-DECLARED: Array.isArray(items) && every element is a plain object → declared, no warning — Draft-7 tuple form including empty []; DRAFT-7-OBJECT-OR-OBJECT-ARRAY-IS-VALID-ITEMS: any other value → unknown WITH 6th code; NO-COERCION via Object()/Array.from()/JSON.parse(JSON.stringify())/typeof v==='object' alone — strict Object.prototype.toString and Array.isArray+every checks only), `buildInputSchemaFirstPropertyItemsIndex`, `formatInputSchemaFirstPropertyItemsIndex`, `cmdElement`/`cmdElementIndex`/`parseElementIndexArgs` in `src/cli.js`; CLI surface 59→60 (`element` between `effect` and `examples`; effect(e=101,f=102)<element(e=101,l=108) at pos 1 (f(102)<l(108)); element(e=101,l=108)<examples(e=101,x=120) at pos 1 (l(108)<x(120))); 18-case M76 smoke matrix; eval scenario `input-schema-first-property-items-index-determinism` (66→67 scenarios); Constraint 69: non-runtime-array-element-shape-validator / non-runtime-element-coercer / non-tuple-form-validator / non-surface-generator-widget-emitter / non-array-input-control-renderer / non-strict-tool-element-emitter / non-DTO-element-deserializer / non-MCP-server-output-element-validator / non-marketplace-package-element-validator / non-validation-layer-element-shape-validator / non-uniqueItems-crossref / non-minItems-crossref / non-maxItems-crossref / non-required-crossref / non-default-crossref / non-const-crossref / non-doc-contradiction-detector / non-LLM-items-inferrer / non-statistical-aggregator. npm test exits 0 with 67 scenarios. See IMPLEMENTATION_NOTES.md § M76 for full detail.

> **M76 Charter Sketch Reservation — 2026-04-28, run_7614b689bb195a71, turn_9bdd363921e8d82e, PM attempt 1, HEAD prior to PM edits at `d9b0f7c`.** [SUPERSEDED by M76 Materialized above] Reserves a future § "Input Schema First Property Items Annotation Presence Index" section for the planning-phase M76 charter. Axis: `input_schema.properties[firstKey].items` (JSON-Schema Draft 7 ARRAY-ELEMENT-SUBSCHEMA annotation governing per-element shape — Draft-7 permits primary form `items: { ...schema }` (single schema applied to every element) and tuple form `items: [schema_pos_0, ...]` (ordered per-position schemas); sibling array-axis to M73's `minItems` (array-cardinality-floor), M74's `maxItems` (array-cardinality-ceiling), and M75's `uniqueItems` (array-element-pairwise-distinctness boolean), but ORTHOGONAL — `minItems`/`maxItems` count cardinality, `uniqueItems` constrains pairwise-distinctness, `items` declares per-element shape regardless of cardinality and regardless of distinctness). Bucket-key enum (closed four-value): `declared | undeclared | not_applicable | unknown`. Aggregation_key enum (closed three-value): `array_element_subschema_constraint | not_applicable | unknown`. Six frozen warning reason codes (5 baseline + axis-specific 6th `input_schema_properties_first_property_items_invalid_when_present`). Bucket iteration order: `declared → undeclared → not_applicable → unknown` (deterministic stable-output only — NOT a surface-generator-priority / widget-rendering-readiness-rank / command-palette-form-completeness-tier / action-widget-input-control-tier / DTO-element-deserialization-priority / runtime-payload-shape-tolerance / array-element-shape-significance / MCP-server-output-strictness ranking). Rules: NULL-AS-ABSENT (`firstVal.items === null` → `undeclared`, mirrors M55–M75 null-as-absent precedent); ABSENT-AS-UNDECLARED (own property absent OR `firstVal.items === undefined` → `undeclared` — Draft-7 default no-element-shape-constraint); TYPE-APPLICABILITY-ARRAY (`firstVal.type` is a string but NOT `'array'` → `not_applicable`, mirrors M73/M74/M75 TYPE-APPLICABILITY-ARRAY); PLAIN-OBJECT-AS-DECLARED (`Object.prototype.toString.call(firstVal.items) === '[object Object]'` → `declared` — Draft-7 primary form); ARRAY-OF-OBJECTS-AS-DECLARED (`Array.isArray(firstVal.items) && every element is a plain object` → `declared` — Draft-7 tuple form, including empty `[]` zero-element tuple); DRAFT-7-OBJECT-OR-OBJECT-ARRAY-IS-VALID-ITEMS (any other shape: string / number / boolean / NaN / Infinity / non-plain-object reference type / mixed-element tuple → `unknown` WITH 6th code; NO-COERCION — strict `Object.prototype.toString` and `Array.isArray + every plain-object` checks only). CLI surface 59 → 60 with new noun `element` between `effect` and `examples` (effect(e=101,f=102) < element(e=101,l=108) at pos 1 (f(102)<l(108)); element(e=101,l=108) < examples(e=101,x=120) at pos 1 (l(108)<x(120))). Result-array field name: `first_property_items_states[]`. Per-bucket field name: `input_schema_first_property_items`. Constraint 69 (proposed) reserves M76 as "non-runtime-array-element-shape-validator / non-runtime-element-coercer / non-tuple-form-validator / non-surface-generator-widget-emitter / non-array-input-control-renderer / non-strict-tool-element-emitter / non-DTO-element-deserializer / non-MCP-server-output-element-validator / non-marketplace-package-element-validator / non-validation-layer-element-shape-validator / non-uniqueItems-crossref / non-minItems-crossref / non-maxItems-crossref / non-required-crossref / non-default-crossref / non-const-crossref / non-doc-contradiction-detector / non-LLM-items-inferrer / non-statistical-aggregator". VISION primary citation: lines 507–508 (`### Surface Generator`) — first milestone to use this section as primary. The full M76 § will be materialized by the dev role's implementation phase per the M27–M75 PM-vs-dev ownership split. M76 is implementation-ready planned work, not shipped.

> **M75 Materialized — 2026-04-28, run_ed87531287b641c8, turn_68fba981954c38ba, dev attempt 1.** All PM-frozen scope carried forward verbatim. Implementation: `classifyInputSchemaFirstPropertyUniqueItems` (pure function; TYPE-APPLICABILITY-ARRAY: firstVal.type string but NOT 'array' → not_applicable; ABSENT-AS-NOT-UNIQUE: uniqueItems absent/undefined → not_unique, no warning; NULL-AS-ABSENT: uniqueItems:null → not_unique, no warning — mirrors M55–M74; BOOLEAN-FALSE-AS-NOT-UNIQUE: uniqueItems:false → not_unique — explicit-false agrees with absence-default, mirrors M72 BOOLEAN-FALSE-AS-NOT-NULLABLE; BOOLEAN-TRUE-AS-UNIQUE: uniqueItems:true → unique; DRAFT-7-BOOLEAN-IS-VALID-UNIQUE-ITEMS: non-boolean non-null present value → unknown WITH 6th code; NO-COERCION via Boolean()/!!/v?true:false — strict === true / === false only), `buildInputSchemaFirstPropertyUniqueItemsIndex`, `formatInputSchemaFirstPropertyUniqueItemsIndex`, `cmdUnique`/`cmdUniqueIndex`/`parseUniqueIndexArgs` in `src/cli.js`; CLI surface 58→59 (`unique` between `surface` and `upper`; surface(s=115,u=117)<unique(u=117) at pos 0; unique(u=117,n=110)<upper(u=117,p=112) at pos 1); 18-case M75 smoke matrix; eval scenario `input-schema-first-property-unique-items-index-determinism` (65→66 scenarios); Constraint 68: non-runtime-array-element-uniqueness-validator / non-runtime-array-deduplicator / non-DTO-deduplication-validator / non-tool-and-skill-compiler-strictness-aggregator / non-strict-tool-emitter / non-MCP-server-output-distinctness-aggregator / non-marketplace-package-distinctness-validator / non-validation-layer-array-distinctness-validator / non-required-crossref / non-default-crossref / non-const-crossref / non-minItems-crossref / non-maxItems-crossref / non-items-schema-crossref / non-doc-contradiction-detector / non-LLM-uniqueItems-inferrer / non-statistical-aggregator. npm test exits 0 with 66 scenarios. See IMPLEMENTATION_NOTES.md § M75 for full detail.

> **M75 Charter Sketch Reservation — 2026-04-28, run_ed87531287b641c8, turn_9016136acd7fe256, PM attempt 1, HEAD prior to PM edits at `a25a272`.** [SUPERSEDED by M75 Materialized above] Reserves a future § "Input Schema First Property UniqueItems Annotation Presence Index" section for the planning-phase M75 charter. Axis: `input_schema.properties[firstKey].uniqueItems` (JSON-Schema Draft 7 BOOLEAN annotation governing array-element-pairwise-distinctness — sibling-class boolean axis to M72's `nullable` (same boolean class on a different field semantics: `nullable` is null-permission, `uniqueItems` is array-element-distinctness); sibling array-axis to M73's `minItems` (array-cardinality-floor) and M74's `maxItems` (array-cardinality-ceiling), but ORTHOGONAL — `minItems`/`maxItems` count cardinality, `uniqueItems` constrains pairwise-distinctness regardless of cardinality). Bucket-key enum (closed four-value): `unique | not_unique | not_applicable | unknown`. Aggregation_key enum (closed three-value): `array_element_uniqueness_constraint | not_applicable | unknown`. Six frozen warning reason codes (5 baseline + axis-specific 6th `input_schema_properties_first_property_unique_items_invalid_when_present`). Bucket iteration order: `unique → not_unique → not_applicable → unknown` (deterministic stable-output only — NOT a tool-and-skill-compiler-priority / strict-tool-emitter-readiness / DTO-distinctness-priority / runtime-payload-shape-tolerance / array-element-uniqueness-significance ranking). Rules: NULL-AS-ABSENT (`uniqueItems: null` → `not_unique`, mirrors M55–M74 null-as-absent precedent); ABSENT-AS-NOT-UNIQUE (own property absent OR `uniqueItems: undefined` → `not_unique` — JSON-Schema-Draft-7 default for `uniqueItems` is `false`); TYPE-APPLICABILITY-ARRAY (`firstVal.type` is a string but NOT `'array'` → `not_applicable`, mirrors M73/M74 TYPE-APPLICABILITY-ARRAY); BOOLEAN-FALSE-AS-NOT-UNIQUE (`firstVal.uniqueItems === false` → `not_unique` — explicit-false semantically agrees with absence-default, mirrors M72 BOOLEAN-FALSE-AS-NOT-NULLABLE); BOOLEAN-TRUE-AS-UNIQUE (`firstVal.uniqueItems === true` → `unique`); DRAFT-7-BOOLEAN-IS-VALID-UNIQUE-ITEMS (any non-boolean: number / string / array / object / NaN / Infinity / etc. → `unknown` WITH 6th code; NO-COERCION via `Boolean()` / `!!` / `v ? true : false` — strict `=== true` and `=== false` only). CLI surface 58 → 59 with new noun `unique` between `surface` and `upper` (surface(s=115,u=117)<unique(u=117) at pos 0 (s(115)<u(117)); unique(u=117,n=110)<upper(u=117,p=112) at pos 1 (n(110)<p(112))). Result-array field name: `first_property_unique_items_states[]`. Per-bucket field name: `input_schema_first_property_unique_items`. Constraint 68 (proposed) reserves M75 as "non-runtime-array-element-uniqueness-validator / non-runtime-array-deduplicator / non-DTO-deduplication-validator / non-tool-and-skill-compiler-strictness-aggregator / non-strict-tool-emitter / non-MCP-server-output-distinctness-aggregator / non-marketplace-package-distinctness-validator / non-validation-layer-array-distinctness-validator / non-required-crossref / non-default-crossref / non-const-crossref / non-minItems-crossref / non-maxItems-crossref / non-items-schema-crossref / non-doc-contradiction-detector / non-LLM-uniqueItems-inferrer / non-statistical-aggregator". VISION primary citation: lines 503–504 (`### Tool And Skill Compiler`) — first milestone to use this section as primary. The full M75 § will be materialized by the dev role's implementation phase per the M27–M74 PM-vs-dev ownership split. M75 is implementation-ready planned work, not shipped.

> **M74 Materialized — 2026-04-28, run_87db4ac59785126d, turn_c84e347e780b8eb0, dev attempt 1.** All PM-frozen scope carried forward verbatim. Implementation: `classifyInputSchemaFirstPropertyMaxItems` (pure function; TYPE-APPLICABILITY-ARRAY: firstVal.type string but NOT 'array' → not_applicable; ABSENT-AS-UNBOUNDED: maxItems absent/undefined → unbounded, no warning; NULL-AS-ABSENT: maxItems:null → unbounded, no warning — mirrors M55–M73; NON-NEGATIVE-INTEGER-IS-VALID-MAX-ITEMS: Number.isInteger(v) && v>=0 → bounded, no warning; PRESENT-AS-PRESENT-ZERO: maxItems:0 → bounded — explicit-zero (empty-array-only ceiling) semantically distinct from absent; DRAFT-7-NON-NEGATIVE-INTEGER-IS-VALID: non-integer/negative/non-finite/NaN/Infinity/-Infinity/string/boolean/array/object → unknown WITH 6th code; NO-COERCION via Number()/parseInt()/parseFloat()), `buildInputSchemaFirstPropertyMaxItemsIndex`, `formatInputSchemaFirstPropertyMaxItemsIndex`, `cmdMost`/`cmdMostIndex`/`parseMostIndexArgs` in `src/cli.js`; CLI surface 57→58 (`most` between `mime` and `nullable`; mime(m=109,i=105)<most(m=109,o=111) at pos 1; most(m=109)<nullable(n=110) at pos 0); 18-case M74 smoke matrix; eval scenario `input-schema-first-property-max-items-index-determinism` (64→65 scenarios); Constraint 67: non-runtime-array-cardinality-ceiling-validator / non-runtime-array-element-counter / non-DTO-array-length-validator / non-capability-compiler-strictness-aggregator / non-tool-skill-compilation-eligibility-validator / non-MCP-server-output-strictness-aggregator / non-marketplace-package-array-shape-validator / non-validation-layer-array-shape-validator / non-required-crossref / non-default-crossref / non-const-crossref / non-minItems-crossref / non-uniqueItems-crossref / non-items-schema-crossref / non-doc-contradiction-detector / non-LLM-max-items-inferrer / non-statistical-aggregator. npm test exits 0 with 65 scenarios. See IMPLEMENTATION_NOTES.md § M74 for full detail.

> **M74 Charter Sketch Reservation — 2026-04-28, run_87db4ac59785126d, turn_2843e5aa6389c008, PM attempt 1, HEAD prior to PM edits at `7b92ac5`.** [SUPERSEDED by M74 Materialized above] Reserves a future § "Input Schema First Property MaxItems Annotation Presence Index" section for the planning-phase M74 charter. Axis: `input_schema.properties[firstKey].maxItems` (JSON-Schema Draft 7 non-negative-integer annotation governing array maximum-element-count — sibling-paired-ceiling axis to M73's `minItems` array-cardinality-floor; same array-element-count axis cluster, opposite direction). Bucket-key enum (closed four-value): `bounded | unbounded | not_applicable | unknown`. Aggregation_key enum (closed three-value): `array_cardinality_ceiling_constraint | not_applicable | unknown`. Six frozen warning reason codes (5 baseline + axis-specific 6th `input_schema_properties_first_property_max_items_invalid_when_present`). Bucket iteration order: `bounded → unbounded → not_applicable → unknown` (deterministic stable-output only). Rules: NULL-AS-ABSENT (`maxItems: null` → `unbounded`); ABSENT-AS-UNBOUNDED (own property absent OR `maxItems: undefined` → `unbounded` — JSON-Schema-Draft-7 default is "no ceiling"); TYPE-APPLICABILITY-ARRAY (`firstVal.type` is a string but NOT `'array'` → `not_applicable`, mirrors M73 TYPE-APPLICABILITY-ARRAY for `minItems`); NON-NEGATIVE-INTEGER-IS-VALID-MAX-ITEMS (`Number.isInteger(v) && v >= 0` → `bounded`); PRESENT-AS-PRESENT-ZERO (`maxItems: 0` → `bounded` — explicit-zero is empty-array-only ceiling, semantically distinct from absent; mirrors M73 PRESENT-AS-PRESENT-ZERO for `minItems: 0`); DRAFT-7-NON-NEGATIVE-INTEGER-IS-VALID (non-integer / negative / non-finite / NaN / Infinity / string / boolean / array / object → `unknown` WITH 6th code; NO-COERCION via `Number()` / `parseInt()` / `parseFloat()`). CLI surface 57 → 58 with new noun `most` between `mime` and `nullable` (mime(m=109,i=105)<most(m=109,o=111) at pos 1 (i(105)<o(111)); most(m=109)<nullable(n=110) at pos 0 (m(109)<n(110))). Result-array field name: `first_property_max_items_states[]`. Per-bucket field name: `input_schema_first_property_max_items`. Constraint 67 (proposed) reserves M74 as "non-runtime-array-cardinality-ceiling-validator / non-runtime-array-element-counter / non-DTO-array-length-validator / non-capability-compiler-strictness-aggregator / non-tool-skill-compilation-eligibility-validator / non-MCP-server-output-strictness-aggregator / non-marketplace-package-array-shape-validator / non-validation-layer-array-shape-validator / non-required-crossref / non-default-crossref / non-const-crossref / non-minItems-crossref / non-uniqueItems-crossref / non-items-schema-crossref / non-doc-contradiction-detector / non-LLM-max-items-inferrer / non-statistical-aggregator". VISION primary citation: lines 499–501 (`### Capability Compiler`) — first milestone to use this section as primary. The full M74 § will be materialized by the dev role's implementation phase per the M27–M73 PM-vs-dev ownership split. M74 is implementation-ready planned work, not shipped.

> **M73 Materialized — 2026-04-28, run_8059727c0a95f709, turn_f038ff1cc17f467d, dev attempt 1.** All PM-frozen scope carried forward verbatim. Implementation: `classifyInputSchemaFirstPropertyMinItems` (pure function; TYPE-APPLICABILITY-ARRAY: firstVal.type string but NOT 'array' → not_applicable; ABSENT-AS-UNBOUNDED: minItems absent/undefined → unbounded, no warning; NULL-AS-ABSENT: minItems:null → unbounded, no warning — mirrors M55–M72; NON-NEGATIVE-INTEGER-IS-VALID-MIN-ITEMS: Number.isInteger(v) && v>=0 → bounded, no warning; PRESENT-AS-PRESENT-ZERO: minItems:0 → bounded — explicit-zero semantically distinct from absent; DRAFT-7-NON-NEGATIVE-INTEGER-IS-VALID: non-integer/negative/non-finite/NaN/Infinity/-Infinity/string/boolean/array/object → unknown WITH 6th code; NO-COERCION via Number()/parseInt()/parseFloat()), `buildInputSchemaFirstPropertyMinItemsIndex`, `formatInputSchemaFirstPropertyMinItemsIndex`, `cmdLeast`/`cmdLeastIndex`/`parseLeastIndexArgs` in `src/cli.js`; CLI surface 56→57 (`least` between `items` and `legacy`; items(i=105)<least(l=108) at pos 0; least(l=108,e=101,a=97)<legacy(l=108,e=101,g=103) at pos 2); 18-case M73 smoke matrix; eval scenario `input-schema-first-property-min-items-index-determinism` (63→64 scenarios); Constraint 66: non-runtime-array-cardinality-floor-validator / non-runtime-array-element-counter / non-DTO-array-length-validator / non-static-understanding-coverage-tier-aggregator / non-route-extraction-quality-validator / non-framework-detection-strictness-aggregator / non-validation-layer-array-shape-validator / non-required-crossref / non-default-crossref / non-const-crossref / non-maxItems-crossref / non-uniqueItems-crossref / non-items-schema-crossref / non-doc-contradiction-detector / non-LLM-min-items-inferrer / non-statistical-aggregator. npm test exits 0 with 64 scenarios. See IMPLEMENTATION_NOTES.md § M73 for full detail.
