...
- [ ] Add a new CLI noun `redaction` with exactly one subcommand, `tusq redaction review`, that reads the local `tusq.manifest.json` and emits a deterministic, per-capability reviewer-facing summary of the M25 `redaction.pii_fields` and M26 `redaction.pii_categories` arrays alongside a frozen per-category advisory text
- [ ] Default `--manifest` path is `tusq.manifest.json` in the current working directory; `--manifest <path>` overrides; a missing manifest file exits 1 with `Manifest not found:` followed by the resolved path before any output is written
- [ ] A malformed manifest (JSON parse error, missing top-level `capabilities` array) exits 1 with an actionable error message and does NOT emit a partial review report
- [ ] Add a `PII_REVIEW_ADVISORY_BY_CATEGORY` frozen lookup object in `src/cli.js` mapping every one of the nine M26 category keys (`email`, `phone`, `government_id`, `name`, `address`, `date_of_birth`, `payment`, `secrets`, `network`) to exactly one single-line reviewer-advisory string; the mapping is total and disjoint; any addition or wording change is a material governance event that MUST land under its own ROADMAP milestone
- [ ] Implement a pure helper function (e.g., `buildRedactionReviewReport(manifest, options)`) that accepts the parsed manifest and a filter option bag and returns a deterministic in-memory report structure; iteration order is capabilities in the manifest's declared `capabilities` array order, and within each capability `pii_fields`/`pii_categories` arrays in their existing parallel order; zero hashing of manifest content; zero side effects
- [ ] Human-readable default output: one section per capability listing `name`, `approved` status, `sensitivity_class` (unchanged from manifest), a `Redaction:` sub-block listing `pii_fields`, `pii_categories`, and the mapped frozen advisory string for each category (once per distinct category per capability, in category-appearance order); capabilities with `pii_fields: []` display a single-line "No canonical PII field-name matches — reviewer action: none required from M27" entry; the output is plain text only (no ANSI escapes) so operators piping to file get byte-identical text
- [ ] `--json` output: emits `{manifest_path, manifest_version, generated_at, capabilities: [{name, approved, sensitivity_class, pii_fields, pii_categories, advisories: [{category, text}]}]}`; advisories within a capability appear once per distinct category in category-appearance order; repeated runs produce byte-identical JSON (no wall-clock timestamps inside per-capability entries; `generated_at` is a single top-level ISO-8601 UTC field reused from the manifest's own `generated_at`, not a fresh re-stamp, so the output is content-deterministic for the same manifest input)
- [ ] `--capability <name>` filters the report to a single capability; an unknown name exits 1 with `Capability not found:` followed by the name; the filter MUST NOT partial-match; zero output precedes the error
- [ ] `--help` prints the M27 command shape, flag list, exit codes, and an explicit "This is a reviewer aid, not a runtime enforcement gate" callout; help text is byte-identical to the `tusq redaction review --help` output across runs
- [ ] Keep the command fully local/offline — no network I/O, no TCP bind, no MCP handshake, no target-product call, no manifest write, no `src/` scan, no capability execution; the command reads only the manifest file path and writes only to stdout/stderr
