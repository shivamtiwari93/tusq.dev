...
- [ ] Extend manifest generation in `src/cli.js` so that every generated capability populates `capability.redaction.pii_categories` by labeling each entry of the M25 `pii_fields` array with its canonical PII category; the array has the same length and same order as `pii_fields`; no value inspection, no new source-text scanning, no new dependency
- [ ] Add a `PII_CATEGORY_BY_NAME` frozen lookup object in `src/cli.js` mapping every normalized entry in the M25 `PII_CANONICAL_NAMES` set to exactly one of nine category keys: `"email"`, `"phone"`, `"government_id"`, `"name"`, `"address"`, `"date_of_birth"`, `"payment"`, `"secrets"`, `"network"` — the mapping is total and disjoint; M25 and M26 freeze together
- [ ] Implement a pure helper function (e.g., `extractPiiFieldCategories(piiFields)`) that accepts the M25-emitted `redaction.pii_fields` array and returns a parallel categories array; iteration is in `pii_fields` order; lookup uses the same normalization rule as M25 (`toLowerCase()` + strip `_`/`-`); throws synchronously if a `pii_fields` entry does not map to a known category (defensive guard against M25/M26 set drift)
- [ ] Populate `capability.redaction.pii_categories` with the extractor's return value on every capability; when `pii_fields` is empty, `pii_categories` MUST be `[]` (never absent); other `redaction` fields (`log_level`, `mask_in_traces`, `retention_days`) MUST remain `null` in V1.7 — no auto-population under M26
- [ ] `capability.sensitivity_class` MUST NOT be auto-escalated under M26; Constraint 16 propagates verbatim (stays `"unknown"` until an explicit V2 increment)
- [ ] `scoreConfidence()` MUST NOT be modified — category presence is a redaction signal, not a confidence signal
- [ ] `extractPiiFieldCategories` applies uniformly to every capability regardless of source framework (Express, Fastify, NestJS) — the extractor runs wherever `extractPiiFieldHints` already runs
- [ ] Scanner and manifest generation remain fully local/offline — no network I/O, no retention-policy file fetch, no library import for category or retention inference (no `pii-detector`, no `presidio`, no compliance library); the extractor is a pure in-memory function over M25's `pii_fields` array plus the frozen lookup
- [ ] Deterministic output: `pii_categories` order equals `pii_fields` order (source-literal declaration order from M15/M24/M25); repeated runs MUST produce byte-identical arrays
- [ ] Every capability sees a one-time `capability_digest` flip on the first post-M26 scan (both non-PII capabilities that gain `pii_categories: []` and PII capabilities that gain a populated array); M13 approval-gate semantics propagate verbatim — reviewers MUST re-approve affected capabilities via `tusq approve` for them to re-appear in `tools/list`; RELEASE_NOTES MUST document the broad re-approval implication before M26 is consumed downstream
- [ ] Add smoke coverage for: (a) Fastify fixture route with `pii_fields: ["email", "password"]` produces `pii_categories: ["email", "secrets"]` in the same order; (b) case/underscore/hyphen variants of a category-triggering name produce the correct category (`user_email` → `"email"`, `USER_EMAIL` → `"email"`); (c) capability with `pii_fields: []` produces `pii_categories: []` (array, never absent); (d) mixed-category fixture (e.g., `email` + `ssn` + `credit_card`) produces `pii_categories: ["email", "government_id", "payment"]`; (e) Express and NestJS fixtures emit `pii_categories` alongside `pii_fields` with matching length and order; (f) repeated manifest generations produce byte-identical `pii_categories` ordering; (g) `sensitivity_class` stays `"unknown"` on every capability; (h) `retention_days`, `log_level`, `mask_in_traces` all stay `null` on every capability; (i) `tusq policy verify --strict` is unchanged by M26
- [ ] Add one eval regression scenario (`pii-category-label-determinism`) asserting that a fixture exercising multiple PII categories produces the expected `pii_categories` array in declaration order and that the scenario is deterministic across three repeated runs; total eval scenarios becomes **9**
