...
- An optional dry-run example for reviewers who want argument validation and a plan preview without adding live execution: scaffold the policy in one step with `tusq policy init --mode dry-run --reviewer you@example.com`, then start `tusq serve --policy .tusq/execution-policy.json`; subsequent `tools/call` responses return `executed: false`, a policy echo, and a `dry_run_plan` object with a deterministic `plan_hash`. Reviewers who want to preview the policy without writing it can add `--dry-run`; reviewers who want to keep the runtime surface unchanged can omit `--mode` entirely and accept the safe default `mode: "describe-only"`
