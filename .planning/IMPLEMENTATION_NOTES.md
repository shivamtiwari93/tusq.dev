...
**Implementation:**
- Added `INPUT_SCHEMA_FIRST_PROPERTY_MINIMUM_ENUM` (frozen Set: lower_bounded/lower_unbounded/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_MINIMUM_AGGREGATION_KEY_ENUM` (frozen Set: numeric_lower_bound_constraint/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_MINIMUM_BUCKET_ORDER` (frozen array: lower_bounded/lower_unbounded/not_applicable) constants in `src/cli.js` after M64 multipleOf constants.
- Added guard functions `_guardInputSchemaFirstPropertyMinimumBucketKey` and `_guardInputSchemaFirstPropertyMinimumAggregationKey`.
- Added `classifyInputSchemaFirstPropertyMinimum(inputSchema)` pure function implementing: unknown for null/undefined/non-object inputSchema; unknown for missing/non-string type; not_applicable for type!=='object' string; unknown for missing/null/non-object properties (when type==='object'); not_applicable for zero-property object; unknown for non-plain-object firstVal (5th frozen code: `input_schema_properties_first_property_descriptor_invalid`); lower_unbounded for absent/undefined/null minimum (null-as-absent per M55–M64, no warning); lower_bounded for `typeof v === 'number' && Number.isFinite(v)` (ZERO-IS-VALID-LOWER-BOUND: 0→lower_bounded; NEGATIVE-IS-VALID-LOWER-BOUND: -273.15→lower_bounded; FRACTIONAL-IS-VALID-LOWER-BOUND: 0.5→lower_bounded; no warning for valid values); unknown for present non-null non-finite-number minimum (6th frozen code: `input_schema_properties_first_property_minimum_invalid_when_present` — covers NaN, Infinity, -Infinity, string/'0', boolean/true, array/[0], plain object/{}). STRICT `typeof === 'number' && Number.isFinite()` check — NO Number()/parseFloat()/truthy coercion.
- Added `buildInputSchemaFirstPropertyMinimumIndex(manifest, manifestPath)`, `formatInputSchemaFirstPropertyMinimumIndex(index)`, `cmdLower(args)`, `cmdLowerIndex(args)`, `parseLowerIndexArgs(args)` functions.
- Wired `'lower'` case between `'legacy'` and `'method'` in dispatch table.
- Added `'lower'` line between `'legacy'` and `'method'` in `printHelp()` with text `Index capabilities by input schema first property minimum numeric-lower-bound annotation presence for planning review`.
- Added `'lower'` and `'lower index'` entries in `printCommandHelp()`.
- CLI surface: 48→49.

**Tests:**
- Updated all 30 `!== 48` help-count assertions to `!== 49` in `tests/smoke.mjs`.
- Added 24-case M65 smoke matrix (a-x4) with synthetic fixture covering lower_bounded_cap (minimum:1), zero_bounded_cap (minimum:0 — ZERO-IS-VALID-LOWER-BOUND), unbounded_cap (no minimum field), not_applicable_cap (type:'array'). Covers: bucket order (lower_bounded→lower_unbounded→not_applicable), filter flags, case-sensitivity (`--lower LOWER_BOUNDED` → exit 1), null-as-absent (minimum:null→lower_unbounded, no warning), ZERO-IS-VALID-LOWER-BOUND (minimum:0→lower_bounded, no warning), NEGATIVE-IS-VALID-LOWER-BOUND (minimum:-273.15→lower_bounded, no warning), FRACTIONAL-IS-VALID-LOWER-BOUND (minimum:0.5→lower_bounded, no warning), string malformation (minimum:'0'→unknown+6th code), boolean malformation (minimum:true→unknown+6th code), all 6 warning reason codes, aggregation_key closed enum (numeric_lower_bound_constraint/not_applicable/unknown), non-persistence, help enumeration and ordering (lower between legacy and method).
- Added `input-schema-first-property-minimum-index-determinism` eval scenario to `tests/evals/governed-cli-scenarios.json` (55→56 scenarios) with 8 synthetic capabilities covering all four buckets (including ZERO-IS-VALID-LOWER-BOUND zero_bounded_cap, NEGATIVE-IS-VALID-LOWER-BOUND neg_bounded_cap, null-as-absent null_minimum_cap, insertion-order verification keys={z,a,b}/firstKey=z has minimum=5).
- Added `runInputSchemaFirstPropertyMinimumIndexDeterminismScenario` handler in `tests/eval-regression.mjs`.

**Verification:** `npm test` exits 0 with 'Smoke tests passed' and 'Eval regression harness passed (56 scenarios)'. `node bin/tusq.js help | grep -c '^  [a-z]'` → 49. `lower` appears between `legacy` and `method` in help. `node bin/tusq.js lower index --manifest tests/fixtures/express-sample/tusq.manifest.json --json` exits 0 with `first_property_minimum_states[]` containing `lower_unbounded` (get_users_api_v1_users_id + post_users_users, aggregation_key=numeric_lower_bound_constraint, capability_count=2) and `not_applicable` (get_users_users, aggregation_key=not_applicable, capability_count=1) buckets; `lower_bounded` bucket absent on canonical fixture — empty-bucket-MUST-NOT-appear invariant confirmed. `--lower LOWER_BOUNDED` → exit 1 (case-sensitive enforcement). `--lower lower_bounded` → exit 1 (absent-bucket enforcement). `--lower lower_unbounded` → exit 0. Zero package drift. Zero fixture mutation.

---

## M64 (run_3626d963236136d0, turn_243c1b5e877fb108, dev)

**Challenge to PM turn:** PM (turn_299c8f65bb142aef) correctly bound M64: Static Capability Input Schema First Property MultipleOf Numeric-Divisibility Annotation Presence Index (V1.45 PROPOSED) under intake charter `intent_1777398700273_b24a` (vision_scan, roadmap_exhausted_vision_open). git diff HEAD~1..HEAD --name-only confirms PM modified exactly 4 PM-owned files (ROADMAP.md, PM_SIGNOFF.md, SYSTEM_SPEC.md, command-surface.md), zero source drift in src/bin/tests/website/package.json. All five PM decisions upheld: (1) new noun `divisor` inserted between `diff` and `domain` (diff(d=100,i=105,f=102)<divisor(d=100,i=105,v=118) at pos 2 (f(102)<v(118)); divisor(d=100,i=105,v=118)<domain(d=100,o=111) at pos 1 (i(105)<o(111))); (2) four-value bucket-key enum `multiple_constrained|multiple_unconstrained|not_applicable|unknown`; (3) aggregation_key enum `numeric_divisibility_constraint|not_applicable|unknown` three-value; (4) bucket iteration order `multiple_constrained→multiple_unconstrained→not_applicable→unknown`; (5) six frozen warning reason codes (five from M55–M63 + SIXTH: `input_schema_properties_first_property_multiple_of_invalid_when_present` — covers ALL non-strictly-positive-finite malformations under single consolidated code); NULL-AS-ABSENT `multipleOf:null` → multiple_unconstrained (no warning, mirrors M55–M63 precedent); EXPLICIT-ZERO-IS-INVALID (M64-SPECIFIC): `multipleOf:0` → unknown WITH 6th code (NOT boundary-active — JSON-Schema strictly rejects multipleOf:0); FRACTIONAL-DIVISORS-ARE-VALID (M64-SPECIFIC): `multipleOf:0.5` → multiple_constrained (predicate is `typeof==='number'&&Number.isFinite()&&>0`, NOT `Number.isInteger()&&>=0`); STRICT `typeof v === 'number' && Number.isFinite(v) && v > 0` (NO Number()/parseFloat()/truthy coercion); result array field name `first_property_multiple_of_states`; per-bucket field name `input_schema_first_property_multiple_of`. Challenge resolved: no new objections.

**Implementation:**
- Added `INPUT_SCHEMA_FIRST_PROPERTY_MULTIPLE_OF_ENUM` (frozen Set: multiple_constrained/multiple_unconstrained/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_MULTIPLE_OF_AGGREGATION_KEY_ENUM` (frozen Set: numeric_divisibility_constraint/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_MULTIPLE_OF_BUCKET_ORDER` (frozen array: multiple_constrained/multiple_unconstrained/not_applicable) constants in `src/cli.js` after M63 maxLength constants.
- Added guard functions `_guardInputSchemaFirstPropertyMultipleOfBucketKey` and `_guardInputSchemaFirstPropertyMultipleOfAggregationKey`.
- Added `classifyInputSchemaFirstPropertyMultipleOf(inputSchema)` pure function implementing: unknown for null/undefined/non-object inputSchema; unknown for missing/non-string type; not_applicable for type!=='object' string; unknown for missing/null/non-object properties (when type==='object'); not_applicable for zero-property object; unknown for non-plain-object firstVal (5th frozen code: `input_schema_properties_first_property_descriptor_invalid`); multiple_unconstrained for absent/undefined/null multipleOf (null-as-absent per M55–M63, no warning); multiple_constrained for `typeof v === 'number' && Number.isFinite(v) && v > 0` (FRACTIONAL-DIVISORS-ARE-VALID: 0.5→multiple_constrained; EXPLICIT-ZERO-IS-INVALID: 0→falls through to unknown; no warning for valid values); unknown for present non-null non-strictly-positive-finite multipleOf (6th frozen code: `input_schema_properties_first_property_multiple_of_invalid_when_present` — covers zero/0, negative/-1, NaN, Infinity, -Infinity, string/'5', boolean/true, array/[5], plain object/{}). STRICT `typeof === 'number' && Number.isFinite() && > 0` check — NO Number()/parseFloat()/truthy coercion.
- Added `buildInputSchemaFirstPropertyMultipleOfIndex(manifest, manifestPath)`, `formatInputSchemaFirstPropertyMultipleOfIndex(index)`, `cmdDivisor(args)`, `cmdDivisorIndex(args)`, `parseDivisorIndexArgs(args)` functions.
- Wired `'divisor'` case between `'diff'` and `'domain'` in dispatch table.
- Added `'divisor'` line between `'diff'` and `'domain'` in `printHelp()` with text `Index capabilities by input schema first property multipleOf numeric-divisibility annotation presence for planning review`.
- Added `'divisor'` and `'divisor index'` entries in `printCommandHelp()`.
- CLI surface: 47→48.

**Tests:**
- Updated all 29 `!== 47` help-count assertions to `!== 48` in `tests/smoke.mjs`.
- Added 24-case M64 smoke matrix (a-x4) with synthetic fixture covering constrained_cap (multipleOf:5), frac_constrained_cap (multipleOf:0.5 — FRACTIONAL-DIVISORS-ARE-VALID), unconstrained_cap (no multipleOf field), not_applicable_cap (type:'array'). Covers: bucket order (multiple_constrained→multiple_unconstrained→not_applicable), filter flags, case-sensitivity (`--divisor MULTIPLE_CONSTRAINED` → exit 1), null-as-absent (multipleOf:null→multiple_unconstrained, no warning), EXPLICIT-ZERO-IS-INVALID (multipleOf:0→unknown WITH 6th code), negative malformation (multipleOf:-1→unknown), FRACTIONAL-DIVISORS-ARE-VALID (multipleOf:0.5→multiple_constrained, no warning), string malformation (multipleOf:'5'→unknown), boolean malformation (multipleOf:true→unknown), all 6 warning reason codes, aggregation_key closed enum (numeric_divisibility_constraint/not_applicable/unknown), non-persistence, help enumeration and ordering (divisor between diff and domain).
- Added `input-schema-first-property-multiple-of-index-determinism` eval scenario to `tests/evals/governed-cli-scenarios.json` (54→55 scenarios) with 7 synthetic capabilities covering all four buckets (including FRACTIONAL-DIVISORS-ARE-VALID frac_constrained_cap, null-as-absent null_multiple_of_cap, insertion-order verification keys={z,a,b}/firstKey=z has multipleOf=5).
- Added `runInputSchemaFirstPropertyMultipleOfIndexDeterminismScenario` handler in `tests/eval-regression.mjs`.

**Verification:** `npm test` exits 0 with 'Smoke tests passed' and 'Eval regression harness passed (55 scenarios)'. `node bin/tusq.js help | grep -c '^  [a-z]'` → 48. `divisor` appears between `diff` and `domain` in help. `node bin/tusq.js divisor index --manifest tests/fixtures/express-sample/tusq.manifest.json --json` exits 0 with `first_property_multiple_of_states[]` containing `multiple_unconstrained` (get_users_api_v1_users_id + post_users_users, aggregation_key=numeric_divisibility_constraint, capability_count=2) and `not_applicable` (get_users_users, aggregation_key=not_applicable, capability_count=1) buckets; `multiple_constrained` bucket absent on canonical fixture — empty-bucket-MUST-NOT-appear invariant confirmed. `--divisor MULTIPLE_CONSTRAINED` → exit 1 (case-sensitive enforcement). `--divisor multiple_constrained` → exit 1 (absent-bucket enforcement). `--divisor multiple_unconstrained` → exit 0. Zero package drift. Zero fixture mutation.

---

## M63 (run_e29754fde9f7e4f7, turn_a43e548e5465caff, dev)

**Challenge to PM turn:** PM (turn_9b08017116bfbfdc) correctly bound M63: Static Capability Input Schema First Property MaxLength String-Length-Ceiling Annotation Presence Index (V1.44 PROPOSED) under intake charter `intent_1777396463568_d089` (vision_scan, roadmap_exhausted_vision_open). git diff HEAD~1..HEAD --name-only confirms PM modified exactly 4 PM-owned files (ROADMAP.md, PM_SIGNOFF.md, SYSTEM_SPEC.md, command-surface.md), zero source drift in src/bin/tests/website/package.json. All five PM decisions upheld: (1) new noun `ceiling` inserted between `caption` and `choice` (caption(c=99,a=97)<ceiling(c=99,e=101) at pos 1 (a(97)<e(101)); ceiling(c=99,e=101)<choice(c=99,h=104) at pos 1 (e(101)<h(104))); (2) four-value bucket-key enum `length_capped|length_uncapped|not_applicable|unknown`; (3) aggregation_key enum `string_length_ceiling_constraint|not_applicable|unknown` three-value; (4) bucket iteration order `length_capped→length_uncapped→not_applicable→unknown`; (5) six frozen warning reason codes (five from M55/M56/M57/M58/M59/M60/M61/M62 + SIXTH: `input_schema_properties_first_property_max_length_invalid_when_present` — covers ALL non-non-negative-integer malformations under a single consolidated code); NULL-AS-ABSENT `maxLength:null` → length_uncapped (no warning, mirrors M55/M56/M57/M58/M59/M60/M61/M62 precedent); EXPLICIT-ZERO-IS-CAPPED `maxLength:0` → length_capped (no warning, mirrors M58/M60/M61/M62 explicit-default-value precedent for boundary axes); STRICT-NUMERIC `Number.isInteger() && >= 0` (NO Number()/parseInt()/truthy coercion); result array field name `first_property_max_length_states`; per-bucket field name `input_schema_first_property_max_length`. Challenge resolved: no new objections.

**Implementation:**
- Added `INPUT_SCHEMA_FIRST_PROPERTY_MAX_LENGTH_ENUM` (frozen Set: length_capped/length_uncapped/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_MAX_LENGTH_AGGREGATION_KEY_ENUM` (frozen Set: string_length_ceiling_constraint/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_MAX_LENGTH_BUCKET_ORDER` (frozen array: length_capped/length_uncapped/not_applicable) constants in `src/cli.js` after M62 minLength constants.
- Added guard functions `_guardInputSchemaFirstPropertyMaxLengthBucketKey` and `_guardInputSchemaFirstPropertyMaxLengthAggregationKey`.
- Added `classifyInputSchemaFirstPropertyMaxLength(inputSchema)` pure function implementing: unknown for null/undefined/non-object inputSchema; unknown for missing/non-string type; not_applicable for type!=='object' string; unknown for missing/null/non-object properties (when type==='object'); not_applicable for zero-property object; unknown for non-plain-object firstVal (5th frozen code: `input_schema_properties_first_property_descriptor_invalid`); length_uncapped for absent/undefined/null maxLength (null-as-absent per M55/M56/M57/M58/M59/M60/M61/M62, no warning); length_capped for `Number.isInteger(maxLength) && maxLength >= 0` (includes maxLength===0 — EXPLICIT-ZERO-IS-CAPPED, no warning); unknown for present non-null non-non-negative-integer maxLength (6th frozen code: `input_schema_properties_first_property_max_length_invalid_when_present` — covers negative integer/-1, fractional/1.5, NaN, Infinity, -Infinity, string/'100', boolean/true, array/[1], plain object/{}). STRICT `Number.isInteger() && >= 0` check — NO Number()/parseInt()/truthy coercion.
- Added `buildInputSchemaFirstPropertyMaxLengthIndex(manifest, manifestPath)`, `formatInputSchemaFirstPropertyMaxLengthIndex(index)`, `cmdCeiling(args)`, `cmdCeilingIndex(args)`, `parseCeilingIndexArgs(args)` functions.
- Wired `'ceiling'` case between `'caption'` and `'choice'` in dispatch table.
- Added `'ceiling'` line between `'caption'` and `'choice'` in `printHelp()` with text `Index capabilities by input schema first property maxLength string-length-ceiling annotation presence for planning review`.
- Added `'ceiling'` and `'ceiling index'` entries in `printCommandHelp()`.
- CLI surface: 46→47.

**Tests:**
- Updated all 28 `!== 46` help-count assertions to `!== 47` in `tests/smoke.mjs`.
- Added 24-case M63 smoke matrix (a-x4) with synthetic fixture covering capped_cap (maxLength:100), zero_cap_cap (maxLength:0 — EXPLICIT-ZERO-IS-CAPPED), uncapped_cap (no maxLength field), not_applicable_cap (type:'array'). Covers: bucket order (length_capped→length_uncapped→not_applicable), filter flags, case-sensitivity (`--ceiling LENGTH_CAPPED` → exit 1), null-as-absent (maxLength:null→length_uncapped, no warning), EXPLICIT-ZERO-IS-CAPPED (maxLength:0→length_capped, no warning), negative integer malformation (maxLength:-1→unknown WITH 6th code), fractional malformation (maxLength:1.5→unknown), string malformation (maxLength:'100'→unknown), boolean malformation (maxLength:true→unknown), all 6 warning reason codes, aggregation_key closed enum (string_length_ceiling_constraint/not_applicable/unknown), non-persistence, help enumeration and ordering (ceiling between caption and choice).
- Added `input-schema-first-property-max-length-index-determinism` eval scenario to `tests/evals/governed-cli-scenarios.json` (53→54 scenarios) with 7 synthetic capabilities covering all four buckets (including EXPLICIT-ZERO-IS-CAPPED zero_capped_cap, null-as-absent null_max_length_cap, insertion-order verification keys={z,a,b}/firstKey=z has maxLength=200).
- Added `runInputSchemaFirstPropertyMaxLengthIndexDeterminismScenario` handler in `tests/eval-regression.mjs`.

**Verification:** `npm test` exits 0 with 'Smoke tests passed' and 'Eval regression harness passed (54 scenarios)'. `node bin/tusq.js help | grep -c '^  [a-z]'` → 47. `ceiling` appears between `caption` and `choice` in help. `node bin/tusq.js ceiling index --manifest tests/fixtures/express-sample/tusq.manifest.json --json` exits 0 with `first_property_max_length_states[]` containing `length_uncapped` (get_users_api_v1_users_id + post_users_users, aggregation_key=string_length_ceiling_constraint, capability_count=2) and `not_applicable` (get_users_users, aggregation_key=not_applicable, capability_count=1) buckets; `length_capped` bucket absent on canonical fixture — empty-bucket-MUST-NOT-appear invariant confirmed. `--ceiling LENGTH_CAPPED` → exit 1 (case-sensitive enforcement). `--ceiling length_capped` → exit 1 (absent-bucket enforcement). `--ceiling length_uncapped` → exit 0. Zero package drift. Zero fixture mutation.

---

## M62 (run_a9f53305f71ff8e4, turn_0ae9e9cb6c065564, dev)

**Challenge to PM turn:** PM (turn_ba5df362ec35d789) correctly bound M62: Static Capability Input Schema First Property MinLength String-Length-Floor Annotation Presence Index (V1.43 PROPOSED) under intake charter `intent_1777394194739_38d7` (vision_scan, roadmap_exhausted_vision_open). git diff HEAD~1..HEAD --name-only confirms PM modified exactly 4 PM-owned files (ROADMAP.md, PM_SIGNOFF.md, SYSTEM_SPEC.md, command-surface.md), zero source drift in src/bin/tests/website/package.json. All five PM decisions upheld: (1) new noun `floor` inserted between `examples` and `gloss` (examples(e=101,x=120)<floor(f=102,l=108) at pos 0 (e(101)<f(102)); floor(f=102,l=108)<gloss(g=103,l=108) at pos 0 (f(102)<g(103))); (2) four-value bucket-key enum `length_floored|length_unfloored|not_applicable|unknown`; (3) aggregation_key enum `string_length_floor_constraint|not_applicable|unknown` three-value; (4) bucket iteration order `length_floored→length_unfloored→not_applicable→unknown`; (5) six frozen warning reason codes (five from M55/M56/M57/M58/M59/M60/M61 + SIXTH: `input_schema_properties_first_property_min_length_invalid_when_present` — covers ALL non-non-negative-integer malformations under a single consolidated code); NULL-AS-ABSENT `minLength:null` → length_unfloored (no warning, mirrors M55/M56/M57/M58/M59/M60/M61 precedent); EXPLICIT-ZERO-IS-FLOORED `minLength:0` → length_floored (no warning, mirrors M58/M60/M61 explicit-default-value precedent for boundary axes); STRICT-NUMERIC `Number.isInteger() && >= 0` (NO Number()/parseInt()/truthy coercion); result array field name `first_property_min_length_states`; per-bucket field name `input_schema_first_property_min_length`. Challenge resolved: no new objections.

**Implementation:**
- Added `INPUT_SCHEMA_FIRST_PROPERTY_MIN_LENGTH_ENUM` (frozen Set: length_floored/length_unfloored/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_MIN_LENGTH_AGGREGATION_KEY_ENUM` (frozen Set: string_length_floor_constraint/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_MIN_LENGTH_BUCKET_ORDER` (frozen array: length_floored/length_unfloored/not_applicable) constants in `src/cli.js` after M61 writeOnly constants.
- Added guard functions `_guardInputSchemaFirstPropertyMinLengthBucketKey` and `_guardInputSchemaFirstPropertyMinLengthAggregationKey`.
- Added `classifyInputSchemaFirstPropertyMinLength(inputSchema)` pure function implementing: unknown for null/undefined/non-object inputSchema; unknown for missing/non-string type; not_applicable for type!=='object' string; unknown for missing/null/non-object properties (when type==='object'); not_applicable for zero-property object; unknown for non-plain-object firstVal (5th frozen code: `input_schema_properties_first_property_descriptor_invalid`); length_unfloored for absent/undefined/null minLength (null-as-absent per M55/M56/M57/M58/M59/M60/M61, no warning); length_floored for `Number.isInteger(minLength) && minLength >= 0` (includes minLength===0 — EXPLICIT-ZERO-IS-FLOORED, no warning); unknown for present non-null non-non-negative-integer minLength (6th frozen code: `input_schema_properties_first_property_min_length_invalid_when_present` — covers negative integer/-1, fractional/1.5, NaN, Infinity, -Infinity, string/'1', boolean/true, array/[1], plain object/{}). STRICT `Number.isInteger() && >= 0` check — NO Number()/parseInt()/truthy coercion.
- Added `buildInputSchemaFirstPropertyMinLengthIndex(manifest, manifestPath)`, `formatInputSchemaFirstPropertyMinLengthIndex(index)`, `cmdFloor(args)`, `cmdFloorIndex(args)`, `parseFloorIndexArgs(args)` functions.
- Wired `'floor'` case between `'examples'` and `'gloss'` in dispatch table.
- Added `'floor'` line between `'examples'` and `'gloss'` in `printHelp()` with text `Index capabilities by input schema first property minLength string-length-floor annotation presence for planning review`.
- Added `'floor'` and `'floor index'` entries in `printCommandHelp()`.
- CLI surface: 45→46.

**Tests:**
- Updated all 27 `!== 45` help-count assertions to `!== 46` in `tests/smoke.mjs`. Updated error message references from M61/secret to M62/floor.
- Added 24-case M62 smoke matrix (a-x4) with synthetic fixture covering floored_cap (minLength:1), zero_floor_cap (minLength:0 — EXPLICIT-ZERO-IS-FLOORED), unfloored_cap (no minLength field), not_applicable_cap (type:'array'). Covers: bucket order (length_floored→length_unfloored→not_applicable), filter flags, case-sensitivity (`--floor LENGTH_FLOORED` → exit 1), null-as-absent (minLength:null→length_unfloored, no warning), EXPLICIT-ZERO-IS-FLOORED (minLength:0→length_floored, no warning), negative integer malformation (minLength:-1→unknown WITH 6th code), fractional malformation (minLength:1.5→unknown), string malformation (minLength:'1'→unknown), boolean malformation (minLength:true→unknown), all 6 warning reason codes, aggregation_key closed enum (string_length_floor_constraint/not_applicable/unknown), non-persistence, help enumeration and ordering (floor between examples and gloss).
- Added `input-schema-first-property-min-length-index-determinism` eval scenario to `tests/evals/governed-cli-scenarios.json` (52→53 scenarios) with 7 synthetic capabilities covering all four buckets (including EXPLICIT-ZERO-IS-FLOORED zero_floored_cap, null-as-absent null_min_length_cap, insertion-order verification keys={z,a,b}/firstKey=z has minLength=5).
- Added `runInputSchemaFirstPropertyMinLengthIndexDeterminismScenario` handler in `tests/eval-regression.mjs`.

**Verification:** `npm test` exits 0 with 'Smoke tests passed' and 'Eval regression harness passed (53 scenarios)'. `node bin/tusq.js help | grep -c '^  [a-z]'` → 46. `floor` appears between `examples` and `gloss` in help. `node bin/tusq.js floor index --manifest tests/fixtures/express-sample/tusq.manifest.json --json` exits 0 with `first_property_min_length_states[]` containing `length_unfloored` (get_users_api_v1_users_id + post_users_users) and `not_applicable` (get_users_users) buckets; `length_floored` bucket absent on canonical fixture — empty-bucket-MUST-NOT-appear invariant confirmed. `--floor LENGTH_FLOORED` → exit 1 (case-sensitive enforcement). `--floor length_floored` → exit 1 (absent-bucket enforcement). Zero package drift. Zero fixture mutation.

---

## M61 (run_95643a76d3091ffd, turn_31d6f8498c2b8b0b, dev)

**Challenge to PM turn:** PM (turn_c7ec0d02a286e887) correctly bound M61: Static Capability Input Schema First Property WriteOnly Annotation Presence Index (V1.42 PROPOSED) under intake charter `intent_1777392151436_b276` (vision_scan, roadmap_exhausted_vision_open). git diff HEAD~1..HEAD --name-only confirms PM modified exactly 4 PM-owned files (ROADMAP.md, PM_SIGNOFF.md, SYSTEM_SPEC.md, command-surface.md), zero source drift in src/bin/tests/website/package.json. All five PM decisions upheld: (1) new noun `secret` inserted between `seal` and `sensitivity` (seal(s=115,e=101,a=97)<secret(s=115,e=101,c=99) at pos 2 (a(97)<c(99)); secret(s=115,e=101,c=99)<sensitivity(s=115,e=101,n=110) at pos 2 (c(99)<n(110))); (2) four-value bucket-key enum `write_only|not_write_only|not_applicable|unknown`; (3) aggregation_key enum `output_visibility|not_applicable|unknown` three-value; (4) bucket iteration order `write_only→not_write_only→not_applicable→unknown`; (5) six frozen warning reason codes (five from M55/M56/M57/M58/M59/M60 + SIXTH: `input_schema_properties_first_property_write_only_invalid_when_present` — covers ALL non-boolean malformations under a single consolidated code); NULL-AS-ABSENT `writeOnly:null` → not_write_only (no warning, mirrors M55/M56/M57/M58/M59/M60 precedent); EXPLICIT-FALSE-IS-NOT-WRITE-ONLY `writeOnly:false` → not_write_only (no warning, mirrors M60 EXPLICIT-FALSE-IS-MUTABLE precedent); STRICT-BOOLEAN `=== true` and `=== false` (NO truthy/falsy coercion); result array field name `first_property_write_only_states`; per-bucket field name `input_schema_first_property_write_only`. Challenge resolved: no new objections.

**Implementation:**
- Added `INPUT_SCHEMA_FIRST_PROPERTY_WRITE_ONLY_ENUM` (frozen Set: write_only/not_write_only/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_WRITE_ONLY_AGGREGATION_KEY_ENUM` (frozen Set: output_visibility/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_WRITE_ONLY_BUCKET_ORDER` (frozen array: write_only/not_write_only/not_applicable) constants in `src/cli.js` after M60 readOnly constants.
- Added guard functions `_guardInputSchemaFirstPropertyWriteOnlyBucketKey` and `_guardInputSchemaFirstPropertyWriteOnlyAggregationKey`.
- Added `classifyInputSchemaFirstPropertyWriteOnly(inputSchema)` pure function implementing: unknown for null/undefined/non-object inputSchema; unknown for missing/non-string type; not_applicable for type!=='object' string; unknown for missing/null/non-object properties (when type==='object'); not_applicable for zero-property object; unknown for non-plain-object firstVal (5th frozen code: `input_schema_properties_first_property_descriptor_invalid`); not_write_only for absent/undefined/null writeOnly (null-as-absent per M55/M56/M57/M58/M59/M60, no warning); not_write_only for `writeOnly === false` (EXPLICIT-FALSE-IS-NOT-WRITE-ONLY, no warning); write_only for `writeOnly === true`; unknown for present non-null non-boolean writeOnly (6th frozen code: `input_schema_properties_first_property_write_only_invalid_when_present`). STRICT `=== true` and `=== false` checks — NO truthy/falsy coercion.
- Added `buildInputSchemaFirstPropertyWriteOnlyIndex(manifest, manifestPath)`, `formatInputSchemaFirstPropertyWriteOnlyIndex(index)`, `cmdSecret(args)`, `cmdSecretIndex(args)`, `parseSecretIndexArgs(args)` functions.
- Wired `'secret'` case between `'seal'` and `'sensitivity'` in dispatch table.
- Added `'secret'` line between `'seal'` and `'sensitivity'` in `printHelp()` with text `Index capabilities by input schema first property writeOnly output-suppression annotation for planning review`.
- Added `'secret'` and `'secret index'` entries in `printCommandHelp()`.
- CLI surface: 44→45.

**Tests:**
- Updated all 26 `!== 44` help-count assertions to `!== 45` in `tests/smoke.mjs`.
- Added 24-case M61 smoke matrix (a-x4) with synthetic fixture covering write_only_cap (writeOnly:true), not_write_only_cap (no writeOnly field), not_applicable_cap (type:'array'). Covers: bucket order (write_only→not_write_only→not_applicable), filter flags, case-sensitivity (`--secret WRITE_ONLY` → exit 1), null-as-absent (writeOnly:null→not_write_only, no warning), EXPLICIT-FALSE-IS-NOT-WRITE-ONLY (writeOnly:false→not_write_only, no warning), all 6 warning reason codes with 6th covering string/number/array/object malformations, aggregation_key closed enum (output_visibility/not_applicable/unknown), non-persistence, help enumeration and ordering.
- Added `input-schema-first-property-write-only-index-determinism` eval scenario to `tests/evals/governed-cli-scenarios.json` (51→52 scenarios) with 7 synthetic capabilities covering all four buckets, insertion-order verification (keys={z,a,b}, firstKey=z has writeOnly=true → write_only bucket), null-as-absent, and EXPLICIT-FALSE-IS-NOT-WRITE-ONLY.
- Added `runInputSchemaFirstPropertyWriteOnlyIndexDeterminismScenario` handler in `tests/eval-regression.mjs`.

**Verification:** `npm test` exits 0 with 'Smoke tests passed' and 'Eval regression harness passed (52 scenarios)'. `node bin/tusq.js help | grep -c '^  [a-z]'` → 45. `secret` appears between `seal` and `sensitivity` in help. `node bin/tusq.js secret index --manifest tests/fixtures/express-sample/tusq.manifest.json --json` exits 0 with `first_property_write_only_states[]` containing `not_write_only` and `not_applicable` buckets; `write_only` bucket absent on canonical fixture — empty-bucket-MUST-NOT-appear invariant confirmed. `--secret WRITE_ONLY` → exit 1 (case-sensitive enforcement). `--secret write_only` → exit 1 (absent-bucket enforcement). Zero package drift. Zero fixture mutation.

---

## M60 (run_139647ed3258809b, turn_76d6180219f49289, dev)

**Challenge to PM turn:** PM (turn_769eaad1ebd861f1) correctly bound M60: Static Capability Input Schema First Property ReadOnly Annotation Presence Index (V1.41 PROPOSED) under intake charter `intent_1777389814604_0fa4` (vision_scan, roadmap_exhausted_vision_open). git diff 040a8db..HEAD --name-only confirms PM modified exactly 4 PM-owned files (ROADMAP.md, PM_SIGNOFF.md, SYSTEM_SPEC.md, command-surface.md), zero source drift in src/bin/tests/website/package.json. All five PM decisions upheld: (1) new noun `seal` inserted between `sample` and `sensitivity` (sample(s=115,a=97)<seal(s=115,e=101) at pos 1 (a(97)<e(101)); seal(s=115,e=101,a=97)<sensitivity(s=115,e=101,n=110) at pos 2 (a(97)<n(110))); (2) four-value bucket-key enum `readonly|mutable|not_applicable|unknown`; (3) aggregation_key enum `mutability_state|not_applicable|unknown` three-value; (4) bucket iteration order `readonly→mutable→not_applicable→unknown`; (5) six frozen warning reason codes (five from M55/M56/M57/M58/M59 + SIXTH: `input_schema_properties_first_property_read_only_invalid_when_present` — covers ALL non-boolean malformations under a single consolidated code); NULL-AS-ABSENT `readOnly:null` → mutable (no warning, mirrors M55/M56/M57/M58/M59 precedent); EXPLICIT-FALSE-IS-MUTABLE `readOnly:false` → mutable (no warning, mirrors M58 EXPLICIT-FALSE-IS-ACTIVE precedent); STRICT-BOOLEAN `=== true` and `=== false` (NO truthy/falsy coercion); result array field name `first_property_read_only_states`; per-bucket field name `input_schema_first_property_read_only`. Challenge resolved: no new objections.

**Implementation:**
- Added `INPUT_SCHEMA_FIRST_PROPERTY_READ_ONLY_ENUM` (frozen Set: readonly/mutable/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_READ_ONLY_AGGREGATION_KEY_ENUM` (frozen Set: mutability_state/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_READ_ONLY_BUCKET_ORDER` (frozen array: readonly/mutable/not_applicable) constants in `src/cli.js` after M59 pattern constants.
- Added guard functions `_guardInputSchemaFirstPropertyReadOnlyBucketKey` and `_guardInputSchemaFirstPropertyReadOnlyAggregationKey`.
- Added `classifyInputSchemaFirstPropertyReadOnly(inputSchema)` pure function implementing: unknown for null/undefined/non-object inputSchema; unknown for missing/non-string type; not_applicable for type!=='object' string; unknown for missing/null/non-object properties (when type==='object'); not_applicable for zero-property object; unknown for non-plain-object firstVal (5th frozen code: `input_schema_properties_first_property_descriptor_invalid`); mutable for absent/undefined/null readOnly (null-as-absent per M55/M56/M57/M58/M59, no warning); mutable for `readOnly === false` (EXPLICIT-FALSE-IS-MUTABLE, no warning); readonly for `readOnly === true`; unknown for present non-null non-boolean readOnly (6th frozen code: `input_schema_properties_first_property_read_only_invalid_when_present`). STRICT `=== true` and `=== false` checks — NO truthy/falsy coercion.
- Added `buildInputSchemaFirstPropertyReadOnlyIndex(manifest, manifestPath)`, `formatInputSchemaFirstPropertyReadOnlyIndex(index)`, `cmdSeal(args)`, `cmdSealIndex(args)`, `parseSealIndexArgs(args)` functions.
- Wired `'seal'` case between `'sample'` and `'sensitivity'` in dispatch table.
- Added `'seal'` line between `'sample'` and `'sensitivity'` in `printHelp()` with text `Index capabilities by input schema first property readOnly mutability annotation for planning review`.
- Added `'seal'` and `'seal index'` entries in `printCommandHelp()`.
- CLI surface: 43→44.

**Tests:**
- Updated all 25 `!== 43` help-count assertions to `!== 44` in `tests/smoke.mjs`.
- Added 24-case M60 smoke matrix (a-x4) with synthetic fixture covering readonly_cap (readOnly:true), mutable_cap (no readOnly field), not_applicable_cap (type:'array'). Covers: bucket order (readonly→mutable→not_applicable), filter flags, case-sensitivity (`--seal READONLY` → exit 1), null-as-absent (readOnly:null→mutable, no warning), EXPLICIT-FALSE-IS-MUTABLE (readOnly:false→mutable, no warning), all 6 warning reason codes with 6th covering string/number/array/object malformations, aggregation_key closed enum (mutability_state/not_applicable/unknown), non-persistence, help enumeration and ordering.
- Added `input-schema-first-property-read-only-index-determinism` eval scenario to `tests/evals/governed-cli-scenarios.json` (50→51 scenarios) with 7 synthetic capabilities covering all four buckets, insertion-order verification (keys={z,a,b}, firstKey=z has readOnly=true → readonly bucket), null-as-absent, and EXPLICIT-FALSE-IS-MUTABLE.
- Added `runInputSchemaFirstPropertyReadOnlyIndexDeterminismScenario` handler in `tests/eval-regression.mjs`.

**Verification:** `npm test` exits 0 with 'Smoke tests passed' and 'Eval regression harness passed (51 scenarios)'. `node bin/tusq.js help | grep -c '^  [a-z]'` → 44. `seal` appears between `sample` and `sensitivity` in help. `node bin/tusq.js seal index --manifest tests/fixtures/express-sample/tusq.manifest.json --json` exits 0 with `first_property_read_only_states[]` containing `mutable` and `not_applicable` buckets; `readonly` bucket absent on canonical fixture — empty-bucket-MUST-NOT-appear invariant confirmed. `--seal READONLY` → exit 1 (case-sensitive enforcement). Zero package drift. Zero fixture mutation.

---

## M59 (run_8024d2c7f8ee0f10, turn_faf61809c4ae34cb, dev)

**Challenge to PM turn:** PM (turn_5943645cb90ffa81) correctly bound M59: Static Capability Input Schema First Property Pattern Regex Annotation Presence Index Export from Manifest Evidence (~0.5 day) — V1.40 (PROPOSED) under intake charter `intent_1777387559022_51c5` (vision_scan, category `roadmap_exhausted_vision_open`). git diff c26191b..HEAD --name-only confirms PM modified exactly 4 PM-owned files (ROADMAP.md, PM_SIGNOFF.md, SYSTEM_SPEC.md, command-surface.md), zero source drift in src/bin/tests/website/package.json. All five PM decisions upheld: (1) new noun `regex` inserted between `redaction` and `request` (redaction(r=114,e=101,d=100)<regex(r=114,e=101,g=103) at pos 2 (d(100)<g(103)); regex(r=114,e=101,g=103)<request(r=114,e=101,q=113) at pos 2 (g(103)<q(113))); (2) four-value bucket-key enum `patterned|unpatterned|not_applicable|unknown`; (3) aggregation_key enum `pattern_constraint|not_applicable|unknown` three-value; (4) bucket iteration order `patterned→unpatterned→not_applicable→unknown`; (5) six frozen warning reason codes (five from M55/M56/M57/M58 + SIXTH: `input_schema_properties_first_property_pattern_invalid_when_present` — covers BOTH non-string AND empty-string malformations under a single consolidated code); NULL-AS-ABSENT `pattern:null` → unpatterned (no warning, mirrors M55/M56/M57/M58 precedent); WHITESPACE-ONLY `pattern:'   '` → patterned (length >= 1 is the only check — no trim); EMPTY-STRING `pattern:''` → unknown WITH 6th frozen code (deliberate alignment with M57's empty-string-malformed precedent); MUST NOT compile or validate regex syntax; result array field name `first_property_pattern_constraints`; per-bucket field name `input_schema_first_property_pattern`. Challenge resolved: no new objections.

**Implementation:**
- Added `INPUT_SCHEMA_FIRST_PROPERTY_PATTERN_ENUM` (frozen Set: patterned/unpatterned/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_PATTERN_AGGREGATION_KEY_ENUM` (frozen Set: pattern_constraint/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_PATTERN_BUCKET_ORDER` (frozen array: patterned/unpatterned/not_applicable) constants in `src/cli.js` after M58 deprecated constants.
- Added guard functions `_guardInputSchemaFirstPropertyPatternBucketKey` and `_guardInputSchemaFirstPropertyPatternAggregationKey`.
- Added `classifyInputSchemaFirstPropertyPattern(inputSchema)` pure function implementing: unknown for null/undefined/non-object inputSchema; unknown for missing/non-string type; not_applicable for type!=='object' string; unknown for missing/null/non-object properties (when type==='object'); not_applicable for zero-property object; unknown for non-plain-object firstVal (5th frozen code: `input_schema_properties_first_property_descriptor_invalid`); unpatterned for absent/undefined/null pattern (null-as-absent per M55/M56/M57/M58, no warning); patterned for string with length>=1 (WHITESPACE-ONLY-COUNTS-AS-PATTERNED, no trim, no warning); unknown for non-string OR empty-string pattern (6th frozen code: `input_schema_properties_first_property_pattern_invalid_when_present`). MUST NOT compile or interpret the regex.
- Added `buildInputSchemaFirstPropertyPatternIndex(manifest, manifestPath)`, `formatInputSchemaFirstPropertyPatternIndex(index)`, `cmdRegex(args)`, `cmdRegexIndex(args)`, `parseRegexIndexArgs(args)` functions.
- Wired `'regex'` case between `'redaction'` and `'request'` in dispatch table.
- Added `'regex'` line between `'redaction'` and `'request'` in `printHelp()` with text `Index capabilities by input schema first property pattern regex annotation presence for planning review`.
- Added `'regex'` and `'regex index'` entries in `printCommandHelp()`.
- CLI surface: 42→43.

**Tests:**
- Updated all 24 `!== 42` help-count assertions to `!== 43` in `tests/smoke.mjs`.
- Added 24-case M59 smoke matrix (a-x4) with synthetic fixture covering patterned_cap (pattern:'^[a-z]+$'), unpatterned_cap (no pattern field), not_applicable_cap (type:'array'). Covers: bucket order (patterned→unpatterned→not_applicable), filter flags, case-sensitivity (`--regex PATTERNED` → exit 1), null-as-absent (pattern:null→unpatterned, no warning), WHITESPACE-ONLY-COUNTS-AS-PATTERNED (pattern:'   '→patterned, no trim), EMPTY-STRING IS-MALFORMED (pattern:''→unknown WITH 6th code), all 6 warning reason codes, aggregation_key closed enum (pattern_constraint/not_applicable/unknown), non-persistence, help enumeration and ordering.
- Added `input-schema-first-property-pattern-index-determinism` eval scenario to `tests/evals/governed-cli-scenarios.json` (49→50 scenarios) with 6 synthetic capabilities covering all four buckets and insertion-order verification (keys={z,a,b}, firstKey=z has pattern='^foo$' → patterned bucket, NOT sorted 'a').
- Added `runInputSchemaFirstPropertyPatternIndexDeterminismScenario` handler in `tests/eval-regression.mjs`.

**Verification:** `npm test` exits 0 with 'Smoke tests passed' and 'Eval regression harness passed (50 scenarios)'. `node bin/tusq.js help | grep -c '^  [a-z]'` → 43. `regex` appears between `redaction` and `request` in help. `node bin/tusq.js regex index --manifest tests/fixtures/express-sample/tusq.manifest.json --json` exits 0 with `first_property_pattern_constraints[]` containing `unpatterned` and/or `not_applicable` buckets; `patterned` bucket absent on canonical fixture — empty-bucket-MUST-NOT-appear invariant confirmed. `--regex PATTERNED` → exit 1 (case-sensitive enforcement). Zero package drift. Zero fixture mutation.

---

## M58 (run_68913f73e2cc30a6, turn_729ead92ab56d17f, dev)

**Challenge to PM turn:** PM (turn_46c96427c73e4d4e) correctly bound M58: Static Capability Input Schema First Property Deprecated Annotation Presence Index Export from Manifest Evidence (~0.5 day) — V1.39 (PROPOSED) under intake charter `intent_1777385467583_6504` (vision_scan, category `roadmap_exhausted_vision_open`). git diff fea7501..HEAD --name-only confirms PM modified exactly 4 PM-owned files (ROADMAP.md, PM_SIGNOFF.md, SYSTEM_SPEC.md, command-surface.md), zero source drift in src/bin/tests/website/package.json. All five PM decisions upheld: (1) new noun `legacy` inserted between `items` and `method` (items(i=105)<legacy(l=108) at pos 0; legacy(l=108)<method(m=109) at pos 0); (2) four-value bucket-key enum `deprecated|active|not_applicable|unknown`; (3) aggregation_key enum `lifecycle_stage|not_applicable|unknown` three-value; (4) bucket iteration order `deprecated→active→not_applicable→unknown`; (5) six frozen warning reason codes (five from M55/M56/M57 + SIXTH: `input_schema_properties_first_property_deprecated_invalid_when_present` — covers ALL non-boolean malformations: string 'true', number 1/0, array, object); NULL-AS-ABSENT `deprecated:null` → active (no warning, mirrors M55/M56/M57 precedent); EXPLICIT-FALSE-IS-ACTIVE `deprecated:false` → active (no warning); NO truthy/falsy coercion; result array field name `first_property_deprecation_states`. Challenge resolved: no new objections.

**Implementation:**
- Added `INPUT_SCHEMA_FIRST_PROPERTY_DEPRECATED_ENUM` (frozen Set: deprecated/active/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_DEPRECATED_AGGREGATION_KEY_ENUM` (frozen Set: lifecycle_stage/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_DEPRECATED_BUCKET_ORDER` (frozen array: deprecated/active/not_applicable) constants in `src/cli.js` after M57 title constants.
- Added guard functions `_guardInputSchemaFirstPropertyDeprecatedBucketKey` and `_guardInputSchemaFirstPropertyDeprecatedAggregationKey`.
- Added `classifyInputSchemaFirstPropertyDeprecated(inputSchema)` pure function implementing: unknown for null/undefined/non-object inputSchema; unknown for missing/non-string type; not_applicable for type!=='object' string; unknown for missing/null/non-object properties (when type==='object'); not_applicable for zero-property object; unknown for non-plain-object firstVal (5th frozen code); active for absent/undefined/null deprecated (null-as-absent, no warning); active for deprecated===false (EXPLICIT-FALSE-IS-ACTIVE, no warning); deprecated for deprecated===true; unknown for non-boolean deprecated (6th frozen code). Strict `typeof firstVal.deprecated === 'boolean'` check — NO truthy/falsy coercion.
- Added `buildInputSchemaFirstPropertyDeprecatedIndex`, `formatInputSchemaFirstPropertyDeprecatedIndex`, `cmdLegacy`, `cmdLegacyIndex`, `parseLegacyIndexArgs` functions.
- Wired `'legacy'` case between `'items'` and `'method'` in dispatch table.
- Added `'legacy'` line between `'items'` and `'method'` in `printHelp()`.
- Added `'legacy'` and `'legacy index'` entries in `printCommandHelp()`.
- CLI surface: 41→42.

**Tests:**
- Updated all 23 `!== 41` help-count assertions to `!== 42` in `tests/smoke.mjs`.
- Added 24-case M58 smoke matrix (a-x4) with synthetic fixture covering deprecated_cap (deprecated:true), active_false_cap (deprecated:false → active), active_absent_cap (absent → active), not_applicable_cap (type:'array'). Covers: bucket order (deprecated→active→not_applicable), filter flags, case-sensitivity, null-as-absent, EXPLICIT-FALSE-IS-ACTIVE, all 6 warning reason codes, aggregation_key closed enum (lifecycle_stage/not_applicable/unknown), non-persistence, help enumeration and ordering.
- Added `input-schema-first-property-deprecated-index-determinism` eval scenario to `tests/evals/governed-cli-scenarios.json` (48→49 scenarios) with 6 synthetic capabilities covering all four buckets and insertion-order verification (keys={z,a,b}, firstKey=z has deprecated=true → deprecated bucket, NOT sorted 'a').
- Added `runInputSchemaFirstPropertyDeprecatedIndexDeterminismScenario` handler in `tests/eval-regression.mjs`.

**Verification:** `npm test` exits 0 with 'Smoke tests passed' and 'Eval regression harness passed (49 scenarios)'. `node bin/tusq.js help | grep -c '^  [a-z]'` → 42. `legacy` appears between `items` and `method` in help. `node bin/tusq.js legacy index --manifest tests/fixtures/express-sample/tusq.manifest.json --json` exits 0 with `first_property_deprecation_states[]` containing `active` bucket (get_users_api_v1_users_id + post_users_users, aggregation_key=lifecycle_stage, capability_count=2) and `not_applicable` bucket (get_users_users, aggregation_key=not_applicable, capability_count=1); `deprecated` bucket absent — empty-bucket-MUST-NOT-appear invariant confirmed. `--legacy DEPRECATED` → exit 1 (case-sensitive enforcement). Zero package drift. Zero fixture mutation.

---

## M57 (run_27565cc0d89187ef, turn_09b015ce0e7b8ed1, dev)

**Challenge to PM turn:** PM (turn_7bb9f75f896a40a2) correctly bound M57: Static Capability Input Schema First Property Title Presence Index Export from Manifest Evidence (~0.5 day) — V1.38 (PROPOSED) under intake charter `intent_1777383136065_38f3` (vision_scan, category `roadmap_exhausted_vision_open`). git diff 906dfca..HEAD --name-only confirms PM modified exactly 4 PM-owned files (ROADMAP.md, PM_SIGNOFF.md, SYSTEM_SPEC.md, command-surface.md), zero source drift in src/bin/tests/website/package.json. All five PM decisions upheld: (1) new noun `caption` inserted between `binding` and `choice` (binding(b=98)<caption(c=99) at pos 0; caption(c=99,a=97)<choice(c=99,h=104) at pos 0 same, pos 1 a(97)<h(104)); (2) four-value bucket-key enum `titled|untitled|not_applicable|unknown`; (3) aggregation_key enum `title_label|not_applicable|unknown` three-value; (4) bucket iteration order `titled→untitled→not_applicable→unknown`; (5) six frozen warning reason codes (five from M55/M56 + SIXTH: `input_schema_properties_first_property_title_invalid_when_present`); EMPTY-STRING `title:''` is unknown WITH 6th frozen code (deliberate alignment with M54 empty-enum-is-malformed and M56 empty-array-is-malformed precedents); `firstVal.title === null` treated as absent → untitled (null-as-absent per M55/M56 precedent); WHITESPACE-ONLY `title:'   '` counts as titled (string length >= 1 is the only check at this milestone); result array field name `first_property_titles`. Challenge resolved: no new objections.

**Implementation:**
- Added `INPUT_SCHEMA_FIRST_PROPERTY_TITLE_ENUM` (frozen Set: titled/untitled/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_TITLE_AGGREGATION_KEY_ENUM` (frozen Set: title_label/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_TITLE_BUCKET_ORDER` (frozen array: titled/untitled/not_applicable) constants in `src/cli.js` after M56 examples constants.
- Added guard functions `_guardInputSchemaFirstPropertyTitleBucketKey` and `_guardInputSchemaFirstPropertyTitleAggregationKey` mirroring M35–M56 precedent.
- Added `classifyInputSchemaFirstPropertyTitle(inputSchema)` pure function: returns `'not_applicable'` for non-'object' type string or zero-property object (no warning); returns `'unknown'` with `input_schema_properties_first_property_descriptor_invalid` when firstVal is not a plain object (FIFTH FROZEN CODE — carried forward from M55/M56); checks `firstVal.title` using `Object.prototype.hasOwnProperty.call(firstVal, 'title')`: absent/undefined/null → `'untitled'` (no warning; null-as-absent); non-string present non-null → `'unknown'` with `input_schema_properties_first_property_title_invalid_when_present` (SIXTH FROZEN CODE); empty string (length===0) → `'unknown'` with same SIXTH CODE (EMPTY-STRING IS-MALFORMED, deliberate alignment with M54/M56); string with length>=1 → `'titled'` (no warning; WHITESPACE-ONLY-COUNTS-AS-TITLED).
- Added `buildInputSchemaFirstPropertyTitleIndex(manifest, manifestPath)`, `formatInputSchemaFirstPropertyTitleIndex(index)`, `cmdCaption(args)`, `cmdCaptionIndex(args)`, `parseCaptionIndexArgs(args)`.
- Added `case 'caption'` dispatch between `binding` and `choice`; added `caption` between `binding` and `choice` in `printHelp()`; added `caption` and `caption index` entries in `printCommandHelp()`. CLI surface: 40→41.
- Updated all M35–M56 help-count assertions in `tests/smoke.mjs` from `!== 40` to `!== 41`.
- Added 24-case M57 smoke test matrix (a–x2/x3/x4) with synthetic fixture providing titled/untitled/not_applicable discrimination. Key M57-specific cases: (s) WHITESPACE-ONLY `title:'   '` → titled (whitespace-trim deferred to successor — contrast with potential misclassification as untitled); (t) EMPTY-STRING `title:''` → unknown WITH 6th frozen warning code (deliberate alignment with M54/M56 malformed-is-empty precedent); (q) null title → untitled (null-as-absent per M55/M56 precedent, no warning); (u) number title (42) → unknown with 6th code; (v) array title → unknown with 6th code; (g) `--caption TITLED` uppercase → exit 1 (case-sensitive enforcement).
- Added `input-schema-first-property-title-presence-index-determinism` eval scenario (47→48 scenarios) with `runInputSchemaFirstPropertyTitlePresenceIndexDeterminismScenario` handler. Scenario uses keys={z,a,b} where z has title='Subscription ID' (firstKey=z→titled, NOT sorted 'a'→untitled) to verify insertion-order semantics.
- Added `tusq caption index` section to `website/docs/cli-reference.md` with M57-vs-M52-vs-M44 noun-distinctness clarification.

**Verification:** `npm test` exits 0. Smoke tests passed. Eval regression harness passed (48 scenarios). `node bin/tusq.js help | grep -c '^  [a-z]'` → 41 (caption between binding and choice: binding(b=98)<caption(c=99,a=97)<choice(c=99,h=104)). Express fixture: get_users_api_v1_users_id and post_users_users → untitled (no title field on first property); get_users_users → not_applicable. No titled bucket on canonical fixture (absent — confirms empty-bucket-MUST-NOT-appear invariant). Zero package drift. Zero fixture mutation. All 18 M57 ROADMAP checkboxes [x]. Sixth frozen warning code `input_schema_properties_first_property_title_invalid_when_present` covers both non-string and empty-string malformations under a single consolidated code.

---

## M56 (run_4c16dd0f2f6674fc, turn_dd4dc63d5e634f5d, dev)

**Challenge to PM turn:** PM (turn_13f1b1f85e65a188) correctly bound M56: Static Capability Input Schema First Property Examples Array Presence Index Export from Manifest Evidence (~0.5 day) — V1.37 (PROPOSED) under intake charter `intent_1777380948827_537a` (vision_scan, category `roadmap_exhausted_vision_open`). git diff 531a09d..HEAD --name-only confirms PM modified exactly 4 PM-owned files (ROADMAP.md, PM_SIGNOFF.md, SYSTEM_SPEC.md, command-surface.md), zero source drift in src/bin/tests/website/package.json. All five PM decisions upheld: (1) new noun `sample` inserted between `response` and `sensitivity` (response(r=114,e=101) < sample(s=115,a=97) at pos 0 (r(114)<s(115)); sample(s=115,a=97) < sensitivity(s=115,e=101) at pos 0 same, pos 1 a(97)<e(101)); (2) four-value bucket-key enum `exampled|unexampled|not_applicable|unknown`; (3) aggregation_key enum `example_set|not_applicable|unknown` three-value; (4) bucket iteration order `exampled→unexampled→not_applicable→unknown`; (5) six frozen warning reason codes (five from M55 + SIXTH: `input_schema_properties_first_property_examples_invalid_when_present`); EMPTY-ARRAY `examples:[]` is unknown WITH 6th frozen code (deliberate alignment with M54 empty-enum-is-malformed precedent); `firstVal.examples === null` treated as absent → unexampled (null-as-absent per M55 precedent); result array field name `first_property_examples`. Challenge resolved: no new objections.

**Implementation:**
- Added `INPUT_SCHEMA_FIRST_PROPERTY_EXAMPLES_ENUM` (frozen Set: exampled/unexampled/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_EXAMPLES_AGGREGATION_KEY_ENUM` (frozen Set: example_set/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_EXAMPLES_BUCKET_ORDER` (frozen array: exampled/unexampled/not_applicable) constants in `src/cli.js` after M55 default-value constants.
- Added guard functions `_guardInputSchemaFirstPropertyExamplesBucketKey` and `_guardInputSchemaFirstPropertyExamplesAggregationKey` mirroring M35–M55 precedent.
- Added `classifyInputSchemaFirstPropertyExamples(inputSchema)` pure function: returns `'not_applicable'` for non-'object' type string or zero-property object (no warning); returns `'unknown'` with `input_schema_properties_first_property_descriptor_invalid` when firstVal is not a plain object (FIFTH FROZEN CODE — from M55); checks `firstVal.examples` using `Object.prototype.hasOwnProperty.call(firstVal, 'examples')`: absent/null/undefined → `'unexampled'` (no warning); non-empty Array → `'exampled'` (no warning); non-array OR empty array → `'unknown'` with `input_schema_properties_first_property_examples_invalid_when_present` (SIXTH FROZEN CODE — single consolidated code covers both non-array and empty-array malformations).
- Added `buildInputSchemaFirstPropertyExamplesIndex(manifest, manifestPath)`, `formatInputSchemaFirstPropertyExamplesIndex(index)`, `cmdSample(args)`, `cmdSampleIndex(args)`, `parseSampleIndexArgs(args)`.
- Added `case 'sample'` dispatch between `response` and `sensitivity`; added `sample` between `response` and `sensitivity` in `printHelp()`; added `sample` and `sample index` entries in `printCommandHelp()`. CLI surface: 39→40.
- Updated all M35–M55 help-count assertions in `tests/smoke.mjs` from `!== 39` to `!== 40`.
- Added 24-case M56 smoke test matrix (a–x) with synthetic fixture providing exampled/unexampled/not_applicable discrimination. Key M56-specific cases: (s) EMPTY-ARRAY `examples:[]` → unknown WITH 6th frozen warning code (deliberate alignment with M54 empty-enum-is-malformed precedent; contrast with M55 where default:[]→defaulted); (q) null examples → unexampled (null-as-absent per M55 precedent, no warning); (r) single-element examples → exampled; (t) string examples → unknown with 6th code; (u) object examples → unknown with 6th code; (x) `--sample EXAMPLED` uppercase → exit 1 (case-sensitive enforcement).
- Added `input-schema-first-property-examples-index-determinism` eval scenario (46→47 scenarios) with `runInputSchemaFirstPropertyExamplesIndexDeterminismScenario` handler. Scenario uses keys={z,a,b} where z has examples=['seed'] (firstKey=z→exampled, NOT sorted 'a'→unexampled) to verify insertion-order semantics.
- Added `tusq sample index` section to `website/docs/cli-reference.md`.

**Verification:** `npm test` exits 0. Smoke tests passed. Eval regression harness passed (47 scenarios). `node bin/tusq.js help | grep -c '^  [a-z]'` → 40 (sample between response and sensitivity: response(r=114,e=101)<sample(s=115,a=97)<sensitivity(s=115,e=101)). Express fixture: get_users_api_v1_users_id and post_users_users → unexampled (no examples field on first property); get_users_users → not_applicable. No exampled bucket on canonical fixture (absent — confirms empty-bucket-MUST-NOT-appear invariant). Zero package drift. Zero fixture mutation. All 18 M56 ROADMAP checkboxes [x]. Sixth frozen warning code `input_schema_properties_first_property_examples_invalid_when_present` covers both non-array and empty-array malformations under a single consolidated code.

---

## M55 (run_a75232d11566c4cb, turn_103ec2af102d2d3a, dev)

**Challenge to PM turn:** PM (turn_cb44215adb5a904f) correctly bound M55: Static Capability Input Schema First Property Default Value Presence Index Export from Manifest Evidence (~0.5 day) — V1.36 (PROPOSED) under intake charter `intent_1777378560241_6c33` (vision_scan, category `roadmap_exhausted_vision_open`). git diff confirms PM modified exactly 4 PM-owned files (ROADMAP.md, PM_SIGNOFF.md, SYSTEM_SPEC.md, command-surface.md), zero source drift in src/bin/tests/website/package.json. All five PM decisions upheld: (1) new noun `preset` inserted between `policy` and `redaction` (policy(p,o=112,111) < preset(p,r=112,114) at pos 1; preset(p,r) < redaction(r) at pos 0: p(112) < r(114)); (2) four-value bucket-key enum `defaulted|undefaulted|not_applicable|unknown`; (3) aggregation_key enum `default_value|not_applicable|unknown` three-value; (4) bucket iteration order `defaulted→undefaulted→not_applicable→unknown`; (5) five frozen warning reason codes with FIFTH being `input_schema_properties_first_property_descriptor_invalid` (formally elevated from OBJ-004/OBJ-005/OBJ-006 pattern); FALSY-DEFAULT-COUNTS-AS-DEFAULTED rule (null/false/0/''/[]/{}→defaulted); result array field name `first_property_default_values`. Challenge resolved: no new objections.

**Implementation:**
- Added `INPUT_SCHEMA_FIRST_PROPERTY_DEFAULT_VALUE_ENUM` (frozen Set: defaulted/undefaulted/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_DEFAULT_VALUE_AGGREGATION_KEY_ENUM` (frozen Set: default_value/not_applicable/unknown), `INPUT_SCHEMA_FIRST_PROPERTY_DEFAULT_VALUE_BUCKET_ORDER` (frozen array: defaulted/undefaulted/not_applicable) constants in `src/cli.js` after M54 enum-constraint constants.
- Added guard functions `_guardInputSchemaFirstPropertyDefaultValueBucketKey` and `_guardInputSchemaFirstPropertyDefaultValueAggregationKey` mirroring M35–M54 precedent.
- Added `classifyInputSchemaFirstPropertyDefaultValue(inputSchema)` pure function: returns `'not_applicable'` for non-'object' type string or zero-property object (no warning); returns `'unknown'` with `input_schema_properties_first_property_descriptor_invalid` when firstVal is not a plain object (FIFTH FROZEN CODE — formally elevated from M52/M53/M54 OBJ-004/OBJ-005/OBJ-006 undeclared pattern); uses HAS-OWN-PROPERTY-AND-NOT-UNDEFINED check (`Object.prototype.hasOwnProperty.call(firstVal, 'default') && firstVal.default !== undefined`) — FALSY-DEFAULT-COUNTS-AS-DEFAULTED (null/false/0/''/[]/{}→defaulted, deliberate divergence from M52/M53's empty-string-counts-as-absent precedent because the operator explicitly typed the falsy value as the pre-fill seed); returns `'undefaulted'` (no warning) when default key absent or present with value===undefined. No axis-specific malformation code because JSON-Schema `default` accepts ANY JSON value type.
- Added `buildInputSchemaFirstPropertyDefaultValueIndex(manifest, manifestPath)`, `formatInputSchemaFirstPropertyDefaultValueIndex(index)`, `cmdPreset(args)`, `cmdPresetIndex(args)`, `parsePresetIndexArgs(args)`.
- Added `case 'preset'` dispatch between `policy` and `redaction`; added `preset` between `policy` and `redaction` in `printHelp()`; added `preset` and `preset index` entries in `printCommandHelp()`. CLI surface: 38→39.
- Updated all M35–M54 help-count assertions in `tests/smoke.mjs` from `!== 38` to `!== 39`.
- Added 24-case M55 smoke matrix (a–x) with synthetic fixture providing defaulted/undefaulted/not_applicable discrimination. Key M55-specific cases: (t) FALSY-DEFAULT-COUNTS-AS-DEFAULTED: null/false/0/empty-string → defaulted WITH no warning (deliberate divergence from M52/M53 trim-empty-counts-as-absent); (u) absent default key → undefaulted (no warning); (v) empty-object default {} → defaulted (FALSY-DEFAULT-COUNTS-AS-DEFAULTED); (w) empty-array default [] → defaulted (contrast with M54 where enum:[]→unknown — here default:[] is a valid pre-fill seed).
- Added `input-schema-first-property-default-value-presence-index-determinism` eval scenario (45→46 scenarios) with `runInputSchemaFirstPropertyDefaultValuePresenceIndexDeterminismScenario` handler. Scenario uses keys={z,a,b} where z has default='seed' (firstKey=z→defaulted, NOT sorted 'a'→undefaulted) to verify insertion-order semantics.
- Added `tusq preset index` section to `website/docs/cli-reference.md`.

**Verification:** `npm test` exits 0. Smoke tests passed. Eval regression harness passed (46 scenarios). Module loads cleanly. `node bin/tusq.js help | grep -c '^  [a-z]'` → 39 (preset between policy and redaction: policy(p,o)<preset(p,r)<redaction(r)). Express fixture: get_users_api_v1_users_id and post_users_users → undefaulted (no default field on first property); get_users_users → not_applicable (zero-property object, no warning). No defaulted bucket on canonical fixture (absent — express sample has no JSON-Schema default values on first properties — confirms empty-bucket-MUST-NOT-appear invariant). Zero package drift. Zero fixture mutation. All 16 M55 ROADMAP checkboxes [x]. M55's fifth frozen warning code `input_schema_properties_first_property_descriptor_invalid` formally retires the OBJ-004/OBJ-005/OBJ-006 "undeclared sixth code" pattern from M52/M53/M54.
