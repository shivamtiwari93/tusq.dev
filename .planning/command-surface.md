...
> **M67 Charter Sketch Reservation — 2026-04-28, run_a32ec6b13eb1a938, turn_87000dff8c907d84, PM attempt 1.** Reserves the M67 boundary for the dev turn to materialize. `tusq above index` to be implemented. CLI surface 50→51. `above` inserted as the FIRST entry in the post-`docs` alphabetical block, immediately before `approve` (`above` (a=97, b=98) < `approve` (a=97, p=112) at position 1 (`b` (98) < `p` (112))). Flags: `--manifest`, `--json`, `--out` (rejects `.tusq/`), `--above` (case-sensitive lowercase only). Output: `first_property_exclusive_minimum_states[]` with 8-field per-bucket shape (`input_schema_first_property_exclusive_minimum`, `aggregation_key`, `capability_count`, `capabilities[]`, `approved_count`, `gated_count`, `has_destructive_side_effect`, `has_restricted_or_confidential_sensitivity`). Bucket-key enum (closed four-value): `lower_exclusive_bounded | lower_exclusive_unbounded | not_applicable | unknown`. Aggregation_key enum (closed three-value): `numeric_lower_exclusive_bound_constraint | not_applicable | unknown`. Six frozen warning reason codes (fifth: `input_schema_properties_first_property_descriptor_invalid`; sixth: `input_schema_properties_first_property_exclusive_minimum_invalid_when_present`). NULL-AS-ABSENT: `exclusiveMinimum: null` → `lower_exclusive_unbounded` (no warning, mirrors M55–M66). ZERO-IS-VALID-EXCLUSIVE-LOWER-BOUND (M67-SPECIFIC, mirrors M65): `exclusiveMinimum: 0` → `lower_exclusive_bounded` (no warning). NEGATIVE-IS-VALID-EXCLUSIVE-LOWER-BOUND (M67-SPECIFIC, mirrors M65): `exclusiveMinimum: -273.15` → `lower_exclusive_bounded` (no warning). FRACTIONAL-IS-VALID-EXCLUSIVE-LOWER-BOUND (M67-SPECIFIC): `exclusiveMinimum: 0.5` / `exclusiveMinimum: -0.001` → `lower_exclusive_bounded` (no warning). DRAFT-4-BOOLEAN-IS-INVALID (M67-SPECIFIC): `exclusiveMinimum: true` / `exclusiveMinimum: false` → `unknown` WITH 6th code (M67 reads only the Draft 6+ finite-number form; cross-Draft polyfill deferred to `M-Above-Draft4-Crossref-1`). STRICT: `typeof v === 'number' && Number.isFinite(v)` — NO `Number()`/`parseFloat()`/truthy coercion (NOT `>= 0`, NOT `> 0`, NOT `Number.isInteger`). NaN/Infinity/-Infinity/non-numeric/Draft-4-boolean → unknown WITH 6th code. Bucket iteration order: `lower_exclusive_bounded → lower_exclusive_unbounded → not_applicable → unknown` (deterministic stable-output convention only — NOT an ecosystem-integration-compiler-criticality / Slack-slash-command-priority / Teams-action-input-strictness / Zendesk-Intercom-Salesforce-HubSpot-Gainsight-form-validator-priority / OpenAI-Claude-MCP-tool-call-wrapper-pre-validation-strictness / marketplace-package-strictness-tier / ecosystem-adapter-deprecation-priority / packaged-tool-fallback-guidance-strength ranking). Empty buckets MUST NOT appear. Read-only invariant: 37 prior peer index commands byte-identical pre/post (`tusq surface plan`, `tusq domain index`, `tusq effect index`, `tusq sensitivity index`, `tusq method index`, `tusq auth index`, `tusq confidence index`, `tusq pii index`, `tusq examples index`, `tusq input index`, `tusq output index`, `tusq path index`, `tusq response index`, `tusq request index`, `tusq description index`, `tusq items index`, `tusq strictness index`, `tusq parameter index`, `tusq shape index`, `tusq signature index`, `tusq obligation index`, `tusq binding index`, `tusq gloss index`, `tusq hint index`, `tusq choice index`, `tusq preset index`, `tusq sample index`, `tusq caption index`, `tusq legacy index`, `tusq regex index`, `tusq seal index`, `tusq secret index`, `tusq floor index`, `tusq ceiling index`, `tusq divisor index`, `tusq lower index`, `tusq upper index`). Non-persistence: `input_schema_first_property_exclusive_minimum` MUST NOT be written into `tusq.manifest.json`. Dev turn replaces this reservation with a full `### tusq above index` entry mirroring M66's `### tusq upper index` entry — synopsis, flags table, output shape, exit codes, peer-index read-only invariant note. See ROADMAP.md § M67 for the full PM-frozen scope.
