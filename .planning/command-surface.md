...
> **M70 Charter Sketch Reservation — 2026-04-28, run_7aa3510dbb544d16, turn_47f0529338159ca6, PM attempt 1.** Reserves the new top-level CLI noun `wire` for the M70 single-subcommand `tusq wire index` (input-schema first-property `contentEncoding` string-content-transfer-encoding annotation presence index). CLI surface 53 → 54. `wire` is inserted alphabetically between `version` and `help` in the post-`docs` block (`version` (v=118) < `wire` (w=119) at position 0; `wire` is the LAST alphabetical noun before the trailing special `help` entry). Flags (planned): `--manifest`, `--json`, `--out`, `--wire` (case-sensitive lowercase, closed four-value enum `encoded | unencoded | not_applicable | unknown`). Output (planned): `first_property_content_encoding_states[]` with 8-field per-bucket shape (`input_schema_first_property_content_encoding`, `aggregation_key`, `capability_count`, `capabilities[]`, `approved_count`, `gated_count`, `has_destructive_side_effect`, `has_restricted_or_confidential_sensitivity`). Six frozen warning reason codes (5 baseline + axis-specific 6th `input_schema_properties_first_property_content_encoding_invalid_when_present` for non-string `contentEncoding` malformations). M70-SPECIFIC rules: NULL-AS-ABSENT (`contentEncoding: null` → `unencoded`), EMPTY-STRING-AS-ABSENT (`contentEncoding: ''` → `unencoded`; deliberate divergence from M69's FALSY-IS-VALID-CONST), TYPE-APPLICABILITY-STRING (firstVal.type !== `'string'` → `not_applicable`), DRAFT-7-STRING-IS-VALID-CONTENT-ENCODING (non-string → `unknown` WITH 6th code; NO-COERCION), ANY-NON-EMPTY-STRING-IS-ENCODED (non-canonical strings still `encoded`; canonical RFC-2045/RFC-4648 validation deferred to `M-Wire-Canonical-Set-Validator-1`). Bucket iteration order: `encoded → unencoded → not_applicable → unknown`. The full `### tusq wire index` entry will replace this reservation block at dev materialization. See ROADMAP.md § M70 for full scope.
