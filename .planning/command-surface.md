...
> **M63 Charter Sketch Reservation — 2026-04-28, run_e29754fde9f7e4f7, turn_9b08017116bfbfdc, PM attempt 1.** Reserves the `### tusq ceiling index` entry for the dev materialization turn. PM-frozen surface delta (dev MUST carry forward verbatim): CLI surface 46 → 47 commands; new top-level noun `ceiling` with single subcommand `index`, inserted alphabetically between `caption` and `choice` in the post-`docs` block (`caption` (c=99, a=97) < `ceiling` (c=99, e=101) at position 1 (`a` (97) < `e` (101)); `ceiling` (c=99, e=101) < `choice` (c=99, h=104) at position 1 (`e` (101) < `h` (104))); insertion sequence `..., binding, caption, ceiling, choice, confidence, ...`. The new `tusq ceiling index` command synopsis: aggregate capabilities by `input_schema.properties[firstKey].maxLength` JSON-Schema string-length-ceiling-annotation presence (a JSON-Schema Draft 4+ non-negative-integer validation keyword), exporting a per-bucket index with the closed four-value bucket-key enum `length_capped | length_uncapped | not_applicable | unknown`. Flags: `--manifest <path>` (default `./tusq.manifest.json`), `--json`, `--out <path>` (rejects paths starting with `.tusq/`), `--ceiling <value>` (case-sensitive lowercase; closed four-value enum). Output shape: `first_property_max_length_states[]` (one entry per non-empty bucket; empty buckets MUST NOT appear) with 8-field per-bucket entry shape (`input_schema_first_property_max_length, aggregation_key, capability_count, capabilities[], approved_count, gated_count, has_destructive_side_effect, has_restricted_or_confidential_sensitivity`). Top-level `warnings[]` always present in `--json` mode. Exit codes: 0 on success; 1 on unknown subcommand, unknown flag, missing manifest, malformed JSON, missing capabilities array, --ceiling no value, --ceiling value not in closed four-value set, --ceiling value with absent bucket, --out path starting with `.tusq/`, --out unwritable. M63-vs-M62 distinctness: `ceiling` reads firstKey.maxLength (non-negative-integer numeric UPPER-bound — JSON-Schema Draft 4+ numeric-bound validation); `floor` reads firstKey.minLength (non-negative-integer numeric LOWER-bound — JSON-Schema Draft 4+ numeric-bound validation) — orthogonal JSON-Schema string-length-bound keywords, read-only-invariant peers. M63-vs-M59 distinctness: `ceiling` reads firstKey.maxLength (numeric integer ceiling); `regex` reads firstKey.pattern (regex string — shape-bound) — orthogonal JSON-Schema string-validation keywords, read-only-invariant peers. NULL-AS-ABSENT: maxLength:null → length_uncapped (no warning). EXPLICIT-ZERO-IS-CAPPED: maxLength:0 → length_capped (no warning, mirrors M58/M60/M61/M62 explicit-default-value precedent for boundary axes; `0` is a valid non-negative integer per JSON-Schema). NO-TYPE-COERCION: maxLength:'1'/'0'/true/false/[1]/{}/-1/1.5/NaN/Infinity → unknown WITH 6th code (`Number.isInteger() && >= 0` strict predicate). SIXTH frozen warning code `input_schema_properties_first_property_max_length_invalid_when_present` covers ALL non-non-negative-integer malformations under a single consolidated code. The dev turn replaces this reservation block with the full `### tusq ceiling index` entry detailing materialized synopsis, flags table, output shape examples, exit codes, and the 33-peer-index read-only invariant note.
