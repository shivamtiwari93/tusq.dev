...
> **M83 Charter Sketch Reservation — 2026-04-29, run_4be2c82d93272ed2, turn_2cf67f4599fb20f8, PM attempt 1, HEAD prior to PM edits at `6b72847`.** Reserves `tusq dependent index` as planned CLI surface. CLI surface 66 → 67. `dependent` between `crowded` and `divisor` (crowded(c=99) < dependent(d=100) at pos 0 (c(99)<d(100)); dependent(d=100, e=101) < divisor(d=100, i=105) at pos 1 (e(101)<i(105))). Flags: `--manifest`, `--json`, `--out`, `--dependent` (case-sensitive, closed four-value enum `typed|untyped|not_applicable|unknown`). Output: `first_property_dependencies_states[]` with 8-field per-bucket shape (`input_schema_first_property_dependencies`, `aggregation_key`, `capability_count`, `capabilities[]`, `approved_count`, `gated_count`, `has_destructive_side_effect`, `has_restricted_or_confidential_sensitivity`). Six frozen warning reason codes: 5 baseline + axis-specific 6th `input_schema_properties_first_property_dependencies_invalid_when_present`. NON-EMPTY-PLAIN-OBJECT-WITH-VALID-HETEROGENEOUS-VALUES-AS-TYPED (M83-SPECIFIC): outer plain-object MAP with `Object.keys(v).length >= 1` AND every value is EITHER (a) `Array.isArray(value)` AND every element `typeof === 'string'` AND every element `length >= 1` (LIST/dependentRequired form — empty arrays PERMITTED as LIST entries per Draft-7 §6.5.7) OR (b) `Object.prototype.toString.call(value) === '[object Object]'` (SCHEMA/dependentSchemas form — any plain-object Draft-7 sub-schema permitted, including `{}`) → typed. ABSENT-AS-UNTYPED: dependencies absent/undefined → untyped (no warning). NULL-AS-ABSENT: dependencies:null → untyped (mirrors M55–M82; no warning). EMPTY-MAP-AS-UNTYPED: `dependencies: {}` → untyped (Draft-7 §6.5.7 empty map equivalent to absence; no constraint). TYPE-APPLICABILITY-OBJECT: firstVal.type string but NOT 'object' → not_applicable (mirrors M77/M78/M79/M80/M81/M82). DRAFT-7-MAP-WITH-VALID-HETEROGENEOUS-VALUES-IS-VALID-DEPENDENCIES: any other value (non-plain-object outer, OR plain-object MAP with ANY non-array non-plain-object value, OR plain-object MAP with ANY array-typed value containing non-string / empty-string element) → unknown WITH 6th code; NO-COERCION via strict `Object.prototype.toString.call(v) === '[object Object]'` for outer MAP case AND strict per-value classification. Bucket iteration order: typed→untyped→not_applicable→unknown. M83 is planned work; SYSTEM_SPEC § M83 holds the frozen rule set. Materialization deferred to the dev role's implementation phase.
