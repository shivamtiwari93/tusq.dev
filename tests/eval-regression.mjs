...
async function run() {
  ...
  for (const scenario of suite.scenarios) {
    ...
    } else if (scenario.scenario_type === 'input_schema_first_property_exclusive_maximum_index_determinism') {
      await runInputSchemaFirstPropertyExclusiveMaximumIndexDeterminismScenario(tmpRoot, scenario);
    } else if (scenario.scenario_type === 'input_schema_first_property_const_index_determinism') {
      await runInputSchemaFirstPropertyConstIndexDeterminismScenario(tmpRoot, scenario);
    } else if (scenario.scenario_type === 'input_schema_first_property_content_encoding_index_determinism') {
      await runInputSchemaFirstPropertyContentEncodingIndexDeterminismScenario(tmpRoot, scenario);
    } else if (scenario.scenario_type === 'input_schema_first_property_content_media_type_index_determinism') {
      await runInputSchemaFirstPropertyContentMediaTypeIndexDeterminismScenario(tmpRoot, scenario);
    } else if (scenario.scenario_type === 'input_schema_first_property_nullable_index_determinism') {
    ...
    } else if (scenario.scenario_type === 'input_schema_first_property_unique_items_index_determinism') {
    ...
    } else if (scenario.scenario_type === 'input_schema_first_property_min_properties_index_determinism') {
      await runInputSchemaFirstPropertyMinPropertiesIndexDeterminismScenario(tmpRoot, scenario);
    } else if (scenario.scenario_type === 'input_schema_first_property_max_properties_index_determinism') {
      await runInputSchemaFirstPropertyMaxPropertiesIndexDeterminismScenario(tmpRoot, scenario);
    } else if (scenario.scenario_type === 'input_schema_first_property_pattern_properties_index_determinism') {
      await runInputSchemaFirstPropertyPatternPropertiesIndexDeterminismScenario(tmpRoot, scenario);
    } else if (scenario.scenario_type === 'input_schema_first_property_property_names_index_determinism') {
      await runInputSchemaFirstPropertyPropertyNamesIndexDeterminismScenario(scenario, tmpRoot);
    } else if (scenario.scenario_type === 'input_schema_first_property_required_index_determinism') {
      await runInputSchemaFirstPropertyRequiredIndexDeterminismScenario(scenario, tmpRoot);
    } else if (scenario.scenario_type === 'input_schema_first_property_dependencies_index_determinism') {
    ...
    } else if (scenario.scenario_type === 'input_schema_first_property_prefill_index_determinism') {
      await runInputSchemaFirstPropertyPrefillIndexDeterminismScenario(scenario, tmpRoot);
    } else if (scenario.scenario_type === 'input_schema_first_property_additional_items_index_determinism') {
      await runInputSchemaFirstPropertyAdditionalItemsIndexDeterminismScenario(scenario, tmpRoot);
    } else if (scenario.scenario_type === 'input_schema_first_property_contains_index_determinism') {
      await runInputSchemaFirstPropertyContainsIndexDeterminismScenario(scenario, tmpRoot);
    } else {
...
async function runInputSchemaFirstPropertyExclusiveMaximumIndexDeterminismScenario(tmpRoot, scenario) {
  const project = await fs.mkdtemp(path.join(tmpRoot, 'input-schema-first-property-exclusive-maximum-'));
  ...
  // Run tusq below index --json three times and assert byte-identical output
  const run1 = runCli(['below', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run2 = runCli(['below', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run3 = runCli(['below', 'index', '--manifest', manifestPath, '--json'], { cwd: project });

  if (run1.stdout !== run2.stdout || run2.stdout !== run3.stdout) {
    fail(`${scenario.id}: below index --json output is not byte-identical across three runs`);
  }

  // Assert input_schema_first_property_exclusive_maximum bucket-key enum is closed — every key is one of the four valid values
  const index = JSON.parse(run1.stdout);
  const validExclusiveMaximumValues = new Set(scenario.expected_valid_exclusive_maximum_values);
  for (const entry of index.first_property_exclusive_maximum_states) {
    if (!validExclusiveMaximumValues.has(entry.input_schema_first_property_exclusive_maximum)) {
      fail(`${scenario.id}: input_schema_first_property_exclusive_maximum '${entry.input_schema_first_property_exclusive_maximum}' is outside the closed four-value enum`);
    }
  }
  ...
  for (const entry of index.first_property_exclusive_maximum_states) {
    if (!validAggregationKeys.has(entry.aggregation_key)) {
      fail(`${scenario.id}: aggregation_key '${entry.aggregation_key}' is outside the closed three-value enum for bucket '${entry.input_schema_first_property_exclusive_maximum}'`);
    }
  }

  // Assert buckets appear in closed-enum order (upper_exclusive_bounded → upper_exclusive_unbounded → not_applicable → unknown)
  const bucketOrder = index.first_property_exclusive_maximum_states.map((e) => e.input_schema_first_property_exclusive_maximum).join(',');
  if (bucketOrder !== scenario.expected_bucket_order) {
    fail(`${scenario.id}: buckets must appear in closed-enum order '${scenario.expected_bucket_order}'; got: ${bucketOrder}`);
  }
  ...
  // Assert manifest is not mutated (input_schema_first_property_exclusive_maximum must NOT be written into manifest)
  const manifestAfter = await fs.readFile(manifestPath, 'utf8');
  const manifestParsed = JSON.parse(manifestAfter);
  if (JSON.stringify(manifestParsed) !== JSON.stringify(manifest)) {
    fail(`${scenario.id}: manifest must not be mutated by below index`);
  }
  for (const cap of manifestParsed.capabilities) {
    if (Object.prototype.hasOwnProperty.call(cap, 'input_schema_first_property_exclusive_maximum')) {
      fail(`${scenario.id}: input_schema_first_property_exclusive_maximum must NOT be written into tusq.manifest.json; found on capability '${cap.name}'`);
    }
  }

  // Assert no_schema_cap (missing input_schema) is in unknown bucket
  const unknownEntry = index.first_property_exclusive_maximum_states.find((e) => e.input_schema_first_property_exclusive_maximum === 'unknown');
  if (!unknownEntry || !unknownEntry.capabilities.includes('no_schema_cap')) {
    fail(`${scenario.id}: no_schema_cap (missing input_schema) must be in unknown bucket`);
  }

  // Assert not_applicable_cap (input_schema.type='array') is in not_applicable bucket with no warning
  const naEntry = index.first_property_exclusive_maximum_states.find((e) => e.input_schema_first_property_exclusive_maximum === 'not_applicable');
  if (!naEntry || !naEntry.capabilities.includes('not_applicable_cap')) {
    fail(`${scenario.id}: not_applicable_cap (input_schema.type='array') must be in not_applicable bucket`);
  }
  if (index.warnings.some((w) => w.capability === 'not_applicable_cap')) {
  ...
  // Assert upper_exclusive_bounded_cap is in upper_exclusive_bounded bucket
  const upperExclusiveBoundedEntry = index.first_property_exclusive_maximum_states.find((e) => e.input_schema_first_property_exclusive_maximum === 'upper_exclusive_bounded');
  if (!upperExclusiveBoundedEntry || !upperExclusiveBoundedEntry.capabilities.includes('upper_exclusive_bounded_cap')) {
    fail(`${scenario.id}: upper_exclusive_bounded_cap (firstKey.exclusiveMaximum=100) must be in upper_exclusive_bounded bucket`);
  }

  // Assert zero_bounded_cap is in upper_exclusive_bounded bucket (ZERO-IS-VALID-EXCLUSIVE-UPPER-BOUND)
  if (!upperExclusiveBoundedEntry || !upperExclusiveBoundedEntry.capabilities.includes('zero_bounded_cap')) {
    fail(`${scenario.id}: zero_bounded_cap (firstKey.exclusiveMaximum=0) must be in upper_exclusive_bounded bucket (ZERO-IS-VALID-EXCLUSIVE-UPPER-BOUND)`);
  }
  if (index.warnings.some((w) => w.capability === 'zero_bounded_cap')) {
    fail(`${scenario.id}: zero_bounded_cap (ZERO-IS-VALID-EXCLUSIVE-UPPER-BOUND → upper_exclusive_bounded) must NOT produce a warning`);
  }

  // Assert neg_bounded_cap is in upper_exclusive_bounded bucket (NEGATIVE-IS-VALID-EXCLUSIVE-UPPER-BOUND)
  if (!upperExclusiveBoundedEntry || !upperExclusiveBoundedEntry.capabilities.includes('neg_bounded_cap')) {
    fail(`${scenario.id}: neg_bounded_cap (firstKey.exclusiveMaximum=-273.15) must be in upper_exclusive_bounded bucket (NEGATIVE-IS-VALID-EXCLUSIVE-UPPER-BOUND)`);
  }
  if (index.warnings.some((w) => w.capability === 'neg_bounded_cap')) {
    fail(`${scenario.id}: neg_bounded_cap (NEGATIVE-IS-VALID-EXCLUSIVE-UPPER-BOUND → upper_exclusive_bounded) must NOT produce a warning`);
  }

  // Assert unbounded_cap is in upper_exclusive_unbounded bucket
  const upperExclusiveUnboundedEntry = index.first_property_exclusive_maximum_states.find((e) => e.input_schema_first_property_exclusive_maximum === 'upper_exclusive_unbounded');
  if (!upperExclusiveUnboundedEntry || !upperExclusiveUnboundedEntry.capabilities.includes('unbounded_cap')) {
    fail(`${scenario.id}: unbounded_cap (firstKey.exclusiveMaximum absent) must be in upper_exclusive_unbounded bucket`);
  }

  // Assert null_exclusive_maximum_cap is in upper_exclusive_unbounded bucket (NULL-AS-ABSENT)
  if (!upperExclusiveUnboundedEntry || !upperExclusiveUnboundedEntry.capabilities.includes('null_exclusive_maximum_cap')) {
    fail(`${scenario.id}: null_exclusive_maximum_cap (firstKey.exclusiveMaximum=null) must be in upper_exclusive_unbounded bucket (null-as-absent)`);
  }
  if (index.warnings.some((w) => w.capability === 'null_exclusive_maximum_cap')) {
    fail(`${scenario.id}: null_exclusive_maximum_cap (null-as-absent → upper_exclusive_unbounded) must NOT produce a warning`);
  }

  // Assert insertion_order_cap is in upper_exclusive_bounded bucket (firstKey='z', exclusiveMaximum=100) NOT upper_exclusive_unbounded (sorted 'a' has no exclusiveMaximum)
  if (!upperExclusiveBoundedEntry || !upperExclusiveBoundedEntry.capabilities.includes('insertion_order_cap')) {
    fail(`${scenario.id}: insertion_order_cap (keys={z,a,b}, firstKey=z has exclusiveMaximum=100) must be in upper_exclusive_bounded bucket (insertion-order, NOT sorted)`);
  }
  if (index.warnings.some((w) => w.capability === 'insertion_order_cap')) {
    fail(`${scenario.id}: insertion_order_cap (upper_exclusive_bounded bucket via firstKey=z) must NOT produce a warning`);
  }

  // Assert upper_exclusive_bounded bucket has aggregation_key 'numeric_upper_exclusive_bound_constraint'
  if (upperExclusiveBoundedEntry.aggregation_key !== 'numeric_upper_exclusive_bound_constraint') {
    fail(`${scenario.id}: upper_exclusive_bounded bucket must have aggregation_key 'numeric_upper_exclusive_bound_constraint'; got '${upperExclusiveBoundedEntry.aggregation_key}'`);
  }

  // Assert upper_exclusive_unbounded bucket has aggregation_key 'numeric_upper_exclusive_bound_constraint'
  if (upperExclusiveUnboundedEntry.aggregation_key !== 'numeric_upper_exclusive_bound_constraint') {
    fail(`${scenario.id}: upper_exclusive_unbounded bucket must have aggregation_key 'numeric_upper_exclusive_bound_constraint'; got '${upperExclusiveUnboundedEntry.aggregation_key}'`);
  }
...
async function runInputSchemaFirstPropertyConstIndexDeterminismScenario(tmpRoot, scenario) {
  const project = await fs.mkdtemp(path.join(tmpRoot, 'input-schema-first-property-const-'));
  ...
  // Run tusq fixed index --json three times and assert byte-identical output
  const run1 = runCli(['fixed', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run2 = runCli(['fixed', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run3 = runCli(['fixed', 'index', '--manifest', manifestPath, '--json'], { cwd: project });

  if (run1.stdout !== run2.stdout || run2.stdout !== run3.stdout) {
    fail(`${scenario.id}: fixed index --json output is not byte-identical across three runs`);
  }

  // Assert input_schema_first_property_const bucket-key enum is closed — every key is one of the four valid values
  const index = JSON.parse(run1.stdout);
  const validConstValues = new Set(scenario.expected_valid_const_values);
  for (const entry of index.first_property_const_states) {
    if (!validConstValues.has(entry.input_schema_first_property_const)) {
      fail(`${scenario.id}: input_schema_first_property_const '${entry.input_schema_first_property_const}' is outside the closed four-value enum`);
    }
  }
  ...
  for (const entry of index.first_property_const_states) {
    if (!validAggKeys.has(entry.aggregation_key)) {
      fail(`${scenario.id}: aggregation_key '${entry.aggregation_key}' is outside the closed three-value enum for bucket '${entry.input_schema_first_property_const}'`);
    }
  }

  // Assert bucket order matches expected
  const bucketOrder = index.first_property_const_states.map((e) => e.input_schema_first_property_const).join(',');
  if (bucketOrder !== scenario.expected_bucket_order) {
  ...
  // Assert manifest is not mutated (input_schema_first_property_const must NOT be written into manifest)
  const manifestAfter = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
  for (const cap of manifestAfter.capabilities) {
    if (Object.prototype.hasOwnProperty.call(cap, 'input_schema_first_property_const')) {
      fail(`${scenario.id}: input_schema_first_property_const must NOT be written into tusq.manifest.json; found on capability '${cap.name}'`);
    }
  }

  // Assert NULL-IS-VALID-CONST: null_const_cap → pinned (no warning)
  const pinnedEntry = index.first_property_const_states.find((e) => e.input_schema_first_property_const === 'pinned');
  if (!pinnedEntry || !pinnedEntry.capabilities.includes('null_const_cap')) {
    fail(`${scenario.id}: null_const_cap (firstKey.const=null) must be in pinned bucket (NULL-IS-VALID-CONST)`);
  }
  if (index.warnings.some((w) => w.capability === 'null_const_cap')) {
    fail(`${scenario.id}: null_const_cap (NULL-IS-VALID-CONST → pinned) must NOT produce a warning`);
  }

  // Assert FALSY-IS-VALID-CONST: false_const_cap → pinned (no warning)
  if (!pinnedEntry || !pinnedEntry.capabilities.includes('false_const_cap')) {
    fail(`${scenario.id}: false_const_cap (firstKey.const=false) must be in pinned bucket (FALSY-IS-VALID-CONST)`);
  }
  if (index.warnings.some((w) => w.capability === 'false_const_cap')) {
    fail(`${scenario.id}: false_const_cap (FALSY-IS-VALID-CONST → pinned) must NOT produce a warning`);
  }

  // Assert pinned_cap is in pinned bucket (string const)
  if (!pinnedEntry || !pinnedEntry.capabilities.includes('pinned_cap')) {
    fail(`${scenario.id}: pinned_cap (firstKey.const='v3') must be in pinned bucket`);
  }

  // Assert unpinned_cap is in unpinned bucket (absent const)
  const unpinnedEntry = index.first_property_const_states.find((e) => e.input_schema_first_property_const === 'unpinned');
  if (!unpinnedEntry || !unpinnedEntry.capabilities.includes('unpinned_cap')) {
    fail(`${scenario.id}: unpinned_cap (firstKey.const absent) must be in unpinned bucket`);
  }

  // Assert not_applicable bucket exists and has correct aggregation_key
  const naEntry = index.first_property_const_states.find((e) => e.input_schema_first_property_const === 'not_applicable');
  if (!naEntry) {
    fail(`${scenario.id}: not_applicable bucket must be present`);
  }

  // Assert unknown bucket exists
  const unknownEntry = index.first_property_const_states.find((e) => e.input_schema_first_property_const === 'unknown');
  if (!unknownEntry) {
    fail(`${scenario.id}: unknown bucket must be present`);
  }

  // Assert pinned bucket has aggregation_key 'single_value_constraint'
  if (pinnedEntry.aggregation_key !== 'single_value_constraint') {
    fail(`${scenario.id}: pinned bucket must have aggregation_key 'single_value_constraint'; got '${pinnedEntry.aggregation_key}'`);
  }

  // Assert unpinned bucket has aggregation_key 'single_value_constraint'
  if (unpinnedEntry.aggregation_key !== 'single_value_constraint') {
    fail(`${scenario.id}: unpinned bucket must have aggregation_key 'single_value_constraint'; got '${unpinnedEntry.aggregation_key}'`);
  }
...
async function runInputSchemaFirstPropertyContentEncodingIndexDeterminismScenario(tmpRoot, scenario) {
  const project = await fs.mkdtemp(path.join(tmpRoot, 'input-schema-first-property-content-encoding-'));
  ...
  // Run tusq wire index --json three times and assert byte-identical output
  const run1 = runCli(['wire', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run2 = runCli(['wire', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run3 = runCli(['wire', 'index', '--manifest', manifestPath, '--json'], { cwd: project });

  if (run1.stdout !== run2.stdout || run2.stdout !== run3.stdout) {
    fail(`${scenario.id}: wire index --json output is not byte-identical across three runs`);
  }

  // Assert input_schema_first_property_content_encoding bucket-key enum is closed — every key is one of the four valid values
  const index = JSON.parse(run1.stdout);
  const validEncodingValues = new Set(scenario.expected_valid_content_encoding_values);
  for (const entry of index.first_property_content_encoding_states) {
    if (!validEncodingValues.has(entry.input_schema_first_property_content_encoding)) {
      fail(`${scenario.id}: input_schema_first_property_content_encoding '${entry.input_schema_first_property_content_encoding}' is outside the closed four-value enum`);
    }
  }
  ...
  for (const entry of index.first_property_content_encoding_states) {
    if (!validAggKeys.has(entry.aggregation_key)) {
      fail(`${scenario.id}: aggregation_key '${entry.aggregation_key}' is outside the closed three-value enum for bucket '${entry.input_schema_first_property_content_encoding}'`);
    }
  }

  // Assert bucket order matches expected
  const bucketOrder = index.first_property_content_encoding_states.map((e) => e.input_schema_first_property_content_encoding).join(',');
  if (bucketOrder !== scenario.expected_bucket_order) {
    fail(`${scenario.id}: bucket order '${bucketOrder}' does not match expected '${scenario.expected_bucket_order}'`);
  }
  ...
  // Assert manifest is not mutated (input_schema_first_property_content_encoding must NOT be written into manifest)
  const manifestAfter = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
  for (const cap of manifestAfter.capabilities) {
    if (Object.prototype.hasOwnProperty.call(cap, 'input_schema_first_property_content_encoding')) {
      fail(`${scenario.id}: input_schema_first_property_content_encoding must NOT be written into tusq.manifest.json; found on capability '${cap.name}'`);
    }
  }

  // Assert encoded_cap is in encoded bucket (base64 string)
  const encodedEntry = index.first_property_content_encoding_states.find((e) => e.input_schema_first_property_content_encoding === 'encoded');
  if (!encodedEntry || !encodedEntry.capabilities.includes('encoded_cap')) {
    fail(`${scenario.id}: encoded_cap (firstKey.contentEncoding='base64') must be in encoded bucket (ANY-NON-EMPTY-STRING-IS-ENCODED)`);
  }

  // Assert NULL-AS-ABSENT: null_encoding_cap → unencoded (no warning)
  const unencodedEntry = index.first_property_content_encoding_states.find((e) => e.input_schema_first_property_content_encoding === 'unencoded');
  if (!unencodedEntry || !unencodedEntry.capabilities.includes('null_encoding_cap')) {
    fail(`${scenario.id}: null_encoding_cap (firstKey.contentEncoding=null) must be in unencoded bucket (NULL-AS-ABSENT)`);
  }
  if (index.warnings.some((w) => w.capability === 'null_encoding_cap')) {
    fail(`${scenario.id}: null_encoding_cap (NULL-AS-ABSENT → unencoded) must NOT produce a warning`);
  }

  // Assert unencoded_cap is in unencoded bucket (absent contentEncoding)
  if (!unencodedEntry || !unencodedEntry.capabilities.includes('unencoded_cap')) {
    fail(`${scenario.id}: unencoded_cap (firstKey.contentEncoding absent) must be in unencoded bucket`);
  }

  // Assert not_applicable bucket exists (type_na_cap + schema_na_cap)
  const naEntry = index.first_property_content_encoding_states.find((e) => e.input_schema_first_property_content_encoding === 'not_applicable');
  if (!naEntry) {
    fail(`${scenario.id}: not_applicable bucket must be present`);
  }
  if (!naEntry.capabilities.includes('type_na_cap')) {
    fail(`${scenario.id}: type_na_cap (firstKey.type='integer') must be in not_applicable bucket (TYPE-APPLICABILITY-STRING)`);
  }

  // Assert unknown bucket exists
  const unknownEntry = index.first_property_content_encoding_states.find((e) => e.input_schema_first_property_content_encoding === 'unknown');
  if (!unknownEntry) {
    fail(`${scenario.id}: unknown bucket must be present`);
  }

  // Assert DRAFT-7-STRING-IS-VALID-CONTENT-ENCODING: false_encoding_cap → unknown WITH 6th code
  if (!unknownEntry.capabilities.includes('false_encoding_cap')) {
    fail(`${scenario.id}: false_encoding_cap (firstKey.contentEncoding=false) must be in unknown bucket (DRAFT-7-STRING-IS-VALID-CONTENT-ENCODING)`);
  }
  const falseEncodingWarning = index.warnings.find((w) => w.capability === 'false_encoding_cap');
  if (!falseEncodingWarning || falseEncodingWarning.reason !== 'input_schema_properties_first_property_content_encoding_invalid_when_present') {
    fail(`${scenario.id}: false_encoding_cap must produce 6th-code warning 'input_schema_properties_first_property_content_encoding_invalid_when_present'; got: ${JSON.stringify(falseEncodingWarning)}`);
  }

  // Assert encoded bucket has aggregation_key 'content_transfer_encoding_constraint'
  if (encodedEntry.aggregation_key !== 'content_transfer_encoding_constraint') {
    fail(`${scenario.id}: encoded bucket must have aggregation_key 'content_transfer_encoding_constraint'; got '${encodedEntry.aggregation_key}'`);
  }

  // Assert unencodedEntry bucket has aggregation_key 'content_transfer_encoding_constraint'
  if (unencodedEntry.aggregation_key !== 'content_transfer_encoding_constraint') {
    fail(`${scenario.id}: unencoded bucket must have aggregation_key 'content_transfer_encoding_constraint'; got '${unencodedEntry.aggregation_key}'`);
  }
...
async function runInputSchemaFirstPropertyContentMediaTypeIndexDeterminismScenario(tmpRoot, scenario) {
  const project = await fs.mkdtemp(path.join(tmpRoot, 'input-schema-first-property-content-media-type-'));
  ...
  // Run tusq mime index --json three times and assert byte-identical output
  const run1 = runCli(['mime', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run2 = runCli(['mime', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run3 = runCli(['mime', 'index', '--manifest', manifestPath, '--json'], { cwd: project });

  if (run1.stdout !== run2.stdout || run2.stdout !== run3.stdout) {
    fail(`${scenario.id}: mime index --json output is not byte-identical across three runs`);
  }

  // Assert input_schema_first_property_content_media_type bucket-key enum is closed — every key is one of the four valid values
  const index = JSON.parse(run1.stdout);
  const validMediaTypeValues = new Set(scenario.expected_valid_content_media_type_values);
  for (const entry of index.first_property_content_media_type_states) {
    if (!validMediaTypeValues.has(entry.input_schema_first_property_content_media_type)) {
      fail(`${scenario.id}: input_schema_first_property_content_media_type '${entry.input_schema_first_property_content_media_type}' is outside the closed four-value enum`);
    }
  }
  ...
  for (const entry of index.first_property_content_media_type_states) {
    if (!validAggKeys.has(entry.aggregation_key)) {
      fail(`${scenario.id}: aggregation_key '${entry.aggregation_key}' is outside the closed three-value enum for bucket '${entry.input_schema_first_property_content_media_type}'`);
    }
  }

  // Assert bucket order matches expected
  const bucketOrder = index.first_property_content_media_type_states.map((e) => e.input_schema_first_property_content_media_type).join(',');
  if (bucketOrder !== scenario.expected_bucket_order) {
    fail(`${scenario.id}: bucket order '${bucketOrder}' does not match expected '${scenario.expected_bucket_order}'`);
  }
  ...
  // Assert manifest is not mutated (input_schema_first_property_content_media_type must NOT be written into manifest)
  const manifestAfter = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
  for (const cap of manifestAfter.capabilities) {
    if (Object.prototype.hasOwnProperty.call(cap, 'input_schema_first_property_content_media_type')) {
      fail(`${scenario.id}: input_schema_first_property_content_media_type must NOT be written into tusq.manifest.json; found on capability '${cap.name}'`);
    }
  }

  // Assert typed_cap is in typed bucket (application/json string)
  const typedEntry = index.first_property_content_media_type_states.find((e) => e.input_schema_first_property_content_media_type === 'typed');
  if (!typedEntry || !typedEntry.capabilities.includes('typed_cap')) {
    fail(`${scenario.id}: typed_cap (firstKey.contentMediaType='application/json') must be in typed bucket (ANY-NON-EMPTY-STRING-IS-TYPED)`);
  }

  // Assert NULL-AS-ABSENT: null_media_cap → untyped (no warning)
  const untypedEntry = index.first_property_content_media_type_states.find((e) => e.input_schema_first_property_content_media_type === 'untyped');
  if (!untypedEntry || !untypedEntry.capabilities.includes('null_media_cap')) {
    fail(`${scenario.id}: null_media_cap (firstKey.contentMediaType=null) must be in untyped bucket (NULL-AS-ABSENT)`);
  }
  if (index.warnings.some((w) => w.capability === 'null_media_cap')) {
    fail(`${scenario.id}: null_media_cap (NULL-AS-ABSENT → untyped) must NOT produce a warning`);
  }

  // Assert untyped_cap is in untyped bucket (absent contentMediaType)
  if (!untypedEntry || !untypedEntry.capabilities.includes('untyped_cap')) {
    fail(`${scenario.id}: untyped_cap (firstKey.contentMediaType absent) must be in untyped bucket`);
  }

  // Assert not_applicable bucket exists (type_na_cap + schema_na_cap)
  const naEntry = index.first_property_content_media_type_states.find((e) => e.input_schema_first_property_content_media_type === 'not_applicable');
  if (!naEntry) {
    fail(`${scenario.id}: not_applicable bucket must be present`);
  }
  if (!naEntry.capabilities.includes('type_na_cap')) {
  ...
  // Assert unknown bucket exists
  const unknownEntry = index.first_property_content_media_type_states.find((e) => e.input_schema_first_property_content_media_type === 'unknown');
  if (!unknownEntry) {
    fail(`${scenario.id}: unknown bucket must be present`);
  }

  // Assert DRAFT-7-STRING-IS-VALID-CONTENT-MEDIA-TYPE: false_media_cap → unknown WITH 6th code
  if (!unknownEntry.capabilities.includes('false_media_cap')) {
    fail(`${scenario.id}: false_media_cap (firstKey.contentMediaType=false) must be in unknown bucket (DRAFT-7-STRING-IS-VALID-CONTENT-MEDIA-TYPE)`);
  }
  const falseMediaWarning = index.warnings.find((w) => w.capability === 'false_media_cap');
  if (!falseMediaWarning || falseMediaWarning.reason !== 'input_schema_properties_first_property_content_media_type_invalid_when_present') {
    fail(`${scenario.id}: false_media_cap must produce 6th-code warning 'input_schema_properties_first_property_content_media_type_invalid_when_present'; got: ${JSON.stringify(falseMediaWarning)}`);
  }

  // Assert typed bucket has aggregation_key 'media_type_constraint'
  if (typedEntry.aggregation_key !== 'media_type_constraint') {
    fail(`${scenario.id}: typed bucket must have aggregation_key 'media_type_constraint'; got '${typedEntry.aggregation_key}'`);
  }

  // Assert untyped bucket has aggregation_key 'media_type_constraint'
  if (untypedEntry.aggregation_key !== 'media_type_constraint') {
    fail(`${scenario.id}: untyped bucket must have aggregation_key 'media_type_constraint'; got '${untypedEntry.aggregation_key}'`);
  }
...
async function runInputSchemaFirstPropertyNullableIndexDeterminismScenario(tmpRoot, scenario) {
  const project = await fs.mkdtemp(path.join(tmpRoot, 'input-schema-first-property-nullable-'));
  ...
  // Run tusq nullable index --json three times and assert byte-identical output
  const run1 = runCli(['nullable', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run2 = runCli(['nullable', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run3 = runCli(['nullable', 'index', '--manifest', manifestPath, '--json'], { cwd: project });

  if (run1.stdout !== run2.stdout || run2.stdout !== run3.stdout) {
    fail(`${scenario.id}: nullable index --json output is not byte-identical across three runs`);
  }

  // Assert input_schema_first_property_nullable bucket-key enum is closed — every key is one of the four valid values
  const index = JSON.parse(run1.stdout);
  const validNullableValues = new Set(scenario.expected_valid_nullable_values);
  for (const entry of index.first_property_nullable_states) {
    if (!validNullableValues.has(entry.input_schema_first_property_nullable)) {
      fail(`${scenario.id}: input_schema_first_property_nullable '${entry.input_schema_first_property_nullable}' is outside the closed four-value enum`);
    }
  }
  ...
  for (const entry of index.first_property_nullable_states) {
    if (!validAggKeys.has(entry.aggregation_key)) {
      fail(`${scenario.id}: aggregation_key '${entry.aggregation_key}' is outside the closed three-value enum for bucket '${entry.input_schema_first_property_nullable}'`);
    }
  }

  // Assert bucket order matches expected
  const bucketOrder = index.first_property_nullable_states.map((e) => e.input_schema_first_property_nullable).join(',');
  if (bucketOrder !== scenario.expected_bucket_order) {
    fail(`${scenario.id}: bucket order '${bucketOrder}' does not match expected '${scenario.expected_bucket_order}'`);
  }
  ...
  // Assert manifest is not mutated (input_schema_first_property_nullable must NOT be written into manifest)
  const manifestAfter = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
  for (const cap of manifestAfter.capabilities) {
    if (Object.prototype.hasOwnProperty.call(cap, 'input_schema_first_property_nullable')) {
      fail(`${scenario.id}: input_schema_first_property_nullable must NOT be written into tusq.manifest.json; found on capability '${cap.name}'`);
    }
  }

  // Assert BOOLEAN-TRUE-AS-NULLABLE: nullable_cap → nullable bucket (no warning)
  const nullableEntry = index.first_property_nullable_states.find((e) => e.input_schema_first_property_nullable === 'nullable');
  if (!nullableEntry || !nullableEntry.capabilities.includes('nullable_cap')) {
    fail(`${scenario.id}: nullable_cap (firstKey.nullable=true) must be in nullable bucket (BOOLEAN-TRUE-AS-NULLABLE)`);
  }

  // Assert NO-TYPE-APPLICABILITY: integer_nullable_cap (type='integer', nullable=true) → nullable bucket (no warning)
  if (!nullableEntry || !nullableEntry.capabilities.includes('integer_nullable_cap')) {
    fail(`${scenario.id}: integer_nullable_cap (firstKey.type='integer', nullable=true) must be in nullable bucket (NO-TYPE-APPLICABILITY: nullable applies to ALL JSON-Schema types)`);
  }

  // Assert NULL-AS-ABSENT: null_nullable_cap → not_nullable (no warning)
  const notNullableEntry = index.first_property_nullable_states.find((e) => e.input_schema_first_property_nullable === 'not_nullable');
  if (!notNullableEntry || !notNullableEntry.capabilities.includes('null_nullable_cap')) {
    fail(`${scenario.id}: null_nullable_cap (firstKey.nullable=null) must be in not_nullable bucket (NULL-AS-ABSENT)`);
  }
  if (index.warnings.some((w) => w.capability === 'null_nullable_cap')) {
    fail(`${scenario.id}: null_nullable_cap (NULL-AS-ABSENT → not_nullable) must NOT produce a warning`);
  }

  // Assert ABSENT-AS-NOT-NULLABLE: absent_nullable_cap → not_nullable (no warning)
  if (!notNullableEntry || !notNullableEntry.capabilities.includes('absent_nullable_cap')) {
    fail(`${scenario.id}: absent_nullable_cap (nullable absent) must be in not_nullable bucket (ABSENT-AS-NOT-NULLABLE)`);
  }
  if (index.warnings.some((w) => w.capability === 'absent_nullable_cap')) {
    fail(`${scenario.id}: absent_nullable_cap (ABSENT-AS-NOT-NULLABLE → not_nullable) must NOT produce a warning`);
  }

  // Assert not_applicable bucket exists (schema_na_cap: input_schema.type='array')
  const naEntry = index.first_property_nullable_states.find((e) => e.input_schema_first_property_nullable === 'not_applicable');
  if (!naEntry) {
    fail(`${scenario.id}: not_applicable bucket must be present`);
  }
  if (!naEntry.capabilities.includes('schema_na_cap')) {
    fail(`${scenario.id}: schema_na_cap (input_schema.type='array') must be in not_applicable bucket`);
  }

  // Assert unknown bucket exists
  const unknownEntry = index.first_property_nullable_states.find((e) => e.input_schema_first_property_nullable === 'unknown');
  if (!unknownEntry) {
    fail(`${scenario.id}: unknown bucket must be present`);
  }

  // Assert DRAFT-7-BOOLEAN-IS-VALID-NULLABLE: string_nullable_cap → unknown WITH 6th code
  if (!unknownEntry.capabilities.includes('string_nullable_cap')) {
    fail(`${scenario.id}: string_nullable_cap (firstKey.nullable='yes') must be in unknown bucket (DRAFT-7-BOOLEAN-IS-VALID-NULLABLE)`);
  }
  const stringNullableWarning = index.warnings.find((w) => w.capability === 'string_nullable_cap');
  if (!stringNullableWarning || stringNullableWarning.reason !== 'input_schema_properties_first_property_nullable_invalid_when_present') {
    fail(`${scenario.id}: string_nullable_cap must produce 6th-code warning 'input_schema_properties_first_property_nullable_invalid_when_present'; got: ${JSON.stringify(stringNullableWarning)}`);
  }

  // Assert nullable bucket has aggregation_key 'nullability_constraint'
  if (nullableEntry.aggregation_key !== 'nullability_constraint') {
    fail(`${scenario.id}: nullable bucket must have aggregation_key 'nullability_constraint'; got '${nullableEntry.aggregation_key}'`);
  }

  // Assert not_nullable bucket has aggregation_key 'nullability_constraint'
  if (notNullableEntry.aggregation_key !== 'nullability_constraint') {
    fail(`${scenario.id}: not_nullable bucket must have aggregation_key 'nullability_constraint'; got '${notNullableEntry.aggregation_key}'`);
  }
...
async function runInputSchemaFirstPropertyMinItemsIndexDeterminismScenario(tmpRoot, scenario) {
  const project = await fs.mkdtemp(path.join(tmpRoot, 'input-schema-first-property-min-items-'));
  ...
  // Run tusq least index --json three times and assert byte-identical output
  const run1 = runCli(['least', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run2 = runCli(['least', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run3 = runCli(['least', 'index', '--manifest', manifestPath, '--json'], { cwd: project });

  if (run1.stdout !== run2.stdout || run2.stdout !== run3.stdout) {
    fail(`${scenario.id}: least index --json output is not byte-identical across three runs`);
  }

  // Assert input_schema_first_property_min_items bucket-key enum is closed — every key is one of the four valid values
  const index = JSON.parse(run1.stdout);
  const validMinItemsValues = new Set(scenario.expected_valid_min_items_values);
  for (const entry of index.first_property_min_items_states) {
    if (!validMinItemsValues.has(entry.input_schema_first_property_min_items)) {
      fail(`${scenario.id}: input_schema_first_property_min_items '${entry.input_schema_first_property_min_items}' is outside the closed four-value enum`);
    }
  }
  ...
  for (const entry of index.first_property_min_items_states) {
    if (!validAggKeys.has(entry.aggregation_key)) {
      fail(`${scenario.id}: aggregation_key '${entry.aggregation_key}' is outside the closed three-value enum for bucket '${entry.input_schema_first_property_min_items}'`);
    }
  }

  // Assert bucket order matches expected
  const bucketOrder = index.first_property_min_items_states.map((e) => e.input_schema_first_property_min_items).join(',');
  if (bucketOrder !== scenario.expected_bucket_order) {
    fail(`${scenario.id}: bucket order '${bucketOrder}' does not match expected '${scenario.expected_bucket_order}'`);
  }
  ...
  // Assert manifest is not mutated (input_schema_first_property_min_items must NOT be written into manifest)
  const manifestAfter = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
  for (const cap of manifestAfter.capabilities) {
    if (Object.prototype.hasOwnProperty.call(cap, 'input_schema_first_property_min_items')) {
      fail(`${scenario.id}: input_schema_first_property_min_items must NOT be written into tusq.manifest.json; found on capability '${cap.name}'`);
    }
  }

  // Assert NON-NEGATIVE-INTEGER-IS-VALID-MIN-ITEMS: bounded_cap (minItems=1) → bounded bucket
  const boundedEntry = index.first_property_min_items_states.find((e) => e.input_schema_first_property_min_items === 'bounded');
  if (!boundedEntry || !boundedEntry.capabilities.includes('bounded_cap')) {
    fail(`${scenario.id}: bounded_cap (firstKey.minItems=1) must be in bounded bucket (NON-NEGATIVE-INTEGER-IS-VALID-MIN-ITEMS)`);
  }

  // Assert PRESENT-AS-PRESENT-ZERO: zero_bounded_cap (minItems=0) → bounded bucket
  if (!boundedEntry || !boundedEntry.capabilities.includes('zero_bounded_cap')) {
    fail(`${scenario.id}: zero_bounded_cap (firstKey.minItems=0) must be in bounded bucket (PRESENT-AS-PRESENT-ZERO: explicit-zero → bounded)`);
  }

  // Assert ABSENT-AS-UNBOUNDED: unbounded_cap (minItems absent) → unbounded bucket
  const unboundedEntry = index.first_property_min_items_states.find((e) => e.input_schema_first_property_min_items === 'unbounded');
  if (!unboundedEntry || !unboundedEntry.capabilities.includes('unbounded_cap')) {
    fail(`${scenario.id}: unbounded_cap (minItems absent) must be in unbounded bucket (ABSENT-AS-UNBOUNDED)`);
  }
  ...
  // Assert NULL-AS-ABSENT: null_min_items_cap (minItems=null) → unbounded bucket
  if (!unboundedEntry || !unboundedEntry.capabilities.includes('null_min_items_cap')) {
    fail(`${scenario.id}: null_min_items_cap (firstKey.minItems=null) must be in unbounded bucket (NULL-AS-ABSENT)`);
  }
  if (index.warnings.some((w) => w.capability === 'null_min_items_cap')) {
    fail(`${scenario.id}: null_min_items_cap (NULL-AS-ABSENT → unbounded) must NOT produce a warning`);
  }

  // Assert TYPE-APPLICABILITY-ARRAY: string_type_na_cap (type='string') → not_applicable bucket
  const naEntry = index.first_property_min_items_states.find((e) => e.input_schema_first_property_min_items === 'not_applicable');
  if (!naEntry) {
    fail(`${scenario.id}: not_applicable bucket must be present`);
  }
  ...
  // Assert unknown bucket exists
  const unknownEntry = index.first_property_min_items_states.find((e) => e.input_schema_first_property_min_items === 'unknown');
  if (!unknownEntry) {
    fail(`${scenario.id}: unknown bucket must be present`);
  }

  // Assert DRAFT-7-NON-NEGATIVE-INTEGER-IS-VALID: negative_min_items_cap (minItems=-1) → unknown WITH 6th code
  if (!unknownEntry.capabilities.includes('negative_min_items_cap')) {
    fail(`${scenario.id}: negative_min_items_cap (firstKey.minItems=-1) must be in unknown bucket (DRAFT-7-NON-NEGATIVE-INTEGER-IS-VALID)`);
  }
  const negativeWarning = index.warnings.find((w) => w.capability === 'negative_min_items_cap');
  if (!negativeWarning || negativeWarning.reason !== 'input_schema_properties_first_property_min_items_invalid_when_present') {
    fail(`${scenario.id}: negative_min_items_cap must produce 6th-code warning 'input_schema_properties_first_property_min_items_invalid_when_present'; got: ${JSON.stringify(negativeWarning)}`);
  }

  // Assert bounded bucket has aggregation_key 'array_cardinality_floor_constraint'
  if (boundedEntry.aggregation_key !== 'array_cardinality_floor_constraint') {
    fail(`${scenario.id}: bounded bucket must have aggregation_key 'array_cardinality_floor_constraint'; got '${boundedEntry.aggregation_key}'`);
  }
...
async function runInputSchemaFirstPropertyUniqueItemsIndexDeterminismScenario(tmpRoot, scenario) {
  const project = await fs.mkdtemp(path.join(tmpRoot, 'input-schema-first-property-unique-items-'));
  ...
  // Run tusq unique index --json three times and assert byte-identical output
  const run1 = runCli(['unique', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run2 = runCli(['unique', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run3 = runCli(['unique', 'index', '--manifest', manifestPath, '--json'], { cwd: project });

  if (run1.stdout !== run2.stdout || run2.stdout !== run3.stdout) {
    fail(`${scenario.id}: unique index --json output is not byte-identical across three runs`);
  }

  // Assert input_schema_first_property_unique_items bucket-key enum is closed — every key is one of the four valid values
  const index = JSON.parse(run1.stdout);
  const validUniqueItemsValues = new Set(scenario.expected_valid_unique_items_values);
  for (const entry of index.first_property_unique_items_states) {
    if (!validUniqueItemsValues.has(entry.input_schema_first_property_unique_items)) {
      fail(`${scenario.id}: input_schema_first_property_unique_items '${entry.input_schema_first_property_unique_items}' is outside the closed four-value enum`);
    }
  }
  ...
  for (const entry of index.first_property_unique_items_states) {
    if (!validAggKeys.has(entry.aggregation_key)) {
      fail(`${scenario.id}: aggregation_key '${entry.aggregation_key}' is outside the closed three-value enum for bucket '${entry.input_schema_first_property_unique_items}'`);
    }
  }

  // Assert bucket order matches expected
  const bucketOrder = index.first_property_unique_items_states.map((e) => e.input_schema_first_property_unique_items).join(',');
  if (bucketOrder !== scenario.expected_bucket_order) {
    fail(`${scenario.id}: bucket order '${bucketOrder}' does not match expected '${scenario.expected_bucket_order}'`);
  }
  ...
  // Assert manifest is not mutated (input_schema_first_property_unique_items must NOT be written into manifest)
  const manifestAfter = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
  for (const cap of manifestAfter.capabilities) {
    if (Object.prototype.hasOwnProperty.call(cap, 'input_schema_first_property_unique_items')) {
      fail(`${scenario.id}: input_schema_first_property_unique_items must NOT be written into tusq.manifest.json; found on capability '${cap.name}'`);
    }
  }

  // Assert BOOLEAN-TRUE-AS-UNIQUE: unique_cap (uniqueItems=true) → unique bucket
  const uniqueEntry = index.first_property_unique_items_states.find((e) => e.input_schema_first_property_unique_items === 'unique');
  if (!uniqueEntry || !uniqueEntry.capabilities.includes('unique_cap')) {
    fail(`${scenario.id}: unique_cap (firstKey.uniqueItems=true) must be in unique bucket (BOOLEAN-TRUE-AS-UNIQUE)`);
  }

  // Assert BOOLEAN-FALSE-AS-NOT-UNIQUE: not_unique_false_cap (uniqueItems=false) → not_unique bucket
  const notUniqueEntry = index.first_property_unique_items_states.find((e) => e.input_schema_first_property_unique_items === 'not_unique');
  if (!notUniqueEntry || !notUniqueEntry.capabilities.includes('not_unique_false_cap')) {
    fail(`${scenario.id}: not_unique_false_cap (firstKey.uniqueItems=false) must be in not_unique bucket (BOOLEAN-FALSE-AS-NOT-UNIQUE)`);
  }

  // Assert ABSENT-AS-NOT-UNIQUE: not_unique_absent_cap (uniqueItems absent) → not_unique bucket
  if (!notUniqueEntry || !notUniqueEntry.capabilities.includes('not_unique_absent_cap')) {
    fail(`${scenario.id}: not_unique_absent_cap (uniqueItems absent) must be in not_unique bucket (ABSENT-AS-NOT-UNIQUE)`);
  }
  if (index.warnings.some((w) => w.capability === 'not_unique_absent_cap')) {
    fail(`${scenario.id}: not_unique_absent_cap (ABSENT-AS-NOT-UNIQUE → not_unique) must NOT produce a warning`);
  }

  // Assert NULL-AS-ABSENT: null_unique_items_cap (uniqueItems=null) → not_unique bucket
  if (!notUniqueEntry || !notUniqueEntry.capabilities.includes('null_unique_items_cap')) {
    fail(`${scenario.id}: null_unique_items_cap (firstKey.uniqueItems=null) must be in not_unique bucket (NULL-AS-ABSENT)`);
  }
  if (index.warnings.some((w) => w.capability === 'null_unique_items_cap')) {
    fail(`${scenario.id}: null_unique_items_cap (NULL-AS-ABSENT → not_unique) must NOT produce a warning`);
  }

  // Assert TYPE-APPLICABILITY-ARRAY: string_type_na_cap (type='string') → not_applicable bucket
  const naEntry = index.first_property_unique_items_states.find((e) => e.input_schema_first_property_unique_items === 'not_applicable');
  if (!naEntry) {
    fail(`${scenario.id}: not_applicable bucket must be present`);
  }
  if (!naEntry.capabilities.includes('string_type_na_cap')) {
  ...
  // Assert unknown bucket exists
  const unknownEntry = index.first_property_unique_items_states.find((e) => e.input_schema_first_property_unique_items === 'unknown');
  if (!unknownEntry) {
    fail(`${scenario.id}: unknown bucket must be present`);
  }

  // Assert DRAFT-7-BOOLEAN-IS-VALID-UNIQUE-ITEMS: string_unique_items_cap (uniqueItems='yes') → unknown WITH 6th code
  if (!unknownEntry.capabilities.includes('string_unique_items_cap')) {
    fail(`${scenario.id}: string_unique_items_cap (firstKey.uniqueItems='yes') must be in unknown bucket (DRAFT-7-BOOLEAN-IS-VALID-UNIQUE-ITEMS)`);
  }
  const stringWarning = index.warnings.find((w) => w.capability === 'string_unique_items_cap');
  if (!stringWarning || stringWarning.reason !== 'input_schema_properties_first_property_unique_items_invalid_when_present') {
    fail(`${scenario.id}: string_unique_items_cap must produce 6th-code warning 'input_schema_properties_first_property_unique_items_invalid_when_present'; got: ${JSON.stringify(stringWarning)}`);
  }

  // Assert unique bucket has aggregation_key 'array_element_uniqueness_constraint'
  if (uniqueEntry.aggregation_key !== 'array_element_uniqueness_constraint') {
    fail(`${scenario.id}: unique bucket must have aggregation_key 'array_element_uniqueness_constraint'; got '${uniqueEntry.aggregation_key}'`);
  }

  // Assert not_unique bucket has aggregation_key 'array_element_uniqueness_constraint'
  if (notUniqueEntry.aggregation_key !== 'array_element_uniqueness_constraint') {
    fail(`${scenario.id}: not_unique bucket must have aggregation_key 'array_element_uniqueness_constraint'; got '${notUniqueEntry.aggregation_key}'`);
  }
...
async function runInputSchemaFirstPropertyItemsIndexDeterminismScenario(tmpRoot, scenario) {
  const project = await fs.mkdtemp(path.join(tmpRoot, 'input-schema-first-property-items-'));
  ...
  // Run tusq element index --json three times and assert byte-identical output
  const run1 = runCli(['element', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run2 = runCli(['element', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run3 = runCli(['element', 'index', '--manifest', manifestPath, '--json'], { cwd: project });

  if (run1.stdout !== run2.stdout || run2.stdout !== run3.stdout) {
    fail(`${scenario.id}: element index --json output is not byte-identical across three runs`);
  }

  // Assert input_schema_first_property_items bucket-key enum is closed — every key is one of the four valid values
  const index = JSON.parse(run1.stdout);
  const validItemsValues = new Set(scenario.expected_valid_items_values);
  for (const entry of index.first_property_items_states) {
    if (!validItemsValues.has(entry.input_schema_first_property_items)) {
      fail(`${scenario.id}: input_schema_first_property_items '${entry.input_schema_first_property_items}' is outside the closed four-value enum`);
    }
  }
  ...
  for (const entry of index.first_property_items_states) {
    if (!validAggKeys.has(entry.aggregation_key)) {
      fail(`${scenario.id}: aggregation_key '${entry.aggregation_key}' is outside the closed three-value enum for bucket '${entry.input_schema_first_property_items}'`);
    }
  }

  // Assert bucket order matches expected
  const bucketOrder = index.first_property_items_states.map((e) => e.input_schema_first_property_items).join(',');
  if (bucketOrder !== scenario.expected_bucket_order) {
    fail(`${scenario.id}: bucket order '${bucketOrder}' does not match expected '${scenario.expected_bucket_order}'`);
  }
  ...
  // Assert manifest is not mutated (input_schema_first_property_items must NOT be written into manifest)
  const manifestAfter = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
  for (const cap of manifestAfter.capabilities) {
    if (Object.prototype.hasOwnProperty.call(cap, 'input_schema_first_property_items')) {
      fail(`${scenario.id}: input_schema_first_property_items must NOT be written into tusq.manifest.json; found on capability '${cap.name}'`);
    }
  }

  // Assert PLAIN-OBJECT-AS-DECLARED: declared_plain_cap (items={type:'string'}) → declared bucket
  const declaredEntry = index.first_property_items_states.find((e) => e.input_schema_first_property_items === 'declared');
  if (!declaredEntry || !declaredEntry.capabilities.includes('declared_plain_cap')) {
    fail(`${scenario.id}: declared_plain_cap (firstKey.items={type:'string'}) must be in declared bucket (PLAIN-OBJECT-AS-DECLARED)`);
  }

  // Assert ARRAY-OF-OBJECTS-AS-DECLARED: declared_tuple_cap (items=[{type:'string'},{type:'integer'}]) → declared bucket
  if (!declaredEntry || !declaredEntry.capabilities.includes('declared_tuple_cap')) {
    fail(`${scenario.id}: declared_tuple_cap (firstKey.items=[{type:'string'},{type:'integer'}]) must be in declared bucket (ARRAY-OF-OBJECTS-AS-DECLARED)`);
  }

  // Assert ARRAY-OF-OBJECTS-AS-DECLARED: declared_empty_tuple_cap (items=[]) → declared bucket
  if (!declaredEntry || !declaredEntry.capabilities.includes('declared_empty_tuple_cap')) {
    fail(`${scenario.id}: declared_empty_tuple_cap (firstKey.items=[]) must be in declared bucket (ARRAY-OF-OBJECTS-AS-DECLARED empty tuple)`);
  }

  // Assert ABSENT-AS-UNDECLARED: undeclared_absent_cap (items absent) → undeclared bucket
  const undeclaredEntry = index.first_property_items_states.find((e) => e.input_schema_first_property_items === 'undeclared');
  if (!undeclaredEntry || !undeclaredEntry.capabilities.includes('undeclared_absent_cap')) {
    fail(`${scenario.id}: undeclared_absent_cap (items absent) must be in undeclared bucket (ABSENT-AS-UNDECLARED)`);
  }
  if (index.warnings.some((w) => w.capability === 'undeclared_absent_cap')) {
    fail(`${scenario.id}: undeclared_absent_cap (ABSENT-AS-UNDECLARED → undeclared) must NOT produce a warning`);
  }

  // Assert NULL-AS-ABSENT: undeclared_null_cap (items=null) → undeclared bucket
  if (!undeclaredEntry || !undeclaredEntry.capabilities.includes('undeclared_null_cap')) {
    fail(`${scenario.id}: undeclared_null_cap (firstKey.items=null) must be in undeclared bucket (NULL-AS-ABSENT)`);
  }
  if (index.warnings.some((w) => w.capability === 'undeclared_null_cap')) {
    fail(`${scenario.id}: undeclared_null_cap (NULL-AS-ABSENT → undeclared) must NOT produce a warning`);
  }

  // Assert TYPE-APPLICABILITY-ARRAY: string_type_na_cap (type='string') → not_applicable bucket
  const naEntry = index.first_property_items_states.find((e) => e.input_schema_first_property_items === 'not_applicable');
  if (!naEntry) {
    fail(`${scenario.id}: not_applicable bucket must be present`);
  }
  if (!naEntry.capabilities.includes('string_type_na_cap')) {
  ...
  // Assert unknown bucket exists
  const unknownEntry = index.first_property_items_states.find((e) => e.input_schema_first_property_items === 'unknown');
  if (!unknownEntry) {
    fail(`${scenario.id}: unknown bucket must be present`);
  }

  // Assert DRAFT-7-OBJECT-OR-OBJECT-ARRAY-IS-VALID-ITEMS: string_items_cap (items='string') → unknown WITH 6th code
  if (!unknownEntry.capabilities.includes('string_items_cap')) {
    fail(`${scenario.id}: string_items_cap (firstKey.items='string') must be in unknown bucket (DRAFT-7-OBJECT-OR-OBJECT-ARRAY-IS-VALID-ITEMS)`);
  }
  const stringWarning = index.warnings.find((w) => w.capability === 'string_items_cap');
  if (!stringWarning || stringWarning.reason !== 'input_schema_properties_first_property_items_invalid_when_present') {
    fail(`${scenario.id}: string_items_cap must produce 6th-code warning 'input_schema_properties_first_property_items_invalid_when_present'; got: ${JSON.stringify(stringWarning)}`);
  }

  // Assert declared bucket has aggregation_key 'array_element_subschema_constraint'
  if (declaredEntry.aggregation_key !== 'array_element_subschema_constraint') {
    fail(`${scenario.id}: declared bucket must have aggregation_key 'array_element_subschema_constraint'; got '${declaredEntry.aggregation_key}'`);
  }
...
async function runInputSchemaFirstPropertyMinPropertiesIndexDeterminismScenario(tmpRoot, scenario) {
  const project = await fs.mkdtemp(path.join(tmpRoot, 'input-schema-first-property-min-properties-'));
  ...
  // Run tusq sparse index --json three times and assert byte-identical output
  const run1 = runCli(['sparse', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run2 = runCli(['sparse', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run3 = runCli(['sparse', 'index', '--manifest', manifestPath, '--json'], { cwd: project });

  if (run1.stdout !== run2.stdout || run2.stdout !== run3.stdout) {
    fail(`${scenario.id}: sparse index --json output is not byte-identical across three runs`);
  }

  // Assert input_schema_first_property_min_properties bucket-key enum is closed
  const index = JSON.parse(run1.stdout);
  const validMpValues = new Set(scenario.expected_valid_min_properties_values);
  for (const entry of index.first_property_min_properties_states) {
    if (!validMpValues.has(entry.input_schema_first_property_min_properties)) {
      fail(`${scenario.id}: input_schema_first_property_min_properties '${entry.input_schema_first_property_min_properties}' is outside the closed four-value enum`);
    }
  }
  ...
  for (const entry of index.first_property_min_properties_states) {
    if (!validAggKeys.has(entry.aggregation_key)) {
      fail(`${scenario.id}: aggregation_key '${entry.aggregation_key}' is outside the closed three-value enum for bucket '${entry.input_schema_first_property_min_properties}'`);
    }
  }

  // Assert bucket order matches expected
  const bucketOrder = index.first_property_min_properties_states.map((e) => e.input_schema_first_property_min_properties).join(',');
  if (bucketOrder !== scenario.expected_bucket_order) {
    fail(`${scenario.id}: bucket order '${bucketOrder}' does not match expected '${scenario.expected_bucket_order}'`);
  }
  ...
  // Assert manifest is not mutated (input_schema_first_property_min_properties must NOT be written into manifest)
  const manifestAfter = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
  for (const cap of manifestAfter.capabilities) {
    if (Object.prototype.hasOwnProperty.call(cap, 'input_schema_first_property_min_properties')) {
      fail(`${scenario.id}: input_schema_first_property_min_properties must NOT be written into tusq.manifest.json; found on capability '${cap.name}'`);
    }
  }

  // Assert PRESENT-AS-PRESENT-ZERO + NON-NEGATIVE-INTEGER-IS-VALID-MIN-PROPERTIES: bounded_zero_cap (minProperties=0) → bounded bucket
  const boundedEntry = index.first_property_min_properties_states.find((e) => e.input_schema_first_property_min_properties === 'bounded');
  if (!boundedEntry || !boundedEntry.capabilities.includes('bounded_zero_cap')) {
    fail(`${scenario.id}: bounded_zero_cap (minProperties=0) must be in bounded bucket (PRESENT-AS-PRESENT-ZERO)`);
  }
  if (index.warnings.some((w) => w.capability === 'bounded_zero_cap')) {
    fail(`${scenario.id}: bounded_zero_cap (PRESENT-AS-PRESENT-ZERO → bounded) must NOT produce a warning`);
  }

  // Assert NON-NEGATIVE-INTEGER-IS-VALID-MIN-PROPERTIES: bounded_one_cap → bounded bucket
  if (!boundedEntry || !boundedEntry.capabilities.includes('bounded_one_cap')) {
    fail(`${scenario.id}: bounded_one_cap (minProperties=1) must be in bounded bucket (NON-NEGATIVE-INTEGER-IS-VALID-MIN-PROPERTIES)`);
  }
  if (index.warnings.some((w) => w.capability === 'bounded_one_cap')) {
    fail(`${scenario.id}: bounded_one_cap (NON-NEGATIVE-INTEGER-IS-VALID-MIN-PROPERTIES → bounded) must NOT produce a warning`);
  }

  // Assert ABSENT-AS-UNBOUNDED: unbounded_absent_cap → unbounded bucket
  const unboundedEntry = index.first_property_min_properties_states.find((e) => e.input_schema_first_property_min_properties === 'unbounded');
  if (!unboundedEntry || !unboundedEntry.capabilities.includes('unbounded_absent_cap')) {
    fail(`${scenario.id}: unbounded_absent_cap (minProperties absent) must be in unbounded bucket (ABSENT-AS-UNBOUNDED)`);
  }
  if (index.warnings.some((w) => w.capability === 'unbounded_absent_cap')) {
    fail(`${scenario.id}: unbounded_absent_cap (ABSENT-AS-UNBOUNDED → unbounded) must NOT produce a warning`);
  }

  // Assert NULL-AS-ABSENT: unbounded_null_cap → unbounded bucket
  if (!unboundedEntry || !unboundedEntry.capabilities.includes('unbounded_null_cap')) {
    fail(`${scenario.id}: unbounded_null_cap (minProperties=null) must be in unbounded bucket (NULL-AS-ABSENT)`);
  }
  if (index.warnings.some((w) => w.capability === 'unbounded_null_cap')) {
    fail(`${scenario.id}: unbounded_null_cap (NULL-AS-ABSENT → unbounded) must NOT produce a warning`);
  }

  // Assert TYPE-APPLICABILITY-OBJECT: string_type_na_cap → not_applicable bucket
  const naEntry = index.first_property_min_properties_states.find((e) => e.input_schema_first_property_min_properties === 'not_applicable');
  if (!naEntry) {
    fail(`${scenario.id}: not_applicable bucket must be present`);
  }
  if (!naEntry.capabilities.includes('string_type_na_cap')) {
  ...
  // Assert unknown bucket exists
  const unknownEntry = index.first_property_min_properties_states.find((e) => e.input_schema_first_property_min_properties === 'unknown');
  if (!unknownEntry) {
    fail(`${scenario.id}: unknown bucket must be present`);
  }

  // Assert DRAFT-7-NON-NEGATIVE-INTEGER-IS-VALID-MIN-PROPERTIES: string_mp_cap (minProperties='1') → unknown WITH 6th code
  if (!unknownEntry.capabilities.includes('string_mp_cap')) {
    fail(`${scenario.id}: string_mp_cap (minProperties='1') must be in unknown bucket (DRAFT-7-NON-NEGATIVE-INTEGER-IS-VALID-MIN-PROPERTIES)`);
  }
  const stringMpWarning = index.warnings.find((w) => w.capability === 'string_mp_cap');
  if (!stringMpWarning || stringMpWarning.reason !== 'input_schema_properties_first_property_min_properties_invalid_when_present') {
    fail(`${scenario.id}: string_mp_cap must produce 6th-code warning 'input_schema_properties_first_property_min_properties_invalid_when_present'; got: ${JSON.stringify(stringMpWarning)}`);
  }

  // Assert bounded bucket has aggregation_key 'object_property_count_floor_constraint'
  if (boundedEntry.aggregation_key !== 'object_property_count_floor_constraint') {
    fail(`${scenario.id}: bounded bucket must have aggregation_key 'object_property_count_floor_constraint'; got '${boundedEntry.aggregation_key}'`);
  }

  // Assert unbounded bucket has aggregation_key 'object_property_count_floor_constraint'
  if (unboundedEntry.aggregation_key !== 'object_property_count_floor_constraint') {
    fail(`${scenario.id}: unbounded bucket must have aggregation_key 'object_property_count_floor_constraint'; got '${unboundedEntry.aggregation_key}'`);
  }
...
async function runInputSchemaFirstPropertyMaxPropertiesIndexDeterminismScenario(tmpRoot, scenario) {
  const project = await fs.mkdtemp(path.join(tmpRoot, 'input-schema-first-property-max-properties-'));
  ...
  // Run tusq crowded index --json three times and assert byte-identical output
  const run1 = runCli(['crowded', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run2 = runCli(['crowded', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run3 = runCli(['crowded', 'index', '--manifest', manifestPath, '--json'], { cwd: project });

  if (run1.stdout !== run2.stdout || run2.stdout !== run3.stdout) {
    fail(`${scenario.id}: crowded index --json output is not byte-identical across three runs`);
  }

  // Assert input_schema_first_property_max_properties bucket-key enum is closed
  const index = JSON.parse(run1.stdout);
  const validMpValues = new Set(scenario.expected_valid_min_properties_values);
  for (const entry of index.first_property_max_properties_states) {
    if (!validMpValues.has(entry.input_schema_first_property_max_properties)) {
      fail(`${scenario.id}: input_schema_first_property_max_properties '${entry.input_schema_first_property_max_properties}' is outside the closed four-value enum`);
    }
  }
  ...
  for (const entry of index.first_property_max_properties_states) {
    if (!validAggKeys.has(entry.aggregation_key)) {
      fail(`${scenario.id}: aggregation_key '${entry.aggregation_key}' is outside the closed three-value enum for bucket '${entry.input_schema_first_property_max_properties}'`);
    }
  }

  // Assert bucket order matches expected
  const bucketOrder = index.first_property_max_properties_states.map((e) => e.input_schema_first_property_max_properties).join(',');
  if (bucketOrder !== scenario.expected_bucket_order) {
    fail(`${scenario.id}: bucket order '${bucketOrder}' does not match expected '${scenario.expected_bucket_order}'`);
  }
  ...
  // Assert manifest is not mutated (input_schema_first_property_max_properties must NOT be written into manifest)
  const manifestAfter = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
  for (const cap of manifestAfter.capabilities) {
    if (Object.prototype.hasOwnProperty.call(cap, 'input_schema_first_property_max_properties')) {
      fail(`${scenario.id}: input_schema_first_property_max_properties must NOT be written into tusq.manifest.json; found on capability '${cap.name}'`);
    }
  }

  // Assert PRESENT-AS-PRESENT-ZERO + NON-NEGATIVE-INTEGER-IS-VALID-MAX-PROPERTIES: bounded_zero_cap (maxProperties=0) → bounded bucket
  const boundedEntry = index.first_property_max_properties_states.find((e) => e.input_schema_first_property_max_properties === 'bounded');
  if (!boundedEntry || !boundedEntry.capabilities.includes('bounded_zero_cap')) {
    fail(`${scenario.id}: bounded_zero_cap (maxProperties=0) must be in bounded bucket (PRESENT-AS-PRESENT-ZERO)`);
  }
  if (index.warnings.some((w) => w.capability === 'bounded_zero_cap')) {
    fail(`${scenario.id}: bounded_zero_cap (PRESENT-AS-PRESENT-ZERO → bounded) must NOT produce a warning`);
  }

  // Assert NON-NEGATIVE-INTEGER-IS-VALID-MAX-PROPERTIES: bounded_one_cap → bounded bucket
  if (!boundedEntry || !boundedEntry.capabilities.includes('bounded_one_cap')) {
    fail(`${scenario.id}: bounded_one_cap (maxProperties=1) must be in bounded bucket (NON-NEGATIVE-INTEGER-IS-VALID-MAX-PROPERTIES)`);
  }
  if (index.warnings.some((w) => w.capability === 'bounded_one_cap')) {
    fail(`${scenario.id}: bounded_one_cap (NON-NEGATIVE-INTEGER-IS-VALID-MAX-PROPERTIES → bounded) must NOT produce a warning`);
  }

  // Assert ABSENT-AS-UNBOUNDED: unbounded_absent_cap → unbounded bucket
  const unboundedEntry = index.first_property_max_properties_states.find((e) => e.input_schema_first_property_max_properties === 'unbounded');
  if (!unboundedEntry || !unboundedEntry.capabilities.includes('unbounded_absent_cap')) {
    fail(`${scenario.id}: unbounded_absent_cap (maxProperties absent) must be in unbounded bucket (ABSENT-AS-UNBOUNDED)`);
  }
  if (index.warnings.some((w) => w.capability === 'unbounded_absent_cap')) {
    fail(`${scenario.id}: unbounded_absent_cap (ABSENT-AS-UNBOUNDED → unbounded) must NOT produce a warning`);
  }
  ...
  // Assert NULL-AS-ABSENT: unbounded_null_cap → unbounded bucket
  if (!unboundedEntry || !unboundedEntry.capabilities.includes('unbounded_null_cap')) {
    fail(`${scenario.id}: unbounded_null_cap (maxProperties=null) must be in unbounded bucket (NULL-AS-ABSENT)`);
  }
  if (index.warnings.some((w) => w.capability === 'unbounded_null_cap')) {
    fail(`${scenario.id}: unbounded_null_cap (NULL-AS-ABSENT → unbounded) must NOT produce a warning`);
  }
  ...
  // Assert TYPE-APPLICABILITY-OBJECT: string_type_na_cap → not_applicable bucket
  const naEntry = index.first_property_max_properties_states.find((e) => e.input_schema_first_property_max_properties === 'not_applicable');
  if (!naEntry) {
    fail(`${scenario.id}: not_applicable bucket must be present`);
  }
  if (!naEntry.capabilities.includes('string_type_na_cap')) {
  ...
  // Assert unknown bucket exists
  const unknownEntry = index.first_property_max_properties_states.find((e) => e.input_schema_first_property_max_properties === 'unknown');
  if (!unknownEntry) {
    fail(`${scenario.id}: unknown bucket must be present`);
  }

  // Assert DRAFT-7-NON-NEGATIVE-INTEGER-IS-VALID-MAX-PROPERTIES: string_mp_cap (maxProperties='1') → unknown WITH 6th code
  if (!unknownEntry.capabilities.includes('string_mp_cap')) {
    fail(`${scenario.id}: string_mp_cap (maxProperties='1') must be in unknown bucket (DRAFT-7-NON-NEGATIVE-INTEGER-IS-VALID-MAX-PROPERTIES)`);
  }
  const stringMpWarning = index.warnings.find((w) => w.capability === 'string_mp_cap');
  if (!stringMpWarning || stringMpWarning.reason !== 'input_schema_properties_first_property_max_properties_invalid_when_present') {
    fail(`${scenario.id}: string_mp_cap must produce 6th-code warning 'input_schema_properties_first_property_max_properties_invalid_when_present'; got: ${JSON.stringify(stringMpWarning)}`);
  }

  // Assert bounded bucket has aggregation_key 'object_property_count_ceiling_constraint'
  if (boundedEntry.aggregation_key !== 'object_property_count_ceiling_constraint') {
    fail(`${scenario.id}: bounded bucket must have aggregation_key 'object_property_count_ceiling_constraint'; got '${boundedEntry.aggregation_key}'`);
  }

  // Assert unbounded bucket has aggregation_key 'object_property_count_ceiling_constraint'
  if (unboundedEntry.aggregation_key !== 'object_property_count_ceiling_constraint') {
    fail(`${scenario.id}: unbounded bucket must have aggregation_key 'object_property_count_ceiling_constraint'; got '${unboundedEntry.aggregation_key}'`);
  }
...
async function runInputSchemaFirstPropertyPatternPropertiesIndexDeterminismScenario(tmpRoot, scenario) {
  const project = await fs.mkdtemp(path.join(tmpRoot, 'input-schema-first-property-pattern-properties-'));
  ...
  // Run tusq partition index --json three times and assert byte-identical output
  const run1 = runCli(['partition', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run2 = runCli(['partition', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run3 = runCli(['partition', 'index', '--manifest', manifestPath, '--json'], { cwd: project });

  if (run1.stdout !== run2.stdout || run2.stdout !== run3.stdout) {
    fail(`${scenario.id}: partition index --json output is not byte-identical across three runs`);
  }

  // Assert input_schema_first_property_pattern_properties bucket-key enum is closed
  const index = JSON.parse(run1.stdout);
  const validPpValues = new Set(scenario.expected_valid_pattern_properties_values);
  for (const entry of index.first_property_pattern_properties_states) {
    if (!validPpValues.has(entry.input_schema_first_property_pattern_properties)) {
      fail(`${scenario.id}: input_schema_first_property_pattern_properties '${entry.input_schema_first_property_pattern_properties}' is outside the closed four-value enum`);
    }
  }
  ...
  for (const entry of index.first_property_pattern_properties_states) {
    if (!validAggKeys.has(entry.aggregation_key)) {
      fail(`${scenario.id}: aggregation_key '${entry.aggregation_key}' is outside the closed three-value enum for bucket '${entry.input_schema_first_property_pattern_properties}'`);
    }
  }

  // Assert bucket order matches expected
  const bucketOrder = index.first_property_pattern_properties_states.map((e) => e.input_schema_first_property_pattern_properties).join(',');
  if (bucketOrder !== scenario.expected_bucket_order) {
    fail(`${scenario.id}: bucket order '${bucketOrder}' does not match expected '${scenario.expected_bucket_order}'`);
  }
  ...
  // Assert manifest is not mutated (input_schema_first_property_pattern_properties must NOT be written into manifest)
  const manifestAfter = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
  for (const cap of manifestAfter.capabilities) {
    if (Object.prototype.hasOwnProperty.call(cap, 'input_schema_first_property_pattern_properties')) {
      fail(`${scenario.id}: input_schema_first_property_pattern_properties must NOT be written into tusq.manifest.json; found on capability '${cap.name}'`);
    }
  }

  // Assert PLAIN-OBJECT-WITH-OWN-KEYS-AS-TYPED: typed_obj_cap → typed bucket
  const typedEntry = index.first_property_pattern_properties_states.find((e) => e.input_schema_first_property_pattern_properties === 'typed');
  if (!typedEntry || !typedEntry.capabilities.includes('typed_obj_cap')) {
    fail(`${scenario.id}: typed_obj_cap (patternProperties={'^x-':{type:'string'}}) must be in typed bucket (PLAIN-OBJECT-WITH-OWN-KEYS-AS-TYPED)`);
  }
  if (index.warnings.some((w) => w.capability === 'typed_obj_cap')) {
    fail(`${scenario.id}: typed_obj_cap (PLAIN-OBJECT-WITH-OWN-KEYS-AS-TYPED → typed) must NOT produce a warning`);
  }

  // Assert PLAIN-OBJECT-WITH-OWN-KEYS-AS-TYPED: typed_bool_cap (boolean subschema) → typed bucket
  if (!typedEntry || !typedEntry.capabilities.includes('typed_bool_cap')) {
    fail(`${scenario.id}: typed_bool_cap (patternProperties={'^x-':true}) must be in typed bucket (PLAIN-OBJECT-WITH-OWN-KEYS-AS-TYPED boolean subschema)`);
  }

  // Assert ABSENT-AS-UNTYPED: untyped_absent_cap → untyped bucket
  const untypedEntry = index.first_property_pattern_properties_states.find((e) => e.input_schema_first_property_pattern_properties === 'untyped');
  if (!untypedEntry || !untypedEntry.capabilities.includes('untyped_absent_cap')) {
    fail(`${scenario.id}: untyped_absent_cap (patternProperties absent) must be in untyped bucket (ABSENT-AS-UNTYPED)`);
  }
  if (index.warnings.some((w) => w.capability === 'untyped_absent_cap')) {
    fail(`${scenario.id}: untyped_absent_cap (ABSENT-AS-UNTYPED → untyped) must NOT produce a warning`);
  }

  // Assert NULL-AS-ABSENT: untyped_null_cap → untyped bucket
  if (!untypedEntry || !untypedEntry.capabilities.includes('untyped_null_cap')) {
    fail(`${scenario.id}: untyped_null_cap (patternProperties=null) must be in untyped bucket (NULL-AS-ABSENT)`);
  }
  if (index.warnings.some((w) => w.capability === 'untyped_null_cap')) {
    fail(`${scenario.id}: untyped_null_cap (NULL-AS-ABSENT → untyped) must NOT produce a warning`);
  }

  // Assert EMPTY-MAP-AS-UNTYPED: untyped_empty_map_cap → untyped bucket
  if (!untypedEntry || !untypedEntry.capabilities.includes('untyped_empty_map_cap')) {
    fail(`${scenario.id}: untyped_empty_map_cap (patternProperties={}) must be in untyped bucket (EMPTY-MAP-AS-UNTYPED)`);
  }
  if (index.warnings.some((w) => w.capability === 'untyped_empty_map_cap')) {
    fail(`${scenario.id}: untyped_empty_map_cap (EMPTY-MAP-AS-UNTYPED → untyped) must NOT produce a warning`);
  }

  // Assert TYPE-APPLICABILITY-OBJECT: string_type_na_cap → not_applicable bucket
  const naEntry = index.first_property_pattern_properties_states.find((e) => e.input_schema_first_property_pattern_properties === 'not_applicable');
  if (!naEntry) {
    fail(`${scenario.id}: not_applicable bucket must be present`);
  }
  if (!naEntry.capabilities.includes('string_type_na_cap')) {
  ...
  // Assert unknown bucket exists
  const unknownEntry = index.first_property_pattern_properties_states.find((e) => e.input_schema_first_property_pattern_properties === 'unknown');
  if (!unknownEntry) {
    fail(`${scenario.id}: unknown bucket must be present`);
  }

  // Assert DRAFT-7-PLAIN-OBJECT-MAP-IS-VALID-PATTERN-PROPERTIES: invalid_val_pp_cap → unknown WITH 6th code
  if (!unknownEntry.capabilities.includes('invalid_val_pp_cap')) {
    fail(`${scenario.id}: invalid_val_pp_cap (patternProperties={'^x-':'not-a-subschema'}) must be in unknown bucket (DRAFT-7-PLAIN-OBJECT-MAP-IS-VALID-PATTERN-PROPERTIES)`);
  }
  const invalidValWarning = index.warnings.find((w) => w.capability === 'invalid_val_pp_cap');
  if (!invalidValWarning || invalidValWarning.reason !== 'input_schema_properties_first_property_pattern_properties_invalid_when_present') {
    fail(`${scenario.id}: invalid_val_pp_cap must produce 6th-code warning 'input_schema_properties_first_property_pattern_properties_invalid_when_present'; got: ${JSON.stringify(invalidValWarning)}`);
  }

  // Assert typed bucket has aggregation_key 'object_property_pattern_map_constraint'
  if (typedEntry.aggregation_key !== 'object_property_pattern_map_constraint') {
    fail(`${scenario.id}: typed bucket must have aggregation_key 'object_property_pattern_map_constraint'; got '${typedEntry.aggregation_key}'`);
  }

  // Assert untyped bucket has aggregation_key 'object_property_pattern_map_constraint'
  if (untypedEntry.aggregation_key !== 'object_property_pattern_map_constraint') {
    fail(`${scenario.id}: untyped bucket must have aggregation_key 'object_property_pattern_map_constraint'; got '${untypedEntry.aggregation_key}'`);
  }
...
async function runInputSchemaFirstPropertyPropertyNamesIndexDeterminismScenario(scenario, project) {
  const capabilities = scenario.synthetic_capabilities.map((cap) => {
    const obj = {
      name: cap.name,
      method: cap.method,
  ...
  // Run tusq named index --json three times and assert byte-identical output
  const run1 = runCli(['named', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run2 = runCli(['named', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run3 = runCli(['named', 'index', '--manifest', manifestPath, '--json'], { cwd: project });

  if (run1.stdout !== run2.stdout || run2.stdout !== run3.stdout) {
    fail(`${scenario.id}: named index --json output is not byte-identical across three runs`);
  }

  // Assert input_schema_first_property_property_names bucket-key enum is closed
  const index = JSON.parse(run1.stdout);
  const validPnValues = new Set(scenario.expected_valid_property_names_values);
  for (const entry of index.first_property_property_names_states) {
    if (!validPnValues.has(entry.input_schema_first_property_property_names)) {
      fail(`${scenario.id}: input_schema_first_property_property_names '${entry.input_schema_first_property_property_names}' is outside the closed four-value enum`);
    }
  }
  ...
  for (const entry of index.first_property_property_names_states) {
    if (!validAggKeys.has(entry.aggregation_key)) {
      fail(`${scenario.id}: aggregation_key '${entry.aggregation_key}' is outside the closed three-value enum for bucket '${entry.input_schema_first_property_property_names}'`);
    }
  }

  // Assert bucket order matches expected
  const bucketOrder = index.first_property_property_names_states.map((e) => e.input_schema_first_property_property_names).join(',');
  if (bucketOrder !== scenario.expected_bucket_order) {
    fail(`${scenario.id}: bucket order '${bucketOrder}' does not match expected '${scenario.expected_bucket_order}'`);
  }
  ...
  // Assert manifest is not mutated (input_schema_first_property_property_names must NOT be written into manifest)
  const manifestAfter = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
  for (const cap of manifestAfter.capabilities) {
    if (Object.prototype.hasOwnProperty.call(cap, 'input_schema_first_property_property_names')) {
      fail(`${scenario.id}: input_schema_first_property_property_names must NOT be written into tusq.manifest.json; found on capability '${cap.name}'`);
    }
  }

  // Assert BOOLEAN-FALSE-AS-TYPED: typed_false_cap → typed bucket
  const typedEntry = index.first_property_property_names_states.find((e) => e.input_schema_first_property_property_names === 'typed');
  if (!typedEntry || !typedEntry.capabilities.includes('typed_false_cap')) {
    fail(`${scenario.id}: typed_false_cap (propertyNames=false) must be in typed bucket (BOOLEAN-FALSE-AS-TYPED)`);
  }
  if (index.warnings.some((w) => w.capability === 'typed_false_cap')) {
    fail(`${scenario.id}: typed_false_cap (BOOLEAN-FALSE-AS-TYPED → typed) must NOT produce a warning`);
  }

  // Assert PLAIN-OBJECT-WITH-OWN-KEYS-AS-TYPED: typed_obj_cap → typed bucket
  if (!typedEntry || !typedEntry.capabilities.includes('typed_obj_cap')) {
    fail(`${scenario.id}: typed_obj_cap (propertyNames={pattern:'^[a-z]+$'}) must be in typed bucket (PLAIN-OBJECT-WITH-OWN-KEYS-AS-TYPED)`);
  }
  if (index.warnings.some((w) => w.capability === 'typed_obj_cap')) {
    fail(`${scenario.id}: typed_obj_cap (PLAIN-OBJECT-WITH-OWN-KEYS-AS-TYPED → typed) must NOT produce a warning`);
  }
  ...
  // Assert ABSENT-AS-UNTYPED: untyped_absent_cap → untyped bucket
  const untypedEntry = index.first_property_property_names_states.find((e) => e.input_schema_first_property_property_names === 'untyped');
  if (!untypedEntry || !untypedEntry.capabilities.includes('untyped_absent_cap')) {
    fail(`${scenario.id}: untyped_absent_cap (propertyNames absent) must be in untyped bucket (ABSENT-AS-UNTYPED)`);
  }
  if (index.warnings.some((w) => w.capability === 'untyped_absent_cap')) {
    fail(`${scenario.id}: untyped_absent_cap (ABSENT-AS-UNTYPED → untyped) must NOT produce a warning`);
  }
  ...
  // Assert NULL-AS-ABSENT: untyped_null_cap → untyped bucket
  if (!untypedEntry || !untypedEntry.capabilities.includes('untyped_null_cap')) {
    fail(`${scenario.id}: untyped_null_cap (propertyNames=null) must be in untyped bucket (NULL-AS-ABSENT)`);
  }
  if (index.warnings.some((w) => w.capability === 'untyped_null_cap')) {
    fail(`${scenario.id}: untyped_null_cap (NULL-AS-ABSENT → untyped) must NOT produce a warning`);
  }

  // Assert BOOLEAN-TRUE-AS-UNTYPED: untyped_true_cap → untyped bucket
  if (!untypedEntry || !untypedEntry.capabilities.includes('untyped_true_cap')) {
    fail(`${scenario.id}: untyped_true_cap (propertyNames=true) must be in untyped bucket (BOOLEAN-TRUE-AS-UNTYPED)`);
  }
  if (index.warnings.some((w) => w.capability === 'untyped_true_cap')) {
    fail(`${scenario.id}: untyped_true_cap (BOOLEAN-TRUE-AS-UNTYPED → untyped) must NOT produce a warning`);
  }

  // Assert EMPTY-OBJECT-SCHEMA-AS-UNTYPED: untyped_empty_schema_cap → untyped bucket
  if (!untypedEntry || !untypedEntry.capabilities.includes('untyped_empty_schema_cap')) {
    fail(`${scenario.id}: untyped_empty_schema_cap (propertyNames={}) must be in untyped bucket (EMPTY-OBJECT-SCHEMA-AS-UNTYPED)`);
  }
  if (index.warnings.some((w) => w.capability === 'untyped_empty_schema_cap')) {
    fail(`${scenario.id}: untyped_empty_schema_cap (EMPTY-OBJECT-SCHEMA-AS-UNTYPED → untyped) must NOT produce a warning`);
  }

  // Assert TYPE-APPLICABILITY-OBJECT: string_type_na_cap → not_applicable bucket
  const naEntry = index.first_property_property_names_states.find((e) => e.input_schema_first_property_property_names === 'not_applicable');
  if (!naEntry) {
    fail(`${scenario.id}: not_applicable bucket must be present`);
  }
  if (!naEntry.capabilities.includes('string_type_na_cap')) {
  ...
  // Assert unknown bucket exists
  const unknownEntry = index.first_property_property_names_states.find((e) => e.input_schema_first_property_property_names === 'unknown');
  if (!unknownEntry) {
    fail(`${scenario.id}: unknown bucket must be present`);
  }

  // Assert DRAFT-7-BOOLEAN-OR-OBJECT-IS-VALID-PROPERTY-NAMES: invalid_pn_cap → unknown WITH 6th code
  if (!unknownEntry.capabilities.includes('invalid_pn_cap')) {
    fail(`${scenario.id}: invalid_pn_cap (propertyNames='not-a-schema') must be in unknown bucket (DRAFT-7-BOOLEAN-OR-OBJECT-IS-VALID-PROPERTY-NAMES)`);
  }
  const invalidPnWarning = index.warnings.find((w) => w.capability === 'invalid_pn_cap');
  if (!invalidPnWarning || invalidPnWarning.reason !== 'input_schema_properties_first_property_property_names_invalid_when_present') {
    fail(`${scenario.id}: invalid_pn_cap must produce 6th-code warning 'input_schema_properties_first_property_property_names_invalid_when_present'; got: ${JSON.stringify(invalidPnWarning)}`);
  }

  // Assert typed bucket has aggregation_key 'object_property_name_subschema_constraint'
  if (typedEntry.aggregation_key !== 'object_property_name_subschema_constraint') {
    fail(`${scenario.id}: typed bucket must have aggregation_key 'object_property_name_subschema_constraint'; got '${typedEntry.aggregation_key}'`);
  }

  // Assert untyped bucket has aggregation_key 'object_property_name_subschema_constraint'
  if (untypedEntry.aggregation_key !== 'object_property_name_subschema_constraint') {
    fail(`${scenario.id}: untyped bucket must have aggregation_key 'object_property_name_subschema_constraint'; got '${untypedEntry.aggregation_key}'`);
  }
...
async function runInputSchemaFirstPropertyRequiredIndexDeterminismScenario(scenario, tmpRoot) {
  const capabilities = scenario.synthetic_capabilities.map((cap) => {
    const obj = {
      name: cap.name,
      method: cap.method,
  ...
  // Run tusq required index --json three times and assert byte-identical output
  const run1 = runCli(['required', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run2 = runCli(['required', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run3 = runCli(['required', 'index', '--manifest', manifestPath, '--json'], { cwd: project });

  if (run1.stdout !== run2.stdout || run2.stdout !== run3.stdout) {
    fail(`${scenario.id}: required index --json output is not byte-identical across three runs`);
  }

  // Assert input_schema_first_property_required bucket-key enum is closed
  const index = JSON.parse(run1.stdout);
  const validReqValues = new Set(scenario.expected_valid_required_values);
  for (const entry of index.first_property_required_states) {
    if (!validReqValues.has(entry.input_schema_first_property_required)) {
      fail(`${scenario.id}: input_schema_first_property_required '${entry.input_schema_first_property_required}' is outside the closed four-value enum`);
    }
  }
  ...
  for (const entry of index.first_property_required_states) {
    if (!validAggKeys.has(entry.aggregation_key)) {
      fail(`${scenario.id}: aggregation_key '${entry.aggregation_key}' is outside the closed three-value enum for bucket '${entry.input_schema_first_property_required}'`);
    }
  }

  // Assert bucket order matches expected
  const bucketOrder = index.first_property_required_states.map((e) => e.input_schema_first_property_required).join(',');
  if (bucketOrder !== scenario.expected_bucket_order) {
    fail(`${scenario.id}: bucket order '${bucketOrder}' does not match expected '${scenario.expected_bucket_order}'`);
  }
  ...
  // Assert manifest is not mutated (input_schema_first_property_required must NOT be written into manifest)
  const manifestAfter = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
  for (const cap of manifestAfter.capabilities) {
    if (Object.prototype.hasOwnProperty.call(cap, 'input_schema_first_property_required')) {
      fail(`${scenario.id}: input_schema_first_property_required must NOT be written into tusq.manifest.json; found on capability '${cap.name}'`);
    }
  }

  // Assert NON-EMPTY-STRING-ARRAY-AS-TYPED: typed_req_cap → typed bucket
  const typedEntry = index.first_property_required_states.find((e) => e.input_schema_first_property_required === 'typed');
  if (!typedEntry || !typedEntry.capabilities.includes('typed_req_cap')) {
    fail(`${scenario.id}: typed_req_cap (required=['name','email']) must be in typed bucket (NON-EMPTY-STRING-ARRAY-AS-TYPED)`);
  }
  if (index.warnings.some((w) => w.capability === 'typed_req_cap')) {
    fail(`${scenario.id}: typed_req_cap (NON-EMPTY-STRING-ARRAY-AS-TYPED → typed) must NOT produce a warning`);
  }

  // Assert ABSENT-AS-UNTYPED: untyped_absent_cap → untyped bucket
  const untypedEntry = index.first_property_required_states.find((e) => e.input_schema_first_property_required === 'untyped');
  if (!untypedEntry || !untypedEntry.capabilities.includes('untyped_absent_cap')) {
    fail(`${scenario.id}: untyped_absent_cap (required absent) must be in untyped bucket (ABSENT-AS-UNTYPED)`);
  }
  if (index.warnings.some((w) => w.capability === 'untyped_absent_cap')) {
    fail(`${scenario.id}: untyped_absent_cap (ABSENT-AS-UNTYPED → untyped) must NOT produce a warning`);
  }
  ...
  // Assert NULL-AS-ABSENT: untyped_null_cap → untyped bucket
  if (!untypedEntry || !untypedEntry.capabilities.includes('untyped_null_cap')) {
    fail(`${scenario.id}: untyped_null_cap (required=null) must be in untyped bucket (NULL-AS-ABSENT)`);
  }
  if (index.warnings.some((w) => w.capability === 'untyped_null_cap')) {
    fail(`${scenario.id}: untyped_null_cap (NULL-AS-ABSENT → untyped) must NOT produce a warning`);
  }

  // Assert EMPTY-ARRAY-AS-UNTYPED: untyped_empty_arr_cap → untyped bucket
  if (!untypedEntry || !untypedEntry.capabilities.includes('untyped_empty_arr_cap')) {
    fail(`${scenario.id}: untyped_empty_arr_cap (required=[]) must be in untyped bucket (EMPTY-ARRAY-AS-UNTYPED — M82-SPECIFIC)`);
  }
  if (index.warnings.some((w) => w.capability === 'untyped_empty_arr_cap')) {
    fail(`${scenario.id}: untyped_empty_arr_cap (EMPTY-ARRAY-AS-UNTYPED → untyped) must NOT produce a warning`);
  }

  // Assert TYPE-APPLICABILITY-OBJECT: string_type_na_cap → not_applicable bucket
  const naEntry = index.first_property_required_states.find((e) => e.input_schema_first_property_required === 'not_applicable');
  if (!naEntry) {
    fail(`${scenario.id}: not_applicable bucket must be present`);
  }
  if (!naEntry.capabilities.includes('string_type_na_cap')) {
  ...
  // Assert unknown bucket exists
  const unknownEntry = index.first_property_required_states.find((e) => e.input_schema_first_property_required === 'unknown');
  if (!unknownEntry) {
    fail(`${scenario.id}: unknown bucket must be present`);
  }

  // Assert DRAFT-7-ARRAY-OF-NON-EMPTY-STRING-IS-VALID-REQUIRED: invalid_req_cap → unknown WITH 6th code
  if (!unknownEntry.capabilities.includes('invalid_req_cap')) {
    fail(`${scenario.id}: invalid_req_cap (required='not-an-array') must be in unknown bucket (DRAFT-7-ARRAY-OF-NON-EMPTY-STRING-IS-VALID-REQUIRED)`);
  }
  const invalidReqWarning = index.warnings.find((w) => w.capability === 'invalid_req_cap');
  if (!invalidReqWarning || invalidReqWarning.reason !== 'input_schema_properties_first_property_required_invalid_when_present') {
    fail(`${scenario.id}: invalid_req_cap must produce 6th-code warning 'input_schema_properties_first_property_required_invalid_when_present'; got: ${JSON.stringify(invalidReqWarning)}`);
  }

  // Assert typed bucket has aggregation_key 'object_property_required_keys_constraint'
  if (typedEntry.aggregation_key !== 'object_property_required_keys_constraint') {
    fail(`${scenario.id}: typed bucket must have aggregation_key 'object_property_required_keys_constraint'; got '${typedEntry.aggregation_key}'`);
  }

  // Assert untyped bucket has aggregation_key 'object_property_required_keys_constraint'
  if (untypedEntry.aggregation_key !== 'object_property_required_keys_constraint') {
    fail(`${scenario.id}: untyped bucket must have aggregation_key 'object_property_required_keys_constraint'; got '${untypedEntry.aggregation_key}'`);
  }
...
async function runInputSchemaFirstPropertyDependenciesIndexDeterminismScenario(scenario, tmpRoot) {
  const capabilities = scenario.synthetic_capabilities.map((cap) => {
    const obj = {
      name: cap.name,
      method: cap.method,
  ...
  // Run tusq dependent index --json three times and assert byte-identical output
  const run1 = runCli(['dependent', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run2 = runCli(['dependent', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run3 = runCli(['dependent', 'index', '--manifest', manifestPath, '--json'], { cwd: project });

  if (run1.stdout !== run2.stdout || run2.stdout !== run3.stdout) {
    fail(`${scenario.id}: dependent index --json output is not byte-identical across three runs`);
  }

  // Assert input_schema_first_property_dependencies bucket-key enum is closed
  const index = JSON.parse(run1.stdout);
  const validDepValues = new Set(scenario.expected_valid_required_values);
  for (const entry of index.first_property_dependencies_states) {
    if (!validDepValues.has(entry.input_schema_first_property_dependencies)) {
      fail(`${scenario.id}: input_schema_first_property_dependencies '${entry.input_schema_first_property_dependencies}' is outside the closed four-value enum`);
    }
  }
  ...
  for (const entry of index.first_property_dependencies_states) {
    if (!validAggKeys.has(entry.aggregation_key)) {
      fail(`${scenario.id}: aggregation_key '${entry.aggregation_key}' is outside the closed three-value enum for bucket '${entry.input_schema_first_property_dependencies}'`);
    }
  }

  // Assert bucket order matches expected
  const bucketOrder = index.first_property_dependencies_states.map((e) => e.input_schema_first_property_dependencies).join(',');
  if (bucketOrder !== scenario.expected_bucket_order) {
    fail(`${scenario.id}: bucket order '${bucketOrder}' does not match expected '${scenario.expected_bucket_order}'`);
  }
  ...
  // Assert manifest is not mutated (input_schema_first_property_dependencies must NOT be written into manifest)
  const manifestAfter = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
  for (const cap of manifestAfter.capabilities) {
    if (Object.prototype.hasOwnProperty.call(cap, 'input_schema_first_property_dependencies')) {
      fail(`${scenario.id}: input_schema_first_property_dependencies must NOT be written into tusq.manifest.json; found on capability '${cap.name}'`);
    }
  }

  // Assert NON-EMPTY-PLAIN-OBJECT-WITH-VALID-HETEROGENEOUS-VALUES-AS-TYPED: typed_dep_list_cap → typed bucket
  const typedEntry = index.first_property_dependencies_states.find((e) => e.input_schema_first_property_dependencies === 'typed');
  if (!typedEntry || !typedEntry.capabilities.includes('typed_dep_list_cap')) {
    fail(`${scenario.id}: typed_dep_list_cap (dependencies={prop:['other']}) must be in typed bucket (NON-EMPTY-PLAIN-OBJECT-WITH-VALID-HETEROGENEOUS-VALUES-AS-TYPED)`);
  }
  if (index.warnings.some((w) => w.capability === 'typed_dep_list_cap')) {
    fail(`${scenario.id}: typed_dep_list_cap (LIST form → typed) must NOT produce a warning`);
  }

  // Assert M83-SPECIFIC empty array [] PERMITTED as LIST entry: typed_dep_empty_list_cap → typed bucket
  if (!typedEntry || !typedEntry.capabilities.includes('typed_dep_empty_list_cap')) {
    fail(`${scenario.id}: typed_dep_empty_list_cap (dependencies={prop:[]}) must be in typed bucket (empty [] PERMITTED as LIST entry — M83-SPECIFIC)`);
  }
  if (index.warnings.some((w) => w.capability === 'typed_dep_empty_list_cap')) {
    fail(`${scenario.id}: typed_dep_empty_list_cap (empty LIST form → typed) must NOT produce a warning`);
  }

  // Assert ABSENT-AS-UNTYPED: untyped_dep_absent_cap → untyped bucket
  const untypedEntry = index.first_property_dependencies_states.find((e) => e.input_schema_first_property_dependencies === 'untyped');
  if (!untypedEntry || !untypedEntry.capabilities.includes('untyped_dep_absent_cap')) {
    fail(`${scenario.id}: untyped_dep_absent_cap (dependencies absent) must be in untyped bucket (ABSENT-AS-UNTYPED)`);
  }
  if (index.warnings.some((w) => w.capability === 'untyped_dep_absent_cap')) {
    fail(`${scenario.id}: untyped_dep_absent_cap (ABSENT-AS-UNTYPED → untyped) must NOT produce a warning`);
  }

  // Assert NULL-AS-ABSENT: untyped_dep_null_cap → untyped bucket
  if (!untypedEntry || !untypedEntry.capabilities.includes('untyped_dep_null_cap')) {
    fail(`${scenario.id}: untyped_dep_null_cap (dependencies=null) must be in untyped bucket (NULL-AS-ABSENT)`);
  }
  if (index.warnings.some((w) => w.capability === 'untyped_dep_null_cap')) {
    fail(`${scenario.id}: untyped_dep_null_cap (NULL-AS-ABSENT → untyped) must NOT produce a warning`);
  }

  // Assert EMPTY-MAP-AS-UNTYPED: untyped_dep_empty_map_cap → untyped bucket (M83-SPECIFIC)
  if (!untypedEntry || !untypedEntry.capabilities.includes('untyped_dep_empty_map_cap')) {
    fail(`${scenario.id}: untyped_dep_empty_map_cap (dependencies={}) must be in untyped bucket (EMPTY-MAP-AS-UNTYPED — M83-SPECIFIC)`);
  }
  if (index.warnings.some((w) => w.capability === 'untyped_dep_empty_map_cap')) {
    fail(`${scenario.id}: untyped_dep_empty_map_cap (EMPTY-MAP-AS-UNTYPED → untyped) must NOT produce a warning`);
  }

  // Assert TYPE-APPLICABILITY-OBJECT: na_dep_string_type_cap → not_applicable bucket
  const naEntry = index.first_property_dependencies_states.find((e) => e.input_schema_first_property_dependencies === 'not_applicable');
  if (!naEntry) {
    fail(`${scenario.id}: not_applicable bucket must be present`);
  }
  if (!naEntry.capabilities.includes('na_dep_string_type_cap')) {
    fail(`${scenario.id}: na_dep_string_type_cap (firstKey.type='string') must be in not_applicable bucket (TYPE-APPLICABILITY-OBJECT)`);
  }

  // Assert unknown bucket exists
  const unknownEntry = index.first_property_dependencies_states.find((e) => e.input_schema_first_property_dependencies === 'unknown');
  if (!unknownEntry) {
    fail(`${scenario.id}: unknown bucket must be present`);
  }

  // Assert DRAFT-7-MAP-WITH-VALID-HETEROGENEOUS-VALUES-IS-VALID-DEPENDENCIES: unknown_dep_invalid_cap → unknown WITH 6th code
  if (!unknownEntry.capabilities.includes('unknown_dep_invalid_cap')) {
    fail(`${scenario.id}: unknown_dep_invalid_cap (dependencies='not-an-object') must be in unknown bucket (DRAFT-7-MAP-WITH-VALID-HETEROGENEOUS-VALUES-IS-VALID-DEPENDENCIES)`);
  }
  const invalidDepWarning = index.warnings.find((w) => w.capability === 'unknown_dep_invalid_cap');
  if (!invalidDepWarning || invalidDepWarning.reason !== 'input_schema_properties_first_property_dependencies_invalid_when_present') {
    fail(`${scenario.id}: unknown_dep_invalid_cap must produce 6th-code warning 'input_schema_properties_first_property_dependencies_invalid_when_present'; got: ${JSON.stringify(invalidDepWarning)}`);
  }

  // Assert typed bucket has aggregation_key 'object_property_dependencies_constraint'
  if (typedEntry.aggregation_key !== 'object_property_dependencies_constraint') {
    fail(`${scenario.id}: typed bucket must have aggregation_key 'object_property_dependencies_constraint'; got '${typedEntry.aggregation_key}'`);
  }

  // Assert untyped bucket has aggregation_key 'object_property_dependencies_constraint'
  if (untypedEntry.aggregation_key !== 'object_property_dependencies_constraint') {
    fail(`${scenario.id}: untyped bucket must have aggregation_key 'object_property_dependencies_constraint'; got '${untypedEntry.aggregation_key}'`);
  }
...
async function runInputSchemaFirstPropertyConstantIndexDeterminismScenario(scenario, tmpRoot) {
  const capabilities = scenario.synthetic_capabilities.map((cap) => {
    const obj = {
      name: cap.name,
      method: cap.method,
  ...
  // Patch unknown_const_invalid_cap to have const:NaN (non-JSON value) for 6th-code testing
  const unknownCap = capabilities.find((c) => c.name === 'unknown_const_invalid_cap');
  if (unknownCap && unknownCap.input_schema && unknownCap.input_schema.properties && unknownCap.input_schema.properties.state) {
    unknownCap.input_schema.properties.state.const = NaN;
  }
  ...
  await fs.mkdir(project, { recursive: true });
  const manifestPath = path.join(project, 'tusq.manifest.json');
  // Write only JSON-serializable capabilities (NaN becomes null in JSON, function drops)
  // We use the in-memory manifest for direct testing below
  const serializableManifest = {
    ...manifest,
    capabilities: capabilities.filter((c) => c.name !== 'unknown_const_invalid_cap')
  };
  await writeJson(manifestPath, serializableManifest);

  // Run tusq constant index --json three times and assert byte-identical output
  const run1 = runCli(['constant', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run2 = runCli(['constant', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run3 = runCli(['constant', 'index', '--manifest', manifestPath, '--json'], { cwd: project });

  if (run1.stdout !== run2.stdout || run2.stdout !== run3.stdout) {
    fail(`${scenario.id}: constant index --json output is not byte-identical across three runs`);
  }

  // Assert input_schema_first_property_const bucket-key enum is closed
  const index = JSON.parse(run1.stdout);
  const validConstValues = new Set(scenario.expected_valid_required_values);
  for (const entry of index.first_property_const_states) {
    if (!validConstValues.has(entry.input_schema_first_property_const)) {
      fail(`${scenario.id}: input_schema_first_property_const '${entry.input_schema_first_property_const}' is outside the closed four-value enum`);
    }
  ...
  // Assert JSON-STRING-AS-TYPED: typed_const_string_cap → typed bucket
  const typedEntry = index.first_property_const_states.find((e) => e.input_schema_first_property_const === 'typed');
  if (!typedEntry || !typedEntry.capabilities.includes('typed_const_string_cap')) {
    fail(`${scenario.id}: typed_const_string_cap (const='pending') must be in typed bucket (JSON-STRING-AS-TYPED)`);
  }
  if (index.warnings.some((w) => w.capability === 'typed_const_string_cap')) {
    fail(`${scenario.id}: typed_const_string_cap (string const → typed) must NOT produce a warning`);
  }

  // Assert NULL-AS-TYPED (M84-SPECIFIC): typed_const_null_cap → typed bucket
  if (!typedEntry || !typedEntry.capabilities.includes('typed_const_null_cap')) {
    fail(`${scenario.id}: typed_const_null_cap (const=null) must be in typed bucket (NULL-AS-TYPED — M84-SPECIFIC: distinct from M55–M83 NULL-AS-ABSENT)`);
  }
  if (index.warnings.some((w) => w.capability === 'typed_const_null_cap')) {
    fail(`${scenario.id}: typed_const_null_cap (NULL-AS-TYPED → typed) must NOT produce a warning`);
  }

  // Assert ABSENT-AS-UNTYPED: untyped_const_absent_cap → untyped bucket
  const untypedEntry = index.first_property_const_states.find((e) => e.input_schema_first_property_const === 'untyped');
  if (!untypedEntry || !untypedEntry.capabilities.includes('untyped_const_absent_cap')) {
    fail(`${scenario.id}: untyped_const_absent_cap (const absent) must be in untyped bucket (ABSENT-AS-UNTYPED)`);
  }
  if (index.warnings.some((w) => w.capability === 'untyped_const_absent_cap')) {
    fail(`${scenario.id}: untyped_const_absent_cap (ABSENT-AS-UNTYPED → untyped) must NOT produce a warning`);
  }
  ...
  if (!naEntry.capabilities.includes('na_const_string_outer_cap')) {
    fail(`${scenario.id}: na_const_string_outer_cap (inputSchema.type='string') must be in not_applicable bucket (outer-schema structural prerequisite)`);
  }

  // Assert typed bucket has aggregation_key 'value_pinning_constraint'
  if (typedEntry.aggregation_key !== 'value_pinning_constraint') {
    fail(`${scenario.id}: typed bucket must have aggregation_key 'value_pinning_constraint'; got '${typedEntry.aggregation_key}'`);
  }
...
async function runInputSchemaFirstPropertyPrefillIndexDeterminismScenario(scenario, tmpRoot) {
  const capabilities = scenario.synthetic_capabilities.map((cap) => {
    const obj = {
      name: cap.name,
      method: cap.method,
  ...
  // Patch unknown_default_invalid_cap to have default:NaN (non-JSON value) for 6th-code testing
  const unknownCap = capabilities.find((c) => c.name === 'unknown_default_invalid_cap');
  if (unknownCap && unknownCap.input_schema && unknownCap.input_schema.properties && unknownCap.input_schema.properties.rate) {
    unknownCap.input_schema.properties.rate.default = NaN;
  }
  ...
  const serializableManifest = {
    ...manifest,
    capabilities: capabilities.filter((c) => c.name !== 'unknown_default_invalid_cap')
  };
  await writeJson(manifestPath, serializableManifest);

  // Run tusq prefill index --json three times and assert byte-identical output
  const run1 = runCli(['prefill', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run2 = runCli(['prefill', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run3 = runCli(['prefill', 'index', '--manifest', manifestPath, '--json'], { cwd: project });

  if (run1.stdout !== run2.stdout || run2.stdout !== run3.stdout) {
    fail(`${scenario.id}: prefill index --json output is not byte-identical across three runs`);
  }

  // Assert input_schema_first_property_default bucket-key enum is closed
  const index = JSON.parse(run1.stdout);
  const validEnumValues = new Set(scenario.expected_valid_required_values);
  for (const entry of index.first_property_default_states) {
    if (!validEnumValues.has(entry.input_schema_first_property_default)) {
      fail(`${scenario.id}: input_schema_first_property_default '${entry.input_schema_first_property_default}' is outside the closed four-value enum`);
    }
  }
  ...
  for (const entry of index.first_property_default_states) {
    if (!validAggKeys.has(entry.aggregation_key)) {
      fail(`${scenario.id}: aggregation_key '${entry.aggregation_key}' is outside the closed three-value enum for bucket '${entry.input_schema_first_property_default}'`);
    }
  }

  // Assert bucket order matches expected (typed,untyped,not_applicable from serializable manifest)
  const bucketOrder = index.first_property_default_states.map((e) => e.input_schema_first_property_default).join(',');
  const expectedBucketOrder = 'typed,untyped,not_applicable';
  if (bucketOrder !== expectedBucketOrder) {
    fail(`${scenario.id}: bucket order '${bucketOrder}' does not match expected '${expectedBucketOrder}'`);
  }
  ...
  // Assert manifest is not mutated (input_schema_first_property_default must NOT be written into manifest)
  const manifestAfter = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
  for (const cap of manifestAfter.capabilities) {
    if (Object.prototype.hasOwnProperty.call(cap, 'input_schema_first_property_default')) {
      fail(`${scenario.id}: input_schema_first_property_default must NOT be written into tusq.manifest.json; found on capability '${cap.name}'`);
    }
  }

  // Assert NULL-AS-TYPED (M86-INHERITED-FROM-M84): typed_default_null_cap → typed bucket
  const typedEntry = index.first_property_default_states.find((e) => e.input_schema_first_property_default === 'typed');
  if (!typedEntry || !typedEntry.capabilities.includes('typed_default_null_cap')) {
    fail(`${scenario.id}: typed_default_null_cap (default=null) must be in typed bucket (NULL-AS-TYPED M86-INHERITED-FROM-M84: distinct from M85 NULL-AS-ABSENT for enum)`);
  }
  if (index.warnings.some((w) => w.capability === 'typed_default_null_cap')) {
    fail(`${scenario.id}: typed_default_null_cap (NULL-AS-TYPED → typed) must NOT produce a warning`);
  }

  // Assert JSON-BOOLEAN-AS-TYPED (NO falsy-coercion): typed_default_false_cap → typed bucket
  if (!typedEntry || !typedEntry.capabilities.includes('typed_default_false_cap')) {
    fail(`${scenario.id}: typed_default_false_cap (default=false) must be in typed bucket (JSON-BOOLEAN-AS-TYPED; NO falsy-coercion)`);
  }

  // Assert JSON-ARRAY-AS-TYPED-INCLUDING-EMPTY: typed_default_empty_arr_cap → typed bucket
  if (!typedEntry || !typedEntry.capabilities.includes('typed_default_empty_arr_cap')) {
    fail(`${scenario.id}: typed_default_empty_arr_cap (default=[]) must be in typed bucket (JSON-ARRAY-AS-TYPED-INCLUDING-EMPTY M86-DISTINCT-FROM-M82/M85)`);
  }

  // Assert JSON-PLAIN-OBJECT-AS-TYPED-INCLUDING-EMPTY: typed_default_empty_obj_cap → typed bucket
  if (!typedEntry || !typedEntry.capabilities.includes('typed_default_empty_obj_cap')) {
    fail(`${scenario.id}: typed_default_empty_obj_cap (default={}) must be in typed bucket (JSON-PLAIN-OBJECT-AS-TYPED-INCLUDING-EMPTY M86-DISTINCT-FROM-M81)`);
  }

  // Assert ABSENT-AS-UNTYPED: untyped_default_absent_cap → untyped bucket
  const untypedEntry = index.first_property_default_states.find((e) => e.input_schema_first_property_default === 'untyped');
  if (!untypedEntry || !untypedEntry.capabilities.includes('untyped_default_absent_cap')) {
    fail(`${scenario.id}: untyped_default_absent_cap (default absent) must be in untyped bucket (ABSENT-AS-UNTYPED)`);
  }
  if (index.warnings.some((w) => w.capability === 'untyped_default_absent_cap')) {
    fail(`${scenario.id}: untyped_default_absent_cap (ABSENT-AS-UNTYPED → untyped) must NOT produce a warning`);
  }

  // Assert not_applicable bucket exists
  const naEntry = index.first_property_default_states.find((e) => e.input_schema_first_property_default === 'not_applicable');
  if (!naEntry) {
    fail(`${scenario.id}: not_applicable bucket must be present`);
  }
  if (!naEntry.capabilities.includes('na_default_string_outer_cap')) {
    fail(`${scenario.id}: na_default_string_outer_cap (inputSchema.type='string') must be in not_applicable bucket (outer-schema structural prerequisite)`);
  }

  // Assert typed bucket has aggregation_key 'sample_instance_value'
  if (typedEntry.aggregation_key !== 'sample_instance_value') {
    fail(`${scenario.id}: typed bucket must have aggregation_key 'sample_instance_value'; got '${typedEntry.aggregation_key}'`);
  }

  // Assert untyped bucket has aggregation_key 'sample_instance_value'
  if (untypedEntry.aggregation_key !== 'sample_instance_value') {
    fail(`${scenario.id}: untyped bucket must have aggregation_key 'sample_instance_value'; got '${untypedEntry.aggregation_key}'`);
  }
...
async function runInputSchemaFirstPropertyAdditionalItemsIndexDeterminismScenario(scenario, tmpRoot) {
  const capabilities = scenario.synthetic_capabilities.map((cap) => {
    const obj = {
      name: cap.name,
      method: cap.method,
  ...
  // Run tusq extra index --json three times and assert byte-identical output
  const run1 = runCli(['extra', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run2 = runCli(['extra', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run3 = runCli(['extra', 'index', '--manifest', manifestPath, '--json'], { cwd: project });

  if (run1.stdout !== run2.stdout || run2.stdout !== run3.stdout) {
    fail(`${scenario.id}: extra index --json output is not byte-identical across three runs`);
  }

  // Assert input_schema_first_property_additional_items bucket-key enum is closed
  const index = JSON.parse(run1.stdout);
  const validEnumValues = new Set(scenario.expected_valid_required_values);
  for (const entry of index.first_property_additional_items_states) {
    if (!validEnumValues.has(entry.input_schema_first_property_additional_items)) {
      fail(`${scenario.id}: input_schema_first_property_additional_items '${entry.input_schema_first_property_additional_items}' is outside the closed four-value enum`);
    }
  }
  ...
  for (const entry of index.first_property_additional_items_states) {
    if (!validAggKeys.has(entry.aggregation_key)) {
      fail(`${scenario.id}: aggregation_key '${entry.aggregation_key}' is outside the closed three-value enum for bucket '${entry.input_schema_first_property_additional_items}'`);
    }
  }

  // Assert bucket order matches expected
  const bucketOrder = index.first_property_additional_items_states.map((e) => e.input_schema_first_property_additional_items).join(',');
  const expectedBucketOrder = scenario.expected_bucket_order;
  if (bucketOrder !== expectedBucketOrder) {
    fail(`${scenario.id}: bucket order '${bucketOrder}' does not match expected '${expectedBucketOrder}'`);
  }
  ...
  // Assert manifest is not mutated (input_schema_first_property_additional_items must NOT be written into manifest)
  const manifestAfter = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
  for (const cap of manifestAfter.capabilities) {
    if (Object.prototype.hasOwnProperty.call(cap, 'input_schema_first_property_additional_items')) {
      fail(`${scenario.id}: input_schema_first_property_additional_items must NOT be written into tusq.manifest.json; found on capability '${cap.name}'`);
    }
  }

  // Assert BOOLEAN-SCHEMA-AS-TYPED: typed_bool_true_cap → typed bucket
  const typedEntry = index.first_property_additional_items_states.find((e) => e.input_schema_first_property_additional_items === 'typed');
  if (!typedEntry || !typedEntry.capabilities.includes('typed_bool_true_cap')) {
    fail(`${scenario.id}: typed_bool_true_cap (additionalItems=true) must be in typed bucket (BOOLEAN-SCHEMA-AS-TYPED: Draft-7 §4.4)`);
  }
  if (index.warnings.some((w) => w.capability === 'typed_bool_true_cap')) {
    fail(`${scenario.id}: typed_bool_true_cap (BOOLEAN-SCHEMA-AS-TYPED → typed) must NOT produce a warning`);
  }

  // Assert BOOLEAN-SCHEMA-AS-TYPED false: typed_bool_false_cap → typed bucket (distinct from falsy coercion)
  if (!typedEntry || !typedEntry.capabilities.includes('typed_bool_false_cap')) {
    fail(`${scenario.id}: typed_bool_false_cap (additionalItems=false) must be in typed bucket (BOOLEAN-SCHEMA-AS-TYPED: false is a valid boolean schema, distinct from falsy coercion)`);
  }

  // Assert EMPTY-OBJECT-SCHEMA-AS-UNTYPED (M87-INHERITED-FROM-M81): untyped_empty_obj_cap → untyped bucket
  const untypedEntry = index.first_property_additional_items_states.find((e) => e.input_schema_first_property_additional_items === 'untyped');
  if (!untypedEntry || !untypedEntry.capabilities.includes('untyped_empty_obj_cap')) {
    fail(`${scenario.id}: untyped_empty_obj_cap (additionalItems={}) must be in untyped bucket (EMPTY-OBJECT-SCHEMA-AS-UNTYPED M87-INHERITED-FROM-M81)`);
  }
  if (index.warnings.some((w) => w.capability === 'untyped_empty_obj_cap')) {
    fail(`${scenario.id}: untyped_empty_obj_cap (EMPTY-OBJECT-SCHEMA-AS-UNTYPED → untyped) must NOT produce a warning`);
  }

  // Assert not_applicable bucket exists and TYPE-APPLICABILITY-ARRAY-RESTRICTION
  const naEntry = index.first_property_additional_items_states.find((e) => e.input_schema_first_property_additional_items === 'not_applicable');
  if (!naEntry) {
    fail(`${scenario.id}: not_applicable bucket must be present`);
  }
  if (!naEntry.capabilities.includes('na_type_string_cap')) {
    fail(`${scenario.id}: na_type_string_cap (firstVal.type='string') must be in not_applicable bucket (TYPE-APPLICABILITY-ARRAY-RESTRICTION M87-SPECIFIC)`);
  }

  // Assert TUPLE-ITEMS-PREREQUISITE
  if (!naEntry.capabilities.includes('na_items_absent_cap')) {
    fail(`${scenario.id}: na_items_absent_cap (items absent) must be in not_applicable bucket (TUPLE-ITEMS-PREREQUISITE M87-SPECIFIC)`);
  }

  // Assert NULL-AS-UNKNOWN (M87-SPECIFIC): unknown_null_cap → unknown bucket
  const unknownEntry = index.first_property_additional_items_states.find((e) => e.input_schema_first_property_additional_items === 'unknown');
  if (!unknownEntry || !unknownEntry.capabilities.includes('unknown_null_cap')) {
    fail(`${scenario.id}: unknown_null_cap (additionalItems=null) must be in unknown bucket (NULL-AS-UNKNOWN M87-SPECIFIC: Draft-7 schemas are boolean-or-object; null is neither)`);
  }

  // Assert typed bucket has aggregation_key 'array_tail_items_schema'
  if (typedEntry.aggregation_key !== 'array_tail_items_schema') {
    fail(`${scenario.id}: typed bucket must have aggregation_key 'array_tail_items_schema'; got '${typedEntry.aggregation_key}'`);
  }

  // Assert untyped bucket has aggregation_key 'array_tail_items_schema'
  if (untypedEntry.aggregation_key !== 'array_tail_items_schema') {
    fail(`${scenario.id}: untyped bucket must have aggregation_key 'array_tail_items_schema'; got '${untypedEntry.aggregation_key}'`);
  }
...
async function runInputSchemaFirstPropertyContainsIndexDeterminismScenario(scenario, tmpRoot) {
  const capabilities = scenario.synthetic_capabilities.map((cap) => {
    const obj = {
      name: cap.name,
      method: cap.method,
  ...
  // Run tusq witness index --json three times and assert byte-identical output
  const run1 = runCli(['witness', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run2 = runCli(['witness', 'index', '--manifest', manifestPath, '--json'], { cwd: project });
  const run3 = runCli(['witness', 'index', '--manifest', manifestPath, '--json'], { cwd: project });

  if (run1.stdout !== run2.stdout || run2.stdout !== run3.stdout) {
    fail(`${scenario.id}: witness index --json output is not byte-identical across three runs`);
  }

  // Assert input_schema_first_property_contains bucket-key enum is closed
  const index = JSON.parse(run1.stdout);
  const validEnumValues = new Set(scenario.expected_valid_required_values);
  for (const entry of index.first_property_contains_states) {
    if (!validEnumValues.has(entry.input_schema_first_property_contains)) {
      fail(`${scenario.id}: input_schema_first_property_contains '${entry.input_schema_first_property_contains}' is outside the closed four-value enum`);
    }
  }
  ...
  for (const entry of index.first_property_contains_states) {
    if (!validAggKeys.has(entry.aggregation_key)) {
      fail(`${scenario.id}: aggregation_key '${entry.aggregation_key}' is outside the closed three-value enum for bucket '${entry.input_schema_first_property_contains}'`);
    }
  }

  // Assert bucket order matches expected
  const bucketOrder = index.first_property_contains_states.map((e) => e.input_schema_first_property_contains).join(',');
  const expectedBucketOrder = scenario.expected_bucket_order;
  if (bucketOrder !== expectedBucketOrder) {
    fail(`${scenario.id}: bucket order '${bucketOrder}' does not match expected '${expectedBucketOrder}'`);
  }
  ...
  // Assert manifest is not mutated (input_schema_first_property_contains must NOT be written into manifest)
  const manifestAfter = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
  for (const cap of manifestAfter.capabilities) {
    if (Object.prototype.hasOwnProperty.call(cap, 'input_schema_first_property_contains')) {
      fail(`${scenario.id}: input_schema_first_property_contains must NOT be written into tusq.manifest.json; found on capability '${cap.name}'`);
    }
  }

  // Assert BOOLEAN-SCHEMA-AS-TYPED: typed_bool_true_cap → typed bucket
  const typedEntry = index.first_property_contains_states.find((e) => e.input_schema_first_property_contains === 'typed');
  if (!typedEntry || !typedEntry.capabilities.includes('typed_bool_true_cap')) {
    fail(`${scenario.id}: typed_bool_true_cap (contains=true) must be in typed bucket (BOOLEAN-SCHEMA-AS-TYPED: Draft-7 §4.4)`);
  }
  if (index.warnings.some((w) => w.capability === 'typed_bool_true_cap')) {
    fail(`${scenario.id}: typed_bool_true_cap (BOOLEAN-SCHEMA-AS-TYPED → typed) must NOT produce a warning`);
  }
  ...
  // Assert BOOLEAN-SCHEMA-AS-TYPED false: typed_bool_false_cap → typed bucket (distinct from falsy coercion)
  if (!typedEntry || !typedEntry.capabilities.includes('typed_bool_false_cap')) {
    fail(`${scenario.id}: typed_bool_false_cap (contains=false) must be in typed bucket (BOOLEAN-SCHEMA-AS-TYPED: false is a valid boolean schema, distinct from falsy coercion)`);
  }

  // Assert NO-TUPLE-ITEMS-PREREQUISITE (M88-DISTINCT-FROM-M87): typed_no_items_cap → typed bucket
  // (type:'array', no items property, contains:{type:'integer'} → MUST be typed, NOT not_applicable)
  if (!typedEntry || !typedEntry.capabilities.includes('typed_no_items_cap')) {
    fail(`${scenario.id}: typed_no_items_cap (type:'array', no items, contains:{type:'integer'}) must be in typed bucket (NO-TUPLE-ITEMS-PREREQUISITE M88-DISTINCT-FROM-M87: contains classifies regardless of items shape)`);
  }

  // Assert EMPTY-OBJECT-SCHEMA-AS-UNTYPED (M88-INHERITED-FROM-M81/M87): untyped_empty_obj_cap → untyped bucket
  const untypedEntry = index.first_property_contains_states.find((e) => e.input_schema_first_property_contains === 'untyped');
  if (!untypedEntry || !untypedEntry.capabilities.includes('untyped_empty_obj_cap')) {
    fail(`${scenario.id}: untyped_empty_obj_cap (contains={}) must be in untyped bucket (EMPTY-OBJECT-SCHEMA-AS-UNTYPED M88-INHERITED-FROM-M81/M87)`);
  }
  if (index.warnings.some((w) => w.capability === 'untyped_empty_obj_cap')) {
    fail(`${scenario.id}: untyped_empty_obj_cap (EMPTY-OBJECT-SCHEMA-AS-UNTYPED → untyped) must NOT produce a warning`);
  }
  ...
  // Assert not_applicable bucket exists and TYPE-APPLICABILITY-ARRAY-RESTRICTION
  const naEntry = index.first_property_contains_states.find((e) => e.input_schema_first_property_contains === 'not_applicable');
  if (!naEntry) {
    fail(`${scenario.id}: not_applicable bucket must be present`);
  }
  if (!naEntry.capabilities.includes('na_type_string_cap')) {
    fail(`${scenario.id}: na_type_string_cap (firstVal.type='string') must be in not_applicable bucket (TYPE-APPLICABILITY-ARRAY-RESTRICTION M88-INHERITED-FROM-M87)`);
  }

  // Assert NULL-AS-UNKNOWN (M88-INHERITED-FROM-M87): unknown_null_cap → unknown bucket
  const unknownEntry = index.first_property_contains_states.find((e) => e.input_schema_first_property_contains === 'unknown');
  if (!unknownEntry || !unknownEntry.capabilities.includes('unknown_null_cap')) {
    fail(`${scenario.id}: unknown_null_cap (contains=null) must be in unknown bucket (NULL-AS-UNKNOWN M88-INHERITED-FROM-M87: Draft-7 schemas are boolean-or-object; null is neither)`);
  }

  // Assert typed bucket has aggregation_key 'existential_element_match_subschema'
  if (typedEntry.aggregation_key !== 'existential_element_match_subschema') {
    fail(`${scenario.id}: typed bucket must have aggregation_key 'existential_element_match_subschema'; got '${typedEntry.aggregation_key}'`);
  }

  // Assert untyped bucket has aggregation_key 'existential_element_match_subschema'
  if (untypedEntry.aggregation_key !== 'existential_element_match_subschema') {
    fail(`${scenario.id}: untyped bucket must have aggregation_key 'existential_element_match_subschema'; got '${untypedEntry.aggregation_key}'`);
  }
